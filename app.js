...
const OVERLAY_Z = 1900;
const MODAL_STACK = [];

function refreshModalZOrder() {
  q("overlay").style.zIndex = String(OVERLAY_Z);
  while (MODAL_STACK.length && (!q(MODAL_STACK[MODAL_STACK.length - 1]) || q(MODAL_STACK[MODAL_STACK.length - 1]).classList.contains("hidden"))) MODAL_STACK.pop();
  const visibleIds = MODAL_STACK.filter(id => q(id) && !q(id).classList.contains("hidden"));
  visibleIds.forEach((id, index) => {
    const el = q(id);
    if (el) el.style.zIndex = String(MODAL_BASE_Z + index * 10);
  });
}
...
function openModal(id) {
  ...
  for (let i = MODAL_STACK.length - 1; i >= 0; i--) { if (MODAL_STACK[i] === id) MODAL_STACK.splice(i, 1); }
  MODAL_STACK.push(id);
  refreshModalZOrder();
}
...
function closeModal(id) {
  ...
  for (let i = MODAL_STACK.length - 1; i >= 0; i--) { if (MODAL_STACK[i] === id) MODAL_STACK.splice(i, 1); }
  refreshModalZOrder();
  if (!MODAL_STACK.length) q("overlay").classList.add("hidden");
}
...
function drugOptionsHtml(selectedId = "") {
  return APP.cache.drugs
    .filter(drug => drug.active !== false)
    .map(drug => `<option value="${esc(drug.id)}" ${String(drug.id) === String(selectedId) ? "selected" : ""}>${esc(drug.tradeName || "")} ${esc(drug.strength || "")}</option>`)
    .join("");
}

function buildRowActionsDropdown(row, options = {}) {
  const allowed = canEditPrescription(row);
  const showDelete = options.showDelete ?? APP.currentRole === "ADMIN";
  const showEdit = row.status !== "Returned" && allowed;
  const showReturn = row.status !== "Returned";
  return `
    <div class="row-actions-dropdown">
      <button class="soft-btn mini-btn row-actions-trigger" type="button">Actions ▾</button>
      <div class="row-actions-menu">
        ${showReturn ? `<button class="soft-btn mini-btn latest-return-btn" type="button" data-id="${row.id}">Return Prescription</button>` : ""}
        ${showEdit ? `<button class="primary-btn mini-btn latest-edit-btn" type="button" data-id="${row.id}">Edit Prescription</button>` : ""}
        ${showDelete ? `<button class="soft-btn mini-btn delete-rx-btn action-link-danger" type="button" data-id="${row.id}">Delete Prescription</button>` : ""}
        ${!showReturn && !showEdit && !showDelete ? `<div class="empty-state">No actions</div>` : ""}
      </div>
    </div>`;
}

function buildShipmentRowHtml(index, item = {}) {
  return `
    <div class="multi-entry-card shipment-row" data-row-index="${index}">
      <div class="multi-entry-card-head">
        <div class="multi-entry-title">Drug ${index + 1}</div>
        <button type="button" class="soft-btn remove-multi-row-btn" data-kind="shipment">Remove</button>
      </div>
      <div class="form-grid two-col">
        <div class="field full"><label>Drug</label><select class="shipment-drug-select">${drugOptionsHtml(item.drugId)}</select></div>
        <div class="field"><label>Boxes</label><input class="shipment-boxes-input" type="number" min="0" value="${Number(item.boxes || 0)}"></div>
        <div class="field"><label>Units</label><input class="shipment-units-input" type="number" min="0" value="${Number(item.units || 0)}"></div>
      </div>
    </div>`;
}

function buildTransferRowHtml(index, item = {}) {
  return `
    <div class="multi-entry-card transfer-row" data-row-index="${index}">
      <div class="multi-entry-card-head">
        <div class="multi-entry-title">Drug ${index + 1}</div>
        <button type="button" class="soft-btn remove-multi-row-btn" data-kind="transfer">Remove</button>
      </div>
      <div class="form-grid two-col">
        <div class="field full"><label>Drug</label><select class="transfer-drug-select">${drugOptionsHtml(item.drugId)}</select></div>
        <div class="field"><label>Boxes</label><input class="transfer-boxes-input" type="number" min="0" value="${Number(item.boxes || 0)}"></div>
        <div class="field"><label>Units</label><input class="transfer-units-input" type="number" min="0" value="${Number(item.units || 0)}"></div>
      </div>
    </div>`;
}

function renderShipmentRows(items = []) {
  const host = q("shipmentRows");
  if (!host) return;
  const safeItems = items.length ? items : [{}];
  host.innerHTML = safeItems.map((item, index) => buildShipmentRowHtml(index, item)).join("");
  host.querySelectorAll('.remove-multi-row-btn').forEach(btn => btn.disabled = safeItems.length === 1);
}

function renderTransferRows(items = []) {
  const host = q("transferRows");
  if (!host) return;
  const safeItems = items.length ? items : [{}];
  host.innerHTML = safeItems.map((item, index) => buildTransferRowHtml(index, item)).join("");
  host.querySelectorAll('.remove-multi-row-btn').forEach(btn => btn.disabled = safeItems.length === 1);
}

function collectShipmentRows() {
  return [...document.querySelectorAll('#shipmentRows .shipment-row')].map(row => ({
    drugId: row.querySelector('.shipment-drug-select')?.value || "",
    boxes: Number(row.querySelector('.shipment-boxes-input')?.value || 0),
    units: Number(row.querySelector('.shipment-units-input')?.value || 0)
  }));
}

function collectTransferRows() {
  return [...document.querySelectorAll('#transferRows .transfer-row')].map(row => ({
    drugId: row.querySelector('.transfer-drug-select')?.value || "",
    boxes: Number(row.querySelector('.transfer-boxes-input')?.value || 0),
    units: Number(row.querySelector('.transfer-units-input')?.value || 0)
  }));
}
  ...
  q("recentPrescriptionsTbody").innerHTML = rows.map(row => {
    ...
    return `
      <tr>
        ...
        <td>${buildRowActionsDropdown(row)}</td>
      </tr>`;
    ...
    return `
      <div class="recent-row-layout">
        ...
        <div class="recent-row-actions">
          ${p.status === "Returned" ? statusBadge("Returned") : buildRowActionsDropdown(p)}
        </div>
      </div>`;
...
function renderDrugRows() {
  ...
  q("drugRxTbody").innerHTML = prescriptionScopeRows().filter(row => row.drugId === APP.selectedDrugId).map(row => `
    <tr>
      ...
      <td>${buildRowActionsDropdown(row)}</td>
    </tr>`).join("") || `<tr><td colspan="9" class="empty-state">No prescriptions for this drug.</td></tr>`;
  renderLiveClocks();
}
...
async function receiveShipment() {
  ...
  const items = collectShipmentRows().filter(item => item.drugId && (item.boxes > 0 || item.units > 0));

  if (!items.length) {
    showActionModal("Receive Shipment", "Please add at least one drug with a quantity greater than zero.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  ...
  try {
    ...
    for (const item of items) {
      const drug = APP.cache.drugs.find(d => d.id === item.drugId);
      if (!drug) throw new Error("One of the selected drugs was not found.");
      const inv = invRow(item.drugId, pharmacy);
      if (!inv?.id) throw new Error(`Inventory row not found for ${drug.tradeName || 'selected drug'} in ${pharmacy}.`);

      const updated = normalizeInventory(
        Number(inv.boxes || 0) + Number(item.boxes || 0),
        Number(inv.units || 0) + Number(item.units || 0),
        Number(drug.unitsPerBox || 1)
      );
      ...
      operations.push(
        ...
        },
        {
          ...
          data: {
            ...
            tradeName: drug.tradeName || "",
            pharmacy,
            qtyBoxes: Number(item.boxes || 0),
            qtyUnits: Number(item.units || 0),
            performedBy: APP.currentRole || "",
            note: `Shipment received${items.length > 1 ? ' (multi-drug)' : ''}`,
            dateTime: jordanNowIso()
          }
        }
      );
      inventoryUpdates.push({ id: inv.id, next: { ...inv, ...updated, updatedAt: jordanNowIso() } });
    }

    await apiRequest("batch", { operations });
    inventoryUpdates.forEach(({ id, next }) => {
      const idx = APP.cache.inventory.findIndex(row => row.id === id);
      if (idx >= 0) APP.cache.inventory[idx] = next;
    });

    renderShipmentRows([{}]);
    closeModal("shipmentModal");
    finishActionModal(true, items.length === 1 ? "Shipment received successfully." : `${items.length} drugs were received successfully.`);
    renderAll();
  } catch (error) {
...
async function transferStock() {
  ...
  const items = collectTransferRows().filter(item => item.drugId && (item.boxes > 0 || item.units > 0));

  if (!from || !to || from === to) {
    showActionModal("Transfer Stock", "Please select two different pharmacy locations.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  if (!items.length) {
    showActionModal("Transfer Stock", "Please add at least one drug with a quantity greater than zero.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  ...
  try {
    ...
    for (const item of items) {
      ...
      const fromInv = invRow(item.drugId, from);
      const toInv = invRow(item.drugId, to);
      if (!fromInv?.id || !toInv?.id) throw new Error(`Inventory row missing for ${drug.tradeName || 'selected drug'}.`);

      const unitsPerBox = Number(drug.unitsPerBox || 1);
      const delta = Number(item.boxes || 0) * unitsPerBox + Number(item.units || 0);
      if (delta <= 0) continue;
      if (delta > Number(fromInv.totalUnits || 0)) {
        throw new Error(`Insufficient stock for ${drug.tradeName || 'selected drug'} in ${from}.`);
      }
      ...
      operations.push(
        { type: "update", table: "inventory", id: fromInv.id, data: { ...updatedFrom, updatedAt: serverTimestamp() } },
        { type: "update", table: "inventory", id: toInv.id, data: { ...updatedTo, updatedAt: serverTimestamp() } },
        { type: "set", table: "transactions", id: crypto.randomUUID(), data: {
          id: crypto.randomUUID(),
          type: "Transfer",
          drugId: item.drugId,
          tradeName: drug.tradeName || "",
          ...
          qtyUnits: Number(item.units || 0),
          performedBy: APP.currentRole || "",
          note: `Stock transfer${items.length > 1 ? ' (multi-drug)' : ''}`,
          dateTime: jordanNowIso()
        }}
      );

      cacheUpdates.push({ id: fromInv.id, next: { ...fromInv, ...updatedFrom, updatedAt: jordanNowIso() } });
      cacheUpdates.push({ id: toInv.id, next: { ...toInv, ...updatedTo, updatedAt: jordanNowIso() } });
    }

    await apiRequest("batch", { operations });
    cacheUpdates.forEach(({ id, next }) => {
      const idx = APP.cache.inventory.findIndex(row => row.id === id);
      if (idx >= 0) APP.cache.inventory[idx] = next;
    });
    ...
    finishActionModal(true, items.length === 1 ? "Stock transferred successfully." : `${items.length} drugs were transferred successfully.`);
    renderAll();
  } catch (error) {
      ...
      <table>
        ...
        <tbody>
          ${rows.map(row => `
            ...
            <tr>
              <td colspan="7"><strong>Status:</strong> ${esc(row.status || "-")} &nbsp; | &nbsp; <strong>Prescription Type:</strong> ${esc(row.prescriptionType || "-")} &nbsp; | &nbsp; <strong>Audit Details:</strong> ${row.status === 'Returned' ? esc((row.returnBy || '') + (row.returnDateTime ? ` • ${formatJordanDateTime(row.returnDateTime)}` : '')) || '-' : esc((row.auditBy || '') + (row.auditDateTime ? ` • ${formatJordanDateTime(row.auditDateTime)}` : '')) || '-'}</td>
            </tr>
...
document.addEventListener("click", event => {
  ...
  const rowActionsTrigger = event.target.closest(".row-actions-trigger");
  if (rowActionsTrigger) {
    const wrap = rowActionsTrigger.closest(".row-actions-dropdown");
    document.querySelectorAll(".row-actions-dropdown.open").forEach(el => { if (el !== wrap) el.classList.remove("open"); });
    wrap?.classList.toggle("open");
    return;
  }

  const removeMultiRowBtn = event.target.closest(".remove-multi-row-btn");
  if (removeMultiRowBtn) {
    const kind = removeMultiRowBtn.dataset.kind;
    const row = removeMultiRowBtn.closest(kind === "shipment" ? ".shipment-row" : ".transfer-row");
    row?.remove();
    if (kind === "shipment" && !document.querySelector('#shipmentRows .shipment-row')) renderShipmentRows([{}]);
    if (kind === "transfer" && !document.querySelector('#transferRows .transfer-row')) renderTransferRows([{}]);
    return;
  }
...
q("openShipmentModalBtn").onclick = () => { refreshScopedSelectors(); renderShipmentRows([{}]); openModal("shipmentModal"); };
q("openShipmentModalBtn2").onclick = () => { refreshScopedSelectors(); renderShipmentRows([{}]); openModal("shipmentModal"); };
q("openTransferModalBtn").onclick = () => { refreshScopedSelectors(); renderTransferRows([{}]); openModal("transferModal"); };
q("openTransferModalBtn2").onclick = () => { refreshScopedSelectors(); renderTransferRows([{}]); openModal("transferModal"); };
...
if (q("addShipmentRowBtn")) q("addShipmentRowBtn").onclick = () => {
  const items = collectShipmentRows();
  items.push({});
  renderShipmentRows(items);
};
if (q("addTransferRowBtn")) q("addTransferRowBtn").onclick = () => {
  const items = collectTransferRows();
  items.push({});
  renderTransferRows(items);
};
...
document.addEventListener("click", event => { if (!event.target.closest(".row-actions-dropdown")) document.querySelectorAll(".row-actions-dropdown.open").forEach(el => el.classList.remove("open")); });
