...
function applyCurrentUserReadonlyFields() {
  ...
  ["quickPharmacist","narcoticPharmacist","narcoticInternalReceivePharmacist","narcoticOrderPharmacist","editPharmacist","narcoticEditPharmacist","confirmActionPharmacist"].forEach(id => {
    const el = q(id);
    if (!el) return;
...
function bindListeners() {
  ...
  APP.listeners.push(onSnapshot(collection(db, "users"), snap => {
    ...
    APP.cache.pharmacists = activeUserRows().map(u => ({
      ...
      workplace: (u.allowedPharmacies || [])[0] || "",
      pharmacies: u.allowedPharmacies || [],
      canAudit: !!u.canAudit,
...
function renderStaticOptions() {
  ...
  const scope = currentScopePharmacy();
  const auditors = activeUserRows().filter(user => user.canAudit).map(user => user.userName || user.fullName || user.displayName || "").filter(Boolean);
  q("auditAuditor").innerHTML = `<option value="">Select Auditor</option>` + auditors.map(name => `<option value="${esc(name)}">${esc(name)}</option>`).join("");
  ...
  if (q("userRoleCheckboxes")) q("userRoleCheckboxes").innerHTML = [
...
function selectedUserPharmacies() {
  return [...document.querySelectorAll('.user-pharmacy-checkbox:checked')].map(el => el.value);
}

function selectedUserRoles() {
  return [...document.querySelectorAll('.user-role-checkbox:checked')].map(el => el.value);
}
...
function renderSettings() {
  ...
  q("saveUserBtn").textContent = APP.userEditId ? "Save Changes" : "Add User";
  ...
  q("usersCards").innerHTML = users.map(user => {
    const pharmacies = (user.allowedPharmacies || []).join(", ") || "-";
...
function resetUserForm() {
  APP.userEditId = null;
  q("userName").value = "";
  q("userJobNumber").value = "";
  q("userPassword").value = "";
  q("userCanAudit").value = "false";
  q("userCanNarcotic").value = "false";
  document.querySelectorAll('.user-pharmacy-checkbox,.user-role-checkbox').forEach(el => el.checked = false);
  if (q("toggleUserActiveBtn")) q("toggleUserActiveBtn").classList.add("hidden");
  if (q("toggleUserActiveBtn")) q("toggleUserActiveBtn").dataset.id = "";
  renderSettings();
}
...
function startEditUser(id) {
  ...
  if (!user) return;
  APP.userEditId = id;
  q("userName").value = user.userName || "";
  q("userJobNumber").value = user.employeeNumber || "";
  q("userPassword").value = "";
  q("userCanAudit").value = user.canAudit ? "true" : "false";
  q("userCanNarcotic").value = user.canManageNarcotic ? "true" : "false";
  const pharmacies = user.allowedPharmacies || [];
  ...
  if (q("toggleUserActiveBtn")) {
    q("toggleUserActiveBtn").classList.remove("hidden");
    q("toggleUserActiveBtn").dataset.id = id;
    q("toggleUserActiveBtn").textContent = user.active !== false ? "Deactivate User" : "Activate User";
  }
  renderSettings();
}
...
async function saveUser() {
  ...
  const employeeNumber = q("userJobNumber").value.trim();
  const password = q("userPassword").value.trim();
  ...
  try {
    ...
    const payload = {
      ...
      canAudit: q("userCanAudit").value === "true",
      canManageNarcotic: q("userCanNarcotic").value === "true",
      active: true,
      updatedAt: serverTimestamp()
    };
    if (password) {
...
function renderNarcoticStaticOptions() {
  ...
  const inpt = inpatientPharmacists().map(p => `<option value="${esc(p.userName || p.name || "")}">${esc(p.userName || p.name || "")}</option>`).join("");
  const narcManagers = narcoticManagerPharmacists().map(p => `<option value="${esc(p.userName || p.name || "")}">${esc(p.userName || p.name || "")}</option>`).join("");
