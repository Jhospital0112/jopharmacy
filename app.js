...
function getUserAllowedPharmacies(user) {
  if (!user) return [];
  if (Array.isArray(user.allowedPharmacies)) return user.allowedPharmacies.filter(Boolean);
  return String(user.allowedPharmacies || user.pharmacyScope || "").split("|").map(v => String(v || "").trim()).filter(Boolean);
}
...
function applyCurrentUserReadonlyFields() {
  const label = currentActorName() ? `${currentActorName()}${currentActorEmployeeNumber() ? ` (${currentActorEmployeeNumber()})` : ""}` : "";
  ["quickPharmacist","narcoticPharmacist","narcoticInternalReceivePharmacist","editPharmacist","confirmActionPharmacist","auditAuditor","narcoticEditPharmacist"].forEach(id => {
    const el = q(id);
    if (!el) return;
    if (el.tagName === "SELECT") {
      el.innerHTML = `<option value="${esc(currentActorName())}">${esc(label || currentActorName())}</option>`;
      el.value = currentActorName();
      el.disabled = true;
    } else {
      el.value = label;
      el.readOnly = true;
...
async function continueQuickRegistration(payload) {
  ...
  try {
    ...
    await setDoc(prescriptionRef, {
      drugId, pharmacy, patientName, fileNumber, doctorName, pharmacistName, prescriptionType: prescriptionType || "", qtyBoxes, qtyUnits, status: "New", auditBy: "", auditDateTime: null, auditNote: "", returnBy: "", returnDateTime: null, returnNote: "", dateTime: jordanNowIso(), ...currentActorMeta(), createdAt: jordanNowIso(), updatedBy: currentActorName()
    });
    const txRef = doc(collection(db, "transactions"));
    await setDoc(txRef, { type: "Dispense", drugId, tradeName: drug.tradeName, pharmacy, qtyBoxes, qtyUnits, performedBy: currentActorName(), performedByEmployeeNumber: currentActorEmployeeNumber(), note: `Prescription: ${patientName}`, dateTime: jordanNowIso(), ...currentActorMeta() });
...
async function registerQuickPrescription() {
  ...
  if (!payload.drugId || !payload.patientName || !payload.fileNumber || !payload.doctorName) {
...
async function savePassword() {
  ...
  if (!next || next !== confirm) {
    showActionModal("Password", "New passwords do not match.", false); q("actionOkBtn").classList.remove("hidden"); return;
  }
  if (APP.loginMethod !== "MASTER_OVERRIDE" && await sha256(current) !== APP.currentUser.passwordHash) {
    showActionModal("Password", "Current password is incorrect.", false); q("actionOkBtn").classList.remove("hidden"); return;
  }
  showActionModal("Change Password", "Please wait while the password is being updated...");
  const hash = await sha256(next);
  await updateDoc(doc(db, "users", currentSessionUserId()), { passwordHash: hash, mustChangePassword: false, updatedAt: serverTimestamp() });
...
async function openResetPasswordModal() {
  if (APP.currentRole !== "ADMIN") return;
  const users = APP.cache.users?.length ? APP.cache.users : await fetchRows(collection(db, "users"));
  q("resetPasswordRole").innerHTML = (users || []).map(u => `<option value="${esc(u.id)}">${esc((u.fullName || "User") + " · " + (u.employeeNumber || ""))}</option>`).join("");
...
async function resetSelectedPassword() {
  ...
  await updateDoc(doc(db, "users", userId), { passwordHash: hash, mustChangePassword: false, updatedAt: serverTimestamp() });
