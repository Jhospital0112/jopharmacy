...
function renderPatientsPage() {
  ...
  q("patientsTbody").innerHTML = rows.length ? rows.map(({ rx, calc }) => {
    ...
    return `<tr>
      ...
      <td><button class="soft-btn add-dose-btn" data-add-dose="${esc(rx.id)}">Add Dose</button></td>
...
async function continueQuickRegistration(payload) {
  ...
  try {
    ...
    await setDoc(prescriptionRef, {
      drugId, pharmacy, patientName, fileNumber, doctorName, pharmacistName, prescriptionType: prescriptionType || "", qtyBoxes, qtyUnits, status: "New", auditBy: "", auditDateTime: null, auditNote: "", returnBy: "", returnDateTime: null, returnNote: "", dateTime: jordanNowIso(), createdBy: APP.currentRole, updatedBy: actorDisplayName()
    });
...
function txPrintText(value, fallback = "-") {
  const v = value === undefined || value === null || value === "" ? fallback : value;
  return esc(String(v));
}

function txPrintQtyBoxesUnits(boxes, units) {
  return `${Number(boxes || 0)} box(es) + ${Number(units || 0)} unit(s)`;
}

function txPrintQtyUnits(units) {
  return `${Number(units || 0)} unit(s)`;
}

function txPrintMetaLine(label, value) {
  return `<div class="section" style="margin-top:10px"><strong>${esc(label)}:</strong> ${txPrintText(value)}</div>`;
}

function txPrintTable(headers, rows) {
  return `
    <div class="section">
      <table>
...
function txPrintSignatureBlock(leftName, rightName, leftLabel = "Transferred By", rightLabel = "Received By") {
  return `
    <div class="section" style="margin-top:26px">
      <table>
        <thead>
          <tr>
            <th style="width:50%">${esc(leftLabel)} Signature</th>
            <th style="width:50%">${esc(rightLabel)} Signature</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td style="height:82px;vertical-align:bottom">
              <div><strong>Name:</strong> ${txPrintText(leftName)}</div>
              <div style="margin-top:28px;border-top:1px solid #d8e0eb;padding-top:8px">Signature</div>
            </td>
            <td style="height:82px;vertical-align:bottom">
              <div><strong>Name:</strong> ${txPrintText(rightName)}</div>
              <div style="margin-top:28px;border-top:1px solid #d8e0eb;padding-top:8px">Signature</div>
            </td>
          </tr>
        </tbody>
...
function buildPrintedNormalSelectedTransaction(row) {
  if (!row) return `<div class="section-title">Transaction not found.</div>`;

  if (row.type === "Receive Shipment") {
    const rows = (APP.cache.transactions || []).filter(r => (row.batchId && r.batchId === row.batchId) || String(r.id) === String(row.id));
    const first = rows[0] || row;
    return `
      <div class="section-title">Receive Shipment Transaction</div>
      ${txPrintTable(
        ["Date & Time", "Pharmacy", "Drug", "Boxes", "Units", "Invoice Number", "Invoice Date"],
        rows.map(item => [
          txPrintText(formatJordanDateTime(item.receivedDateTime || item.dateTime || "")),
          txPrintText(item.pharmacy || ""),
          txPrintText(item.tradeName || ""),
          txPrintText(Number(item.qtyBoxes || 0)),
          txPrintText(Number(item.qtyUnits || 0)),
          txPrintText(item.invoiceNumber || ""),
          txPrintText(item.invoiceDate || "")
        ])
      )}
      ${txPrintMetaLine("Received By", first.receivedBy || first.performedBy || "-")}`;
  }
  ...
  if (row.type === "Transfer") {
    ...
    const first = rows[0] || row;
    return `
      <div class="section-title">Stock Transfer Transaction</div>
      ${txPrintTable(
        ["Date & Time", "From Pharmacy", "To Pharmacy", "Drug", "Boxes", "Units", "Transferred By", "Received By"],
        rows.map(item => [
          txPrintText(formatJordanDateTime(item.dateTime || "")),
          txPrintText(item.fromPharmacy || ""),
          txPrintText(item.toPharmacy || ""),
          txPrintText(item.tradeName || ""),
          txPrintText(Number(item.qtyBoxes || 0)),
          txPrintText(Number(item.qtyUnits || 0)),
          txPrintText(item.performedBy || "-"),
          txPrintText(item.receiverPharmacist || "-")
        ])
      )}
      ${txPrintSignatureBlock(first.performedBy || "-", first.receiverPharmacist || "-", "Transferred By", "Received By")}`;
  }

  if (row.type === "Register") {
    return `
      <div class="section-title">Dispensed Prescription Transaction</div>
      ${txPrintTable(
        ["Patient Name", "File Number", "Pharmacy", "Doctor Name", "Dispensed Quantity"],
        [[
          txPrintText(row.patientName || ""),
          txPrintText(row.fileNumber || ""),
          txPrintText(row.pharmacy || ""),
          txPrintText(row.doctorName || ""),
          txPrintText(txPrintQtyBoxesUnits(row.qtyBoxes, row.qtyUnits))
        ]]
      )}
      ${txPrintMetaLine("Dispensed By", row.registeredBy || row.pharmacistName || row.performedBy || "-")}
      ${txPrintMetaLine("Date & Time", formatJordanDateTime(row.registeredDateTime || row.dateTime || ""))}`;
  }
  ...
  if (row.type === "Edit Prescription") {
    ...
    const newValues = row.newValues || {};
    return `
      <div class="section-title">Edited Prescription Transaction</div>
      <div class="section-title" style="font-size:14px">Before Edit</div>
      ${txPrintTable(
        ["Patient Name", "File Number", "Pharmacy", "Doctor Name", "Quantity Before Edit"],
        [[
          txPrintText(oldValues.patientName || row.patientName || ""),
          txPrintText(oldValues.fileNumber || row.fileNumber || ""),
          txPrintText(row.pharmacy || ""),
          txPrintText(oldValues.doctorName || row.doctorName || ""),
          txPrintText(txPrintQtyBoxesUnits(oldValues.qtyBoxes, oldValues.qtyUnits))
        ]]
      )}
      <div class="section-title" style="font-size:14px">After Edit</div>
      ${txPrintTable(
        ["Patient Name", "File Number", "Pharmacy", "Doctor Name", "Quantity After Edit"],
        [[
          txPrintText(newValues.patientName || row.patientName || ""),
          txPrintText(newValues.fileNumber || row.fileNumber || ""),
          txPrintText(row.pharmacy || ""),
          txPrintText(newValues.doctorName || row.doctorName || ""),
          txPrintText(txPrintQtyBoxesUnits(newValues.qtyBoxes, newValues.qtyUnits))
        ]]
      )}
      ${txPrintMetaLine("Edited By", row.editedBy || row.performedBy || "-")}
      ${txPrintMetaLine("Date & Time", formatJordanDateTime(row.editedDateTime || row.dateTime || ""))}`;
  }

  if (row.type === "Delete Prescription") {
    return `
      <div class="section-title">Deleted Prescription Transaction</div>
      ${txPrintTable(
        ["Patient Name", "File Number", "Pharmacy", "Doctor Name", "Dispensed Quantity"],
        [[
          txPrintText(row.patientName || ""),
      ...
      ${txPrintMetaLine("Deleted By", row.deletedBy || row.performedBy || "-")}
      ${txPrintMetaLine("Date & Time", formatJordanDateTime(row.deletedDateTime || row.dateTime || ""))}`;
  }

  if (row.type === "Return") {
    return `
      <div class="section-title">Returned Prescription Transaction</div>
      ${txPrintTable(
        ["Patient Name", "File Number", "Pharmacy", "Doctor Name", "Returned Quantity"],
        [[
          txPrintText(row.patientName || ""),
          txPrintText(row.fileNumber || ""),
          txPrintText(row.pharmacy || ""),
      ...
      ${txPrintMetaLine("Returned By", row.returnBy || row.performedBy || "-")}
      ${txPrintMetaLine("Date & Time", formatJordanDateTime(row.returnDateTime || row.dateTime || ""))}`;
  }

  return `
    <div class="section-title">${txPrintText(row.type || "Transaction")}</div>
    ${txPrintTable(
      ["Drug", "Pharmacy", "Performed By", "Date & Time", "Note"],
      [[
        txPrintText(row.tradeName || ""),
        txPrintText(row.pharmacy || ""),
        txPrintText(row.performedBy || ""),
        txPrintText(formatJordanDateTime(row.dateTime || "")),
        txPrintText(row.note || "")
      ]]
    )}`;
}

function buildPrintedNarcoticSelectedTransaction(row) {
  if (!row) return `<div class="section-title">Transaction not found.</div>`;

  if (row.type === "Internal Stock Receipt") {
    return `
      <div class="section-title">Narcotic Receive Shipment Transaction</div>
      ${txPrintTable(
        ["Date & Time", "Pharmacy", "Drug", "Received Quantity", "Invoice Number", "Invoice Date"],
        [[
          txPrintText(formatJordanDateTime(row.dateTime || "")),
          txPrintText("In-Patient Pharmacy"),
          txPrintText(row.drugName || ""),
          txPrintText(Number(row.quantityReceived || 0)),
          txPrintText(row.invoiceNumber || ""),
          txPrintText(row.invoiceDate || "")
        ]]
      )}
      ${txPrintMetaLine("Received By", row.performedBy || "-")}`;
  }

  if (row.type === "Department Order Movement") {
    return `
      <div class="section-title">Narcotic Stock Transfer Transaction</div>
      ${txPrintTable(
        ["Date & Time", "From Pharmacy", "To Department", "Drug", "Transferred Quantity", "Transferred By", "Received By"],
        [[
          txPrintText(formatJordanDateTime(row.dateTime || "")),
          txPrintText("In-Patient Pharmacy"),
          txPrintText(row.departmentName || ""),
          txPrintText(row.drugName || ""),
          txPrintText(Number(row.quantitySent || 0)),
          txPrintText(row.performedBy || "-"),
          txPrintText(row.nurseName || "-")
        ]]
      )}
      ${txPrintSignatureBlock(row.performedBy || "-", row.nurseName || "-", "Transferred By", "Received By")}`;
  }

  if (row.type === "Register") {
    return `
      <div class="section-title">Narcotic Dispensed Prescription Transaction</div>
      ${txPrintTable(
        ["Patient Name", "File Number", "Department", "Doctor / Rx Number", "Dispensed Quantity"],
        [[
          txPrintText(row.patientName || ""),
          txPrintText(row.fileNumber || ""),
          txPrintText(row.departmentName || ""),
          txPrintText(row.prescriptionNumber || "-"),
          txPrintText(txPrintQtyUnits(row.dispensedUnits || 0))
        ]]
      )}
      ${txPrintMetaLine("Dispensed By", row.performedBy || "-")}
      ${txPrintMetaLine("Date & Time", formatJordanDateTime(row.dateTime || ""))}`;
  }

  if (row.type === "Delete Prescription") {
    return `
      <div class="section-title">Deleted Narcotic Prescription Transaction</div>
      ${txPrintTable(
        ["Patient Name", "File Number", "Department", "Drug", "Dispensed Quantity"],
        [[
          txPrintText(row.patientName || ""),
          txPrintText(row.fileNumber || ""),
          txPrintText(row.departmentName || ""),
          txPrintText(row.drugName || ""),
          txPrintText(txPrintQtyUnits(row.quantitySent || 0))
        ]]
      )}
      ${txPrintMetaLine("Deleted By", row.deletedBy || row.performedBy || "-")}
      ${txPrintMetaLine("Date & Time", formatJordanDateTime(row.deletedDateTime || row.dateTime || ""))}`;
  ...
  if (row.type === "Return") {
    return `
      <div class="section-title">Returned Narcotic Prescription Transaction</div>
      ${txPrintTable(
  ...
  return `
    <div class="section-title">${txPrintText(row.type || "Narcotic Transaction")}</div>
    ${txPrintTable(
      ["Drug", "Department", "Pharmacist", "Date & Time", "Notes"],
      [[
        txPrintText(row.drugName || ""),
        txPrintText(row.departmentName || ""),
        txPrintText(row.performedBy || ""),
        txPrintText(formatJordanDateTime(row.dateTime || "")),
        txPrintText(row.notes || "")
      ]]
    )}`;
}
...
function printSelectedTransaction() {
  ...
  let body = `<div class="section-title">Transaction not found.</div>`;
  let title = "Selected Transaction";
  let subtitle = currentScopePharmacy();

  if (info.scope === "narcotic") {
    const row = (APP.cache.narcoticOrderMovements || []).find(item => String(item.id) === String(info.id));
    body = buildPrintedNarcoticSelectedTransaction(row);
    title = "Narcotic Selected Transaction";
    subtitle = "Narcotic Drugs";
  } else {
    const row = (APP.cache.transactions || []).find(item => String(item.id) === String(info.id));
    body = buildPrintedNormalSelectedTransaction(row);
    title = "Selected Transaction";
    subtitle = row?.pharmacy || currentScopePharmacy();
  }

  const w = window.open("", "_blank");
  if (!w) return;
  w.document.write(buildPrintShell(title, subtitle, body));
  w.document.close();
}
