...
function resetNarcoticEntryForm() {
  ["narcoticPatientName","narcoticFileNumber","narcoticNationalId","narcoticPrescriptionNumber","narcoticDoctorName","narcoticDose","narcoticDiscardDose","narcoticNotes","narcoticDispensedUnits"].forEach(id => { if (q(id)) q(id).value = ""; });
...
function renderNarcoticRecent() {
  ...
  q("narcoticRecentTbody").innerHTML = rows.map(row => {
    ...
    return `<tr>
      ...
      <td>${esc(row.dose || "")}</td>
      <td>${Number(row.dispensedUnits || 1)}</td>
...
function renderNarcoticDrugCards() {
  ...
  q("narcoticDrugCards").innerHTML = APP.cache.narcoticDrugs.map(drug => {
    const totalAvail = APP.cache.narcoticDepartmentStock.filter(r => r.drugId === drug.id).reduce((s,r)=>s+Number(r.availableStockUnits||0),0);
    return `<div class="drug-card">
      <div class="drug-title">${esc(drug.tradeName)} ${esc(drug.strength || "")}</div>
      <div class="drug-meta">${esc(drug.scientificName || "")}</div>
      <div class="drug-meta">${esc(drug.dosageForm || "")}</div>
      <div class="narcotic-drug-badge">Narcotic drug</div>
      <div class="drug-stock"><div>Total available<strong>${Number(totalAvail)} units</strong></div></div>
    </div>`;
  }).join("") || `<div class="empty-state">No narcotic drugs configured.</div>`;
}
...
function renderNarcoticManageDrugsTable() {
  ...
  q("narcoticManageDrugsTbody").innerHTML = APP.cache.narcoticDrugs.map(drug => `
    <tr>
      ...
      <td>${esc(drug.dosageForm || "")}</td>
      <td>${Number(drug.unitsPerBox || 0)}</td>
      <td>${Number(drug.reorderLevelUnits || 0)}</td>
      <td><button class="mini-danger-btn mini-btn delete-narcotic-drug-btn" data-id="${drug.id}">Delete</button></td>
    </tr>
  `).join("") || `<tr><td colspan="7" class="empty-state">No narcotic drugs configured.</td></tr>`;
}
function renderNarcoticOrdersTable() {
  ...
  `).join("") || `<tr><td colspan="6" class="empty-state">No order rows added yet.</td></tr>`;
}
...
async function registerNarcoticPrescription() {
  const payload = {
    ...
    dispensedUnits: Math.max(1, Number(q("narcoticDispensedUnits").value || 1)),
    discardDose: q("narcoticDiscardDose").value.trim(),
    pharmacist: q("narcoticPharmacist").value.trim(),
    notes: q("narcoticNotes").value.trim()
  };
  if (!payload.drugId || !payload.departmentId || !payload.patientName || !payload.fileNumber || !payload.prescriptionNumber || !payload.doctorName || !payload.dose || !payload.pharmacist) {
  ...
  if (!stockRow || Number(stockRow.availableStockUnits || 0) < Number(payload.dispensedUnits || 1)) {
  ...
  batch.update(doc(db, "narcotic_department_stock", stockRow.id), {
    availableStockUnits: Math.max(0, Number(stockRow.availableStockUnits || 0) - Number(payload.dispensedUnits || 1)),
    updatedAt: serverTimestamp()
  });
...
function addNarcoticOrderRow() {
  ...
  APP.narcoticOrdersBatchRows.push({
    ...
    emptyAmpoulesReceived, quantitySent, performedBy, notes: q("narcoticOrderNotes").value.trim()
  });
  q("narcoticOrderEmpty").value = "0"; q("narcoticOrderSent").value = "0"; q("narcoticOrderNotes").value = "";
  renderNarcoticOrdersTable();
  return true;
}
async function submitNarcoticOrders() {
  if (!(APP.narcoticOrdersBatchRows || []).length && !addNarcoticOrderRow()) return;
  const overflowRows = (APP.narcoticOrdersBatchRows || []).filter(row => {
    const stockRow = narcoticDeptStockRow(row.departmentId, row.drugId);
    const fixed = Number(stockRow?.fixedStockUnits || 0);
    const nextAvailable = Number(stockRow?.availableStockUnits || 0) + Number(row.quantitySent || 0);
    return nextAvailable > fixed;
  });
  if (overflowRows.length) {
    const first = overflowRows[0];
    const proceed = window.confirm(`Available stock will become higher than fixed stock for ${first.departmentName} / ${first.drugLabel}.

Press OK to continue or Cancel to stop the operation.`);
    if (!proceed) return;
  }
  showActionModal("Department Orders", "Please wait while department order movements are being saved...");
  const batch = writeBatch(db);
  for (const row of APP.narcoticOrdersBatchRows) {
    const stockRow = narcoticDeptStockRow(row.departmentId, row.drugId);
    const nextAvailable = Number(stockRow?.availableStockUnits || 0) + Number(row.quantitySent || 0);
    if (stockRow?.id) {
...
function openNarcoticDepartmentModal(departmentId) {
  ...
  if (q("narcoticDeptModalFrom")) q("narcoticDeptModalFrom").value = jordanDateKey();
  if (q("narcoticDeptModalTo")) q("narcoticDeptModalTo").value = jordanDateKey();
...
function renderNarcoticDepartmentModal() {
  ...
  const summaryEl = q("narcoticDeptStockSummary");
  const adminGrid = q("narcoticDeptStockAdminGrid");
  const tbody = q("narcoticDeptModalTbody");
  if (!drugId) {
    summaryEl?.classList.add("hidden");
    adminGrid?.classList.add("hidden");
    if (tbody) tbody.innerHTML = `<tr><td colspan="9" class="empty-state">Select a drug to view today's prescriptions and stock details.</td></tr>`;
    if (q("narcoticDeptFixedStock")) q("narcoticDeptFixedStock").value = 0;
    if (q("narcoticDeptAvailableStock")) q("narcoticDeptAvailableStock").value = 0;
    return;
  }
  let rows = APP.cache.narcoticPrescriptions.filter(r => r.departmentId === departmentId && r.drugId === drugId);
  const today = jordanDateKey();
  const from = q("narcoticDeptModalFrom")?.value || today;
  const to = q("narcoticDeptModalTo")?.value || today;
  rows = rows.filter(r => {
  ...
  if (tbody) tbody.innerHTML = rows.map(r => `<tr>
    ...
    <td>${esc(r.dose || "")}</td>
    <td>${Number(r.dispensedUnits || 1)}</td>
    <td>${esc(r.discardDose || "")}</td>
    <td>${esc(r.pharmacist || "")}</td>
  </tr>`).join("") || `<tr><td colspan="9" class="empty-state">No prescriptions found for the selected drug and date range.</td></tr>`;
  const selectedStock = narcoticDeptStockRow(departmentId, drugId) || null;
  const fixed = Number(selectedStock?.fixedStockUnits || 0);
  const avail = Number(selectedStock?.availableStockUnits || 0);
  if (summaryEl) {
    summaryEl.classList.remove("hidden");
    summaryEl.innerHTML = `
      <div class="narcotic-stock-chip">${esc(dept?.departmentName || "")}</div>
      <div class="narcotic-stock-chip">Fixed Stock: ${fixed}</div>
      <div class="narcotic-stock-chip">Available Stock: ${avail}</div>
      <div class="narcotic-stock-chip">Prescriptions: ${rows.length}</div>`;
  }
  if (adminGrid) adminGrid.classList.toggle("hidden", APP.currentRole !== "ADMIN");
...
async function addNarcoticDrug() {
  ...
  const reorderLevelUnits = Math.max(0, Number(q("narcoticManageReorderLevel")?.value || 0));
  ...
  if (q("narcoticManageReorderLevel")) q("narcoticManageReorderLevel").value = 0;
...
function filterNarcoticPrescriptionsForReport() {
  const {departmentId, drugId, from, to} = narcoticReportFilters();
...
function printNarcoticDispensingReport() {
  ...
  const totalDiscard = rows.reduce((s,r)=>s+(Number(r.discardDose)||0),0);
  const body = `<div class="section-title">Narcotic Dispensing Report</div>
    <div class="section"><table><thead><tr><th>Patient</th><th>File No.</th><th>Prescription No.</th><th>Doctor</th><th>Dose</th><th>Discard</th><th>Department</th><th>Pharmacist</th></tr></thead>
    <tbody>${rows.map(r=>`<tr><td>${esc(r.patientName||"")}</td><td>${esc(r.fileNumber||"")}</td><td>${esc(r.prescriptionNumber||"")}</td><td>${esc(r.doctorName||"")}</td><td>${esc(r.dose||"")}</td><td>${esc(r.discardDose||"")}</td><td>${esc(r.departmentName||"")}</td><td>${esc(r.pharmacist||"")}</td></tr>`).join("")}</tbody></table></div>
    <div class="section"><strong>Total prescriptions:</strong> ${rows.length}<br><strong>Total discarded units:</strong> ${totalDiscard}</div>`;
  const w = window.open("", "_blank"); w.document.write(buildPrintShell("Narcotic Dispensing Report", "Narcotic Drugs", body)); w.document.close();
}
function printNarcoticMovementsReport() {
  ...
  const body = `<div class="section-title">Department Order Movements Report</div>
    <div class="section"><table><thead><tr><th>Date & Time</th><th>Department</th><th>Drug</th><th>Empty Received</th><th>Quantity Sent</th><th>By</th><th>Notes</th></tr></thead>
    <tbody>${rows.map(r=>`<tr><td>${esc(formatJordanDateTime(r.dateTime))}</td><td>${esc(r.departmentName||"")}</td><td>${esc(r.drugName||"")}</td><td>${Number(r.emptyAmpoulesReceived||0)}</td><td>${Number(r.quantitySent||0)}</td><td>${esc(r.performedBy||"")}</td><td>${esc(r.notes||"")}</td></tr>`).join("")}</tbody></table></div>
    <div class="section"><strong>Total movements:</strong> ${rows.length}<br><strong>Total empty ampoules received:</strong> ${totalEmpty}<br><strong>Total quantity sent:</strong> ${totalSent}</div>`;
  const w = window.open("", "_blank"); w.document.write(buildPrintShell("Department Order Movements Report", "Narcotic Drugs", body)); w.document.close();
}
function printNarcoticStockReport() {
  const departmentId = q("narcoticReportDepartment")?.value || "";
  const rows = APP.cache.narcoticDepartmentStock.filter(r => !departmentId || r.departmentId === departmentId);
  const body = `<div class="section-title">Department Stock Report</div>
    <div class="section"><table><thead><tr><th>Department</th><th>Drug</th><th>Fixed Stock</th><th>Available Stock</th><th>Difference</th></tr></thead>
    <tbody>${rows.map(r=>`<tr><td>${esc(r.departmentName||"")}</td><td>${esc(r.drugName||"")}</td><td>${Number(r.fixedStockUnits||0)}</td><td>${Number(r.availableStockUnits||0)}</td><td>${Number(r.fixedStockUnits||0)-Number(r.availableStockUnits||0)}</td></tr>`).join("")}</tbody></table></div>`;
  const w = window.open("", "_blank"); w.document.write(buildPrintShell("Department Stock Report", "Narcotic Drugs", body)); w.document.close();
}
