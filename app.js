const API_URL = (window.CDMS_CONFIG && window.CDMS_CONFIG.APPS_SCRIPT_WEB_APP_URL) || (window.APP_CONFIG && window.APP_CONFIG.APPS_SCRIPT_WEB_APP_URL) || "";

if (!API_URL || API_URL.includes("PASTE_YOUR")) {
  alert("Google Sheets Apps Script is not configured yet. Open config.js and paste your APPS_SCRIPT_WEB_APP_URL first.");
}
...
async function getDoc(docRef) {
  const res = await apiRequest("getDoc", { table: docRef.table, id: docRef.id });
  return makeDocSnapshot(res.data || null);
}
...
function applyLocalFilters(rows, filters = []) {
  return (rows || []).filter(row => {
    return (filters || []).every(f => {
      if (!f) return true;
      if (f.op === "==" || f.op === "eq") return row?.[f.field] === f.value;
      return true;
    });
  });
}
...
async function fetchRows(ref) {
  const res = await apiRequest("listDocs", { table: ref.table });
  const rows = res.data || [];
  if (ref.kind === "query") return applyLocalFilters(rows, ref.filters);
  return rows;
}
...
function onSnapshot(ref, callback) {
  ...
  const emit = async () => {
    ...
    try {
      if (ref.kind === "doc") {
        ...
        const payload = snap.exists() ? { id: ref.id, ...snap.data() } : null;
      ...
      const signature = stableStringify(rows);
      if (signature !== lastSignature) {
        lastSignature = signature;
  ...
  emit();
  const timer = setInterval(emit, SNAPSHOT_POLL_MS);
...
// Internal app password only. Data is stored through the connected Google Sheets Apps Script backend.
...
async function registerQuickPrescription() {
  ...
  const bypassDuplicateWarning = String(payload.fileNumber || '').trim() === '999444';
  const duplicates = bypassDuplicateWarning ? [] : findDuplicatePrescriptions(payload);
