    ...
    <head>
      ...
      <style>
        ...
        *{box-sizing:border-box}
        html,body{margin:0;padding:0;background:#eef3f9;font-family:Inter,Arial,sans-serif;color:#16304f}
        body{padding:0}
        .sheet{display:flex;flex-direction:column;gap:0}
        .rx-label{width:95mm;height:60mm;padding:4.8mm 5.2mm;background:#fff;position:relative;overflow:hidden;page-break-after:always}
        .rx-label:last-child{page-break-after:auto}
        .frame{height:100%;border:1px solid #d7e2ef;border-radius:4mm;padding:4mm;background:linear-gradient(180deg,#ffffff 0%,#f8fbff 100%);display:flex;flex-direction:column;gap:2.2mm;box-shadow:inset 0 0 0 1px rgba(255,255,255,.65)}
        .top{display:flex;align-items:flex-start;justify-content:space-between;gap:2mm}
        .system{font-size:2.55mm;font-weight:800;letter-spacing:.08mm;color:#1b446f;text-transform:uppercase;line-height:1.2;max-width:46mm}
        .pill{padding:1.1mm 2.1mm;border-radius:999px;background:#edf4fc;border:1px solid #d7e2ef;font-size:2.25mm;font-weight:800;color:#36506e;white-space:nowrap}
        .drug{padding:2.2mm 2.6mm;border-radius:3.2mm;background:linear-gradient(180deg,#1f5f9e,#183f6a);color:#fff;display:grid;gap:1.6mm}
        .drug-name{font-size:4.4mm;font-weight:900;line-height:1.15;word-break:break-word}
        .drug-qty-row{margin-top:.2mm;padding-top:1.5mm;border-top:1px solid rgba(255,255,255,.22);display:flex;align-items:center;justify-content:space-between;gap:2mm}
        .drug-qty-label{font-size:2.2mm;font-weight:700;color:rgba(255,255,255,.82);line-height:1}
        .drug-qty-value{font-size:3.15mm;font-weight:900;color:#fff;line-height:1.15;text-align:right;max-width:37mm;word-break:break-word}
        .meta-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1.9mm}
        .metric{border:1px solid #dce6f1;border-radius:3mm;padding:1.7mm 2mm;background:#fbfdff;min-height:13mm;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}
        .metric-label{font-size:2.25mm;font-weight:700;color:#67809a;line-height:1.2;text-align:center;width:100%}
        .metric-value{margin-top:1mm;font-size:4.8mm;font-weight:900;color:#16304f;line-height:1;text-align:center;width:100%;display:flex;align-items:center;justify-content:center}
        .footer{margin-top:auto;border-top:1px dashed #cdd9e7;padding-top:2mm;display:grid;gap:1.4mm}
        .footer-row{display:flex;justify-content:space-between;gap:2mm;font-size:2.35mm;color:#4b627c;font-weight:700}
        .footer-row strong{color:#173a66;font-weight:800}
      </style>
    </head>
    <body>
  ...
  const labelsHtml = selectedDrugIds.map(currentDrugId => {
    ...
    return `
      <section class="rx-label">
        <div class="frame">
          <div class="top">
            <div class="system">Controlled Drugs Management System</div>
          ...
          <div class="footer">
            <div class="footer-row"><span><strong>From:</strong> ${esc(from || '-')}</span><span><strong>To:</strong> ${esc(to || '-')}</span></div>
            <div class="footer-row"><span><strong>Pharmacy:</strong> ${esc(pharmacyLabel)}</span><span><strong>Printed:</strong> ${esc(formatJordanDateTime(jordanNowIso()))}</span></div>
          </div>
        </div>
      </section>`;
  }).join('');
