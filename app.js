...
function openDrugLabelModal() {
  if (!APP.selectedDrugId) return;
  const drug = APP.cache.drugs.find(d => d.id === APP.selectedDrugId);
  if (!drug) return;
  const title = document.querySelector(".app-title")?.textContent?.trim() || "CONTROLLED DRUGS MANAGEMENT";
  if (q("drugLabelSystemName")) q("drugLabelSystemName").textContent = title;
  if (q("drugLabelDrugName")) q("drugLabelDrugName").textContent = `${drug.tradeName || ""} ${drug.strength || ""}`.trim() || "-";
  if (q("drugLabelFromDate")) q("drugLabelFromDate").value = q("drugRxFromDate")?.value || jordanDateKey();
  if (q("drugLabelToDate")) q("drugLabelToDate").value = q("drugRxToDate")?.value || jordanDateKey();
  openModal("drugLabelModal");
}

function printDrugSummaryLabel() {
  if (!APP.selectedDrugId) return;
  const drug = APP.cache.drugs.find(d => d.id === APP.selectedDrugId);
  if (!drug) return;

  const from = q("drugLabelFromDate")?.value || "";
  const to = q("drugLabelToDate")?.value || "";
  const effectiveFrom = from || (!to ? jordanDateKey() : "");
  const effectiveTo = to || (!from ? jordanDateKey() : "");
  const rows = prescriptionScopeRows().filter(row => {
    if (row.drugId !== APP.selectedDrugId) return false;
  ...
  const activeRows = rows.filter(row => !["returned", "cancelled"].includes(String(row.status || "").toLowerCase()));
  const returnedRows = rows.filter(row => String(row.status || "").toLowerCase() === "returned");
  const totalUnits = activeRows.reduce((sum, row) => {
    return sum + (Number(row.qtyBoxes || 0) * Number(drug?.unitsPerBox || 1)) + Number(row.qtyUnits || 0);
  }, 0);
  const normalized = normalizeInventory(0, totalUnits, Number(drug?.unitsPerBox || 1));
  const title = document.querySelector(".app-title")?.textContent?.trim() || "CONTROLLED DRUGS MANAGEMENT";
  const body = `
    <div class="label-sheet">
      <div class="label-head">
        <div class="label-system">${esc(title)}</div>
        <div class="label-caption">Drug Summary Label</div>
      </div>
      <div class="label-drug">${esc(`${drug.tradeName || ""} ${drug.strength || ""}`.trim() || "-")}</div>
      <div class="label-grid">
        <div class="label-card wide">
          <span>Period</span>
          <strong>${esc(`${effectiveFrom || "-"} to ${effectiveTo || "-"}`)}</strong>
        </div>
        ...
        <div class="label-card">
          <span>Returned</span>
          <strong>${returnedRows.length}</strong>
        </div>
        <div class="label-card wide qty">
          <span>Dispensed Quantity</span>
          <strong>${normalized.boxes} Box + ${normalized.units} ${esc(unitLabel(drug))}</strong>
        </div>
      </div>
    </div>`;
  ...
  w.document.write(`
    <html>
      <head>
        <title>${esc(`${drug.tradeName || ""} ${drug.strength || ""}`.trim() || "Drug Label")}</title>
        <style>
          @page{size:269pt 170pt;margin:0}
          *{box-sizing:border-box}
          html,body{margin:0;padding:0;background:#eef3f8;font-family:Inter,Arial,sans-serif}
          body{display:flex;align-items:flex-start;justify-content:center;padding:18px}
          .label-sheet{width:269pt;min-height:170pt;background:#fff;border:1.2pt solid #d7e1ee;border-radius:18pt;padding:14pt 16pt;box-shadow:0 10pt 28pt rgba(17,46,91,.12);color:#17324d}
          .label-head{display:flex;justify-content:space-between;align-items:flex-start;gap:10pt;padding-bottom:9pt;border-bottom:1.2pt solid #dbe5f1}
          .label-system{font-size:7.8pt;font-weight:900;line-height:1.35;letter-spacing:.9pt;text-transform:uppercase;max-width:150pt;color:#173a66}
          .label-caption{font-size:8.2pt;font-weight:800;padding:4pt 7pt;border-radius:999pt;background:#edf4fb;color:#285785;white-space:nowrap}
          .label-drug{margin-top:10pt;font-size:16pt;font-weight:900;line-height:1.12;color:#0f2846}
          .label-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8pt;margin-top:11pt}
          .label-card{border:1pt solid #dbe5f1;border-radius:12pt;background:linear-gradient(180deg,#f9fbfe,#f1f6fc);padding:8pt 9pt;min-height:46pt;display:flex;flex-direction:column;justify-content:space-between}
          .label-card.wide{grid-column:1 / -1}
          .label-card span{font-size:7.4pt;font-weight:800;letter-spacing:.45pt;text-transform:uppercase;color:#6d8096}
          .label-card strong{font-size:14pt;font-weight:900;color:#16324e;line-height:1.2}
          .label-card.qty strong{font-size:11.4pt}
          @media print{body{padding:0;background:#fff}.label-sheet{box-shadow:none;margin:0}}
        </style>
      </head>
      <body>${body}</body>
    </html>
...
if (q("openDrugLabelModalBtn")) q("openDrugLabelModalBtn").onclick = openDrugLabelModal;
if (q("printDrugLabelBtn")) q("printDrugLabelBtn").onclick = printDrugSummaryLabel;
