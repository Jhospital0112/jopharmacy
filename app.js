...
const API_URL = (window.CDMS_CONFIG && window.CDMS_CONFIG.APPS_SCRIPT_WEB_APP_URL) || "";

if (!API_URL) {
  alert("Google Sheets Web App URL is not configured. Open config.js and set CDMS_CONFIG.APPS_SCRIPT_WEB_APP_URL first.");
}
...
async function apiRequest(action, payload = {}) {
  ...
  try {
    ...
    throw new Error("Invalid API response from Google Apps Script.");
  }
...
function applyClientFilters(rows, filters = []) {
  return (rows || []).filter(row => {
    return (filters || []).every(f => {
      if (!f) return true;
      const current = row?.[f.field];
      if (f.op === "==" || f.op === "eq") return String(current ?? "") === String(f.value ?? "");
      return true;
    });
  });
}
...
async function fetchRows(ref) {
  ...
  if (ref.kind === "query") return applyClientFilters(rows, ref.filters);
  return rows;
}
...
async function setDoc(docRef, payload) {
  ...
  await apiRequest("setDoc", { table: docRef.table, id: row.id, data: row });
}
...
function themeInit() {
  const saved = localStorage.getItem("cdms_theme") || "light";
  applyTheme(saved);
}
...
function applyTheme(theme) {
  ...
  if (nextTheme === "blue") document.body.classList.add("theme-blue");
  if (nextTheme === "green") document.body.classList.add("theme-green");
  localStorage.setItem("cdms_theme", nextTheme);
  const label = q("currentThemeLabel");
  if (label) label.textContent = nextTheme.charAt(0).toUpperCase() + nextTheme.slice(1);
  document.querySelectorAll("[data-theme-choice]").forEach(btn => {
    btn.classList.toggle("active-theme-choice", btn.dataset.themeChoice === nextTheme);
  });
}
...
function openDuplicateWarningModal(payload, duplicateResult) {
  ...
  const rows = [...(duplicateResult.exactDrugDuplicates || []), ...(duplicateResult.smartDuplicates || [])]
    .sort((a, b) => String(b.dateTime || "").localeCompare(String(a.dateTime || "")));

  q("duplicateWarningTbody").innerHTML = rows.length
...
function openPatientHistoryModal() {
  ...
  const tbody = q("patientHistoryTbody");
  if (!tbody) return;
...
async function savePharmacist() {
  ...
  const duplicate = APP.cache.pharmacists.find(
  ...
  try {
    const livePharmacists = await fetchRows(collection(db, "pharmacists"));
    const duplicateLive = livePharmacists.find(
      p => p.active !== false && String(p.jobNumber || "").trim() === jobNumber && p.id !== APP.pharmacistEditId
    );
    if (duplicateLive) {
      showActionModal("Validation", "This job number already exists. Pharmacist was not saved.", false);
...
q("viewPatientHistoryBtn") && (q("viewPatientHistoryBtn").onclick = openPatientHistoryModal);
q("closePatientHistoryBtn") && (q("closePatientHistoryBtn").onclick = () => closeModal("patientHistoryModal"));
