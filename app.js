  ...
  q("recentPrescriptionsTbody").innerHTML = rows.map(row => {
    ...
    return `
      <tr>
        ...
        <td>${row.status === "Returned" ? "-" : `<button class="soft-btn mini-btn latest-return-btn" data-id="${row.id}">Return</button>`}</td>
        <td>${row.status === "Returned" ? "-" : `<button class="primary-btn mini-btn latest-edit-btn ${allowed ? "" : "hidden"}" data-id="${row.id}">Edit</button>`}</td>
      </tr>`;
...
function suspendRecentPrescriptionsModalForAction() {
  const modal = q("recentPrescriptionsModal");
  if (!modal || modal.classList.contains("hidden")) return;
  modal.dataset.suspended = "true";
...
function restoreRecentPrescriptionsModalAfterAction() {
  ...
  delete modal.dataset.suspended;
}
...
document.addEventListener("click", event => {
  ...
  if (latestReturnBtn) { suspendRecentPrescriptionsModalForAction(); return openConfirmActionModal("return", latestReturnBtn.dataset.id); }
  ...
  if (latestEditBtn) { suspendRecentPrescriptionsModalForAction(); return openEditPrescription(latestEditBtn.dataset.id); }
...
q("openShipmentModalBtn").onclick = () => { refreshScopedSelectors(); resetShipmentBatchRows(); openModal("shipmentModal"); };
q("openShipmentModalBtn2").onclick = () => { refreshScopedSelectors(); resetShipmentBatchRows(); openModal("shipmentModal"); };
q("openTransferModalBtn").onclick = () => { refreshScopedSelectors(); resetTransferBatchRows(); openModal("transferModal"); };
q("openTransferModalBtn2").onclick = () => { refreshScopedSelectors(); resetTransferBatchRows(); openModal("transferModal"); };
