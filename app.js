...
function renderNarcoticSmartChart() {
  if (!q("narcoticSmartChart")) return;
  ...
  for (let i = 6; i >= 0; i -= 1) {
    ...
    const label = formatJordanDateTime(d).slice(5,10);
  ...
  buildBarChart("narcoticSmartChart", items, false);
}
...
function renderNarcoticDepartmentModal() {
  ...
  const dept = narcoticDeptById(departmentId);
  if (q("narcoticDepartmentModalName")) q("narcoticDepartmentModalName").value = dept?.departmentName || "";

  const assignedRows = narcoticDeptAssignedStockRows(departmentId)
    .sort((a,b) => String(a.drugName || "").localeCompare(String(b.drugName || "")));
  if (q("narcoticDeptAssignedTbody")) {
    q("narcoticDeptAssignedTbody").innerHTML = assignedRows.map(r => `<tr>
      <td>${esc(r.drugName || narcoticDrugById(r.drugId)?.tradeName || "")}</td>
      <td>${Number(r.fixedStockUnits || 0)}</td>
      <td>${Number(r.availableStockUnits || 0)}</td>
      <td>${APP.currentRole === "ADMIN" ? `<button class="mini-danger-btn mini-btn remove-drug-from-dept-btn" data-department-id="${departmentId}" data-drug-id="${r.drugId}">Remove</button>` : `<span class="muted">-</span>`}</td>
    </tr>`).join("") || `<tr><td colspan="4" class="empty-state">No drugs assigned to this department.</td></tr>`;
  }
  ...
  if (!drugId) {
    ...
    if (q("narcoticDeptStockSummary")) q("narcoticDeptStockSummary").innerHTML = `
      <div class="narcotic-stock-chip"><span>Department</span><strong>${esc(dept?.departmentName || "")}</strong></div>
      <div class="narcotic-stock-chip"><span>Assigned Drugs</span><strong>${assignedRows.length}</strong></div>`;
    return;
  }
...
async function addDrugToDepartment() {
  if (APP.currentRole !== "ADMIN") return;
  const departmentId = APP.narcoticOpenDepartmentId;
  const drugId = q("narcoticDeptManageDrug")?.value || "";
  if (!departmentId || !drugId) return;
  const dept = narcoticDeptById(departmentId);
  const drug = narcoticDrugById(drugId);
  const stockRow = narcoticDeptStockRow(departmentId, drugId);
  showActionModal("Department Drug", "Please wait while the drug is being assigned...");
  if (stockRow?.id) {
    await updateDoc(doc(db, "narcotic_department_stock", stockRow.id), {
      isAssigned: true,
      departmentName: dept?.departmentName || "",
...
async function removeDrugFromDepartment(departmentId, drugId) {
  if (APP.currentRole !== "ADMIN") return;
  const stockRow = narcoticDeptStockRow(departmentId, drugId);
  if (!stockRow?.id) return;
  showActionModal("Department Drug", "Please wait while the drug is being removed...");
  await updateDoc(doc(db, "narcotic_department_stock", stockRow.id), {
    isAssigned: false,
    fixedStockUnits: 0,
    availableStockUnits: 0,
...
function printNarcoticDispensingReport() {
  ...
  const body = `
    ...
    <div class="section signature-line"><strong>Narcotic Drugs Responsible Signature:</strong> ________________________</div>`;
...
document.addEventListener("click", event => {
  ...
  const viewDeptStockBtn = event.target.closest("[data-view-dept-stock]");
  if (viewDeptStockBtn) {
    return openNarcoticDepartmentModal(viewDeptStockBtn.dataset.viewDeptStock);
  }

  const removeDrugFromDeptBtn = event.target.closest(".remove-drug-from-dept-btn");
  if (removeDrugFromDeptBtn) {
    return removeDrugFromDepartment(removeDrugFromDeptBtn.dataset.departmentId, removeDrugFromDeptBtn.dataset.drugId);
  }
...
q("narcoticOrderDepartment").onchange = renderNarcoticStaticOptions;
...
q("addDrugToDepartmentBtn").onclick = addDrugToDepartment;
...
if (q("narcoticChartDepartmentFilter")) q("narcoticChartDepartmentFilter").onchange = renderNarcoticSmartChart;
if (q("narcoticChartDrugFilter")) q("narcoticChartDrugFilter").onchange = renderNarcoticSmartChart;
