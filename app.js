...
function buildPrescriptionActionsDropdown(row, { prefix = "rx" } = {}) {
  if (!row || row.status === "Returned") return `<span class="muted">-</span>`;
...
function toggleRxActionsMenu(menuId) {
  const menu = q(menuId);
  if (!menu) return;
              ...
              <table>
                ...
                <tbody>
                  ${group.rows.map(row => `
                    ...
                    <tr>
                      <td colspan="7"><strong>Prescription Type:</strong> ${esc(row.prescriptionType || "-")} &nbsp; | &nbsp; <strong>Audit Details:</strong> ${row.status === 'Returned' ? '-' : esc((row.auditBy || '') + (row.auditDateTime ? ` • ${formatJordanDateTime(row.auditDateTime)}` : ''))}</td>
                    </tr>
                  `).join("")}
                </tbody>
              </table>
...
function renderNarcoticStaticOptions() {
  ...
  const filteredDrugs = APP.cache.narcoticDrugs.filter(d => !term || `${d.tradeName || ""} ${d.scientificName || ""} ${d.strength || ""}`.toLowerCase().includes(term));
  const makeDrugOptions = (rows) => rows.map(d => `<option value="${esc(d.id)}">${esc(d.tradeName)} ${esc(d.strength || "")}</option>`).join("");
  if (q("narcoticDrug")) {
    q("narcoticDrug").innerHTML = `<option value="">Select Drug</option>${makeDrugOptions(filteredDrugs)}`;
    if ([...q("narcoticDrug").options].some(opt => opt.value === selectedDrug)) q("narcoticDrug").value = selectedDrug;
  }
  const fullDrugOptions = makeDrugOptions(APP.cache.narcoticDrugs);
  ["narcoticOrderDrug","narcoticReportDrug","narcoticDeptModalDrug"].forEach(id => {
    if (q(id)) q(id).innerHTML = `<option value="">Select Drug</option>${fullDrugOptions}`;
  });
  if (q("narcoticOrderDrug") && [...q("narcoticOrderDrug").options].some(opt => opt.value === selectedOrderDrug)) q("narcoticOrderDrug").value = selectedOrderDrug;
  if (q("narcoticReportDrug") && [...q("narcoticReportDrug").options].some(opt => opt.value === selectedReportDrug)) q("narcoticReportDrug").value = selectedReportDrug;
  if (q("narcoticDeptModalDrug") && [...q("narcoticDeptModalDrug").options].some(opt => opt.value === selectedModalDrug)) q("narcoticDeptModalDrug").value = selectedModalDrug;

  const deptOptions = APP.cache.narcoticDepartments.map(d => `<option value="${esc(d.id)}">${esc(d.departmentName || d.name || "")}</option>`).join("");
  ["narcoticDepartment","narcoticOrderDepartment","narcoticReportDepartment"].forEach(id => {
    const prev = q(id)?.value || "";
    if (q(id)) {
      q(id).innerHTML = `<option value="">Select Department</option>${deptOptions}`;
...
function updateNarcoticAvailableStock() {
  ...
  q("narcoticAvailableStock").value = row ? `${Number(row.availableStockUnits || 0)} ${unitLabel(narcoticDrugById(row.drugId) || {})}` : "-";
}
...
function narcoticActionMenu(row) {
  return APP.currentRole === "ADMIN"
    ? buildPrescriptionActionsDropdown({ id: row.id, status: row.status || "Registered" }, { prefix: "narcotic" })
    : `<button class="primary-btn mini-btn latest-edit-btn" data-id="${row.id}">Edit</button>`;
}
...
function renderNarcoticManageDrugsTable() {
  ...
  q("narcoticManageDrugsTbody").innerHTML = APP.cache.narcoticDrugs.map(drug => `
    <tr>
      ...
      <td>${esc(drug.dosageForm || "")}</td>
      <td>${Number(drug.unitsPerBox || 0)}</td>
      <td><button class="mini-danger-btn mini-btn delete-narcotic-drug-btn" data-id="${drug.id}">Delete</button></td>
    </tr>
...
async function registerNarcoticPrescription() {
  ...
  if (!stockRow || Number(stockRow.availableStockUnits || 0) < Number(payload.dispensedUnits || 0)) {
...
async function deleteNarcoticPrescription(id) {
  ...
  if (!row) return;
  const ok = window.confirm("Delete this narcotic prescription?");
  if (!ok) return;
  const stockRow = narcoticDeptStockRow(row.departmentId, row.drugId);
  const batch = writeBatch(db);
...
function renderNarcoticDepartmentModal() {
  ...
  if (!drugId) {
    ...
    if (q("narcoticDeptModalTbody")) q("narcoticDeptModalTbody").innerHTML = "";
    return;
  }
  ...
  q("narcoticDeptStockSummary").innerHTML = `
    ...
    <div class="narcotic-stock-chip"><span>Prescriptions</span><strong>${rows.length}</strong></div>`;
...
q("openNarcoticRecentModalBtn").onclick = openNarcoticRecentModal;
...
/* ===== Narcotic Drugs v6 overrides ===== */
function narcoticDepartmentActiveStockRows(departmentId) {
  return APP.cache.narcoticDepartmentStock.filter(r => String(r.departmentId) === String(departmentId) && r.active !== false);
}
function narcoticDeptStockRow(departmentId, drugId) {
  return APP.cache.narcoticDepartmentStock.find(r => String(r.departmentId) === String(departmentId) && String(r.drugId) === String(drugId) && r.active !== false);
}
function narcoticDepartmentDrugOptions(departmentId) {
  const rows = narcoticDepartmentActiveStockRows(departmentId);
  const ids = new Set(rows.map(r => String(r.drugId)));
  return APP.cache.narcoticDrugs.filter(d => ids.has(String(d.id)));
}
function renderNarcoticStaticOptions() {
  if (!q("narcoticDrug")) return;
  const selectedDrug = q("narcoticDrug")?.value || "";
  ...
  const selectedMgmtDrug = q("departmentStockManagerDrug")?.value || "";
  const deptId = q("narcoticDepartment")?.value || APP.narcoticOpenDepartmentId || APP.cache.narcoticDepartments[0]?.id || "";
  const orderDeptId = q("narcoticOrderDepartment")?.value || APP.cache.narcoticDepartments[0]?.id || "";
  const modalDeptId = APP.narcoticOpenDepartmentId || deptId;
  const term = (q("narcoticDrugSearch")?.value || "").toLowerCase().trim();

  const deptDrugs = narcoticDepartmentDrugOptions(deptId);
  const filteredDrugs = deptDrugs.filter(d => !term || `${d.tradeName || ""} ${d.scientificName || ""} ${d.strength || ""}`.toLowerCase().includes(term));
  q("narcoticDrug").innerHTML = `<option value="">Select Drug</option>${filteredDrugs.map(d => `<option value="${esc(d.id)}">${esc(d.tradeName || "")} ${esc(d.strength || "")}</option>`).join("")}`;
  if ([...q("narcoticDrug").options].some(opt => opt.value === selectedDrug)) q("narcoticDrug").value = selectedDrug;

  const orderDrugs = narcoticDepartmentDrugOptions(orderDeptId);
  if (q("narcoticOrderDrug")) {
    q("narcoticOrderDrug").innerHTML = `<option value="">Select Drug</option>${orderDrugs.map(d => `<option value="${esc(d.id)}">${esc(d.tradeName || "")} ${esc(d.strength || "")}</option>`).join("")}`;
    if ([...q("narcoticOrderDrug").options].some(opt => opt.value === selectedOrderDrug)) q("narcoticOrderDrug").value = selectedOrderDrug;
  }

  const modalDrugs = narcoticDepartmentDrugOptions(modalDeptId);
  if (q("narcoticDeptModalDrug")) {
    q("narcoticDeptModalDrug").innerHTML = `<option value="">Select Drug</option>${modalDrugs.map(d => `<option value="${esc(d.id)}">${esc(d.tradeName || "")} ${esc(d.strength || "")}</option>`).join("")}`;
    if ([...q("narcoticDeptModalDrug").options].some(opt => opt.value === selectedModalDrug)) q("narcoticDeptModalDrug").value = selectedModalDrug;
  }

  const fullDrugOptions = APP.cache.narcoticDrugs.map(d => `<option value="${esc(d.id)}">${esc(d.tradeName || "")} ${esc(d.strength || "")}</option>`).join("");
  ["narcoticReportDrug","narcoticChartDrug","departmentStockManagerDrug"].forEach(id => { if (q(id)) q(id).innerHTML = `<option value="">All / Select</option>${fullDrugOptions}`; });
  if (q("narcoticReportDrug") && [...q("narcoticReportDrug").options].some(opt => opt.value === selectedReportDrug)) q("narcoticReportDrug").value = selectedReportDrug;
  if (q("narcoticChartDrug") && [...q("narcoticChartDrug").options].some(opt => opt.value === selectedChartDrug)) q("narcoticChartDrug").value = selectedChartDrug;
  if (q("departmentStockManagerDrug") && [...q("departmentStockManagerDrug").options].some(opt => opt.value === selectedMgmtDrug)) q("departmentStockManagerDrug").value = selectedMgmtDrug;

  const deptOptions = APP.cache.narcoticDepartments.map(d => `<option value="${esc(d.id)}">${esc(d.departmentName || d.name || "")}</option>`).join("");
  ["narcoticDepartment","narcoticOrderDepartment","narcoticReportDepartment","narcoticChartDepartment"].forEach(id => {
    if (q(id)) q(id).innerHTML = `<option value="">Select Department</option>${deptOptions}`;
  });
  const selectedDept = deptId;
  if (q("narcoticDepartment") && [...q("narcoticDepartment").options].some(opt => opt.value === selectedDept)) q("narcoticDepartment").value = selectedDept;
  if (q("narcoticOrderDepartment") && [...q("narcoticOrderDepartment").options].some(opt => opt.value === orderDeptId)) q("narcoticOrderDepartment").value = orderDeptId;
  if (q("narcoticChartDepartment") && [...q("narcoticChartDepartment").options].some(opt => opt.value === (q("narcoticChartDepartment").value || selectedDept))) {
    if (!q("narcoticChartDepartment").value) q("narcoticChartDepartment").value = selectedDept;
  }

  const inptPharmacists = getScopePharmacists("In-Patient Pharmacy").map(p => `<option value="${esc(p.name)}">${esc(p.name)}</option>`).join("");
  const narcManagers = narcoticManagerPharmacists().map(p => `<option value="${esc(p.name)}">${esc(p.name)}</option>`).join("");
  if (q("narcoticPharmacist")) q("narcoticPharmacist").innerHTML = `<option value="">Select Pharmacist</option>${inptPharmacists}`;
  if (q("narcoticEditPharmacist")) q("narcoticEditPharmacist").innerHTML = `<option value="">Select Pharmacist</option>${inptPharmacists}`;
  if (q("narcoticOrderPharmacist")) q("narcoticOrderPharmacist").innerHTML = `<option value="">Select Pharmacist</option>${narcManagers}`;
}
function narcoticActionMenu(row) {
  if (!row) return '<span class="muted">-</span>';
  const menuId = `narcotic_actions_${row.id}`;
  const canDelete = APP.currentRole === "ADMIN" || APP.currentRole === "IN_PATIENT_USER";
  return `
    <div class="rx-actions-wrap">
      <button class="soft-btn mini-btn rx-actions-btn" data-rx-actions-toggle="${menuId}">Actions ▾</button>
      <div class="rx-actions-menu hidden" id="${menuId}">
        <button class="primary-btn mini-btn narcotic-edit-btn" data-id="${row.id}">Edit</button>
        ${canDelete ? `<button class="mini-danger-btn mini-btn narcotic-delete-btn" data-id="${row.id}">Delete</button>` : ""}
      </div>
    </div>`;
}
function renderNarcoticRecent() {
  if (!q("narcoticRecentTbody")) return;
  const rows = APP.cache.narcoticPrescriptions.slice(0, 12);
  q("narcoticRecentTbody").innerHTML = rows.map(row => {
    const drug = narcoticDrugById(row.drugId);
    const dept = narcoticDeptById(row.departmentId);
    return `<tr>
...
function renderNarcoticDashboardChart() {
  if (!q("narcoticDispenseChart")) return;
  const deptId = q("narcoticChartDepartment")?.value || "";
  const drugId = q("narcoticChartDrug")?.value || "";
  const today = parseJordanDateTime(`${jordanDateKey()} 00:00:00`) || new Date();
  const items = [];
  for (let i = 6; i >= 0; i--) {
    const d = new Date(today);
    d.setDate(d.getDate() - i);
    const key = formatJordanDateTime(d).slice(0,10);
    const val = APP.cache.narcoticPrescriptions.filter(r => {
      const day = formatJordanDateTime(r.dateTime).slice(0,10);
      if (day !== key) return false;
      if (deptId && r.departmentId !== deptId) return false;
      if (drugId && r.drugId !== drugId) return false;
      return true;
    }).reduce((s,r)=>s + Number(r.dispensedUnits || 0), 0);
    items.push({ label: key.slice(5), value: val });
  }
  buildBarChart("narcoticDispenseChart", items, false);
}
function renderNarcoticManagementCards() {
  if (!q("narcoticDrugCards")) return;
  q("narcoticDrugCards").innerHTML = APP.cache.narcoticDrugs.map(drug => `
      <div class="drug-card narcotic-drug-card" data-narcotic-drug-card="${esc(drug.id)}">
        <div class="drug-title">${esc(drug.tradeName || "")} ${esc(drug.strength || "")}</div>
...
function renderNarcoticPage() {
  if (!APP.currentUser || !q("page-narcotic")) return;
  if (APP.currentRole !== "ADMIN" && APP.narcoticTab !== "dispensing") APP.narcoticTab = "dispensing";
  if (q("narcoticLiveTime")) q("narcoticLiveTime").textContent = formatJordanDateTime(jordanNowIso());
  renderNarcoticStaticOptions();
  renderNarcoticRecent();
  renderNarcoticDepartmentCards();
  renderNarcoticOrdersTable();
  renderNarcoticDepartmentsTable();
  renderNarcoticManageDrugsTable();
  renderNarcoticManagementCards();
  updateNarcoticAvailableStock();
  renderNarcoticDashboardChart();
  updateNarcoticReportPreview();
  setNarcoticTab(APP.narcoticTab || "dispensing");
}
async function registerNarcoticPrescription() {
...
function filterNarcoticMovementsForReport() {
  const {departmentId, drugId, from, to, search} = narcoticReportFilters();
  return APP.cache.narcoticOrderMovements.filter(r => String(r.type || "") === "Department Order Movement").filter(r => {
    const day = String(formatJordanDateTime(r.dateTime)).slice(0,10);
    if (departmentId && r.departmentId !== departmentId) return false;
    if (drugId && r.drugId !== drugId) return false;
    if (from && day < from) return false;
...
function narcoticSignatureBlock() {
  return `
    <div style="margin-top:34px;display:flex;justify-content:flex-end;">
      <div style="min-width:260px;text-align:center;">
        <div style="font-weight:800;margin-bottom:38px;">Narcotic Manager Signature</div>
        <div style="border-top:1px solid #000;padding-top:8px;">Name / Signature</div>
      </div>
    </div>`;
}
function printNarcoticDispensingReport() {
...
function printNarcoticStockReport() {
  ...
  const drugId = q("narcoticReportDrug")?.value || "";
  const dept = narcoticDeptById(departmentId);
  const rows = narcoticDepartmentActiveStockRows(departmentId).filter(r => !drugId || r.drugId === drugId);
  const drug = narcoticDrugById(drugId);
  const body = `
    <div class="section-title">Department Stock Report</div>
    <div class="sub"><strong>Department:</strong> ${esc(dept?.departmentName || "")} &nbsp; | &nbsp; <strong>Drug:</strong> ${esc(drug ? `${drug.tradeName || ""} ${drug.strength || ""}`.trim() : "All drugs")}</div>
...
function openDepartmentStockManagerModal() {
  if (!APP.narcoticOpenDepartmentId) return;
  renderDepartmentStockManagerModal();
  openModal("departmentStockManagerModal");
}
function renderDepartmentStockManagerModal() {
  const departmentId = APP.narcoticOpenDepartmentId;
  const rows = narcoticDepartmentActiveStockRows(departmentId).sort((a,b)=>String(a.drugName||"").localeCompare(String(b.drugName||"")));
  if (q("departmentStockManagerTbody")) q("departmentStockManagerTbody").innerHTML = rows.map(r => `<tr><td>${esc(r.drugName||"")}</td><td>${Number(r.fixedStockUnits||0)}</td><td>${Number(r.availableStockUnits||0)}</td></tr>`).join("") || `<tr><td colspan="3" class="empty-state">No drugs configured.</td></tr>`;
  if (q("departmentStockManagerManageTbody")) q("departmentStockManagerManageTbody").innerHTML = rows.map(r => `<tr><td>${esc(r.drugName||"")}</td><td><button class="mini-danger-btn mini-btn remove-department-drug-btn" data-id="${esc(r.id)}">Remove</button></td></tr>`).join("") || `<tr><td colspan="2" class="empty-state">No drugs configured.</td></tr>`;
  const usedIds = new Set(rows.map(r => String(r.drugId)));
  if (q("departmentStockManagerDrug")) q("departmentStockManagerDrug").innerHTML = `<option value="">Select Drug</option>${APP.cache.narcoticDrugs.filter(d => !usedIds.has(String(d.id))).map(d => `<option value="${esc(d.id)}">${esc(d.tradeName || "")} ${esc(d.strength || "")}</option>`).join("")}`;
}
async function addDepartmentDrugAssignment() {
  if (APP.currentRole !== "ADMIN" || !APP.narcoticOpenDepartmentId) return;
  const drugId = q("departmentStockManagerDrug")?.value || "";
  if (!drugId) return;
  const dept = narcoticDeptById(APP.narcoticOpenDepartmentId);
  const drug = narcoticDrugById(drugId);
  const existing = APP.cache.narcoticDepartmentStock.find(r => String(r.departmentId) === String(APP.narcoticOpenDepartmentId) && String(r.drugId) === String(drugId));
  const payload = {
    departmentId: APP.narcoticOpenDepartmentId,
    departmentName: dept?.departmentName || "",
    drugId,
    drugName: `${drug?.tradeName || ""} ${drug?.strength || ""}`.trim(),
    fixedStockUnits: Math.max(0, Number(q("departmentStockManagerFixed")?.value || 0)),
    availableStockUnits: Math.max(0, Number(q("departmentStockManagerAvailable")?.value || 0)),
    active: true,
    updatedAt: serverTimestamp()
  };
  if (existing) await updateDoc(doc(db, "narcotic_department_stock", existing.id), payload);
  else await setDoc(doc(db, "narcotic_department_stock", `${APP.narcoticOpenDepartmentId}__${drugId}`), { id: `${APP.narcoticOpenDepartmentId}__${drugId}`, ...payload });
  q("departmentStockManagerFixed").value = '0';
  q("departmentStockManagerAvailable").value = '0';
}
async function removeDepartmentDrugAssignment(rowId) {
  if (APP.currentRole !== "ADMIN") return;
  await updateDoc(doc(db, "narcotic_department_stock", rowId), { active: false, updatedAt: serverTimestamp() });
}
function openNarcoticDepartmentModal(departmentId) {
  APP.narcoticOpenDepartmentId = departmentId;
  const dept = narcoticDeptById(departmentId);
...
/* re-bind narcotic handlers after overrides */
if (q("narcoticDrug")) q("narcoticDrug").onchange = () => { updateNarcoticAvailableStock(); renderNarcoticDashboardChart(); };
if (q("narcoticDepartment")) q("narcoticDepartment").onchange = () => { renderNarcoticStaticOptions(); updateNarcoticAvailableStock(); };
if (q("narcoticDrugSearch")) q("narcoticDrugSearch").oninput = renderNarcoticStaticOptions;
if (q("narcoticClearBtn")) q("narcoticClearBtn").onclick = resetNarcoticEntryForm;
if (q("openNarcoticRecentModalBtn")) q("openNarcoticRecentModalBtn").onclick = openNarcoticRecentModal;
if (q("registerNarcoticBtn")) q("registerNarcoticBtn").onclick = registerNarcoticPrescription;
if (q("submitNarcoticOrdersBtn")) q("submitNarcoticOrdersBtn").onclick = submitNarcoticOrders;
if (q("addNarcoticOrderRowBtn")) q("addNarcoticOrderRowBtn").onclick = addNarcoticOrderRow;
if (q("narcoticOrderDepartment")) q("narcoticOrderDepartment").onchange = renderNarcoticStaticOptions;
if (q("narcoticDeptModalDrug")) q("narcoticDeptModalDrug").onchange = renderNarcoticDepartmentModal;
if (q("narcoticDeptModalFrom")) q("narcoticDeptModalFrom").onchange = renderNarcoticDepartmentModal;
if (q("narcoticDeptModalTo")) q("narcoticDeptModalTo").onchange = renderNarcoticDepartmentModal;
if (q("narcoticChartDepartment")) q("narcoticChartDepartment").onchange = renderNarcoticDashboardChart;
if (q("narcoticChartDrug")) q("narcoticChartDrug").onchange = renderNarcoticDashboardChart;
if (q("openDeptStockManagerBtn")) q("openDeptStockManagerBtn").onclick = openDepartmentStockManagerModal;
if (q("addDepartmentDrugBtn")) q("addDepartmentDrugBtn").onclick = addDepartmentDrugAssignment;

document.addEventListener("click", (event) => {
  const removeDeptDrugBtn = event.target.closest(".remove-department-drug-btn");
  if (removeDeptDrugBtn) { removeDepartmentDrugAssignment(removeDeptDrugBtn.dataset.id); return; }
});
