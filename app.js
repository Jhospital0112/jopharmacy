...
function buildSelectedTransactionPrintTable(headers, values) {
  return `
    <table style="width:100%;border-collapse:collapse;margin-top:12px;font-size:13px">
      <thead>
        <tr>
          ${headers.map(header => `<th style="border:1px solid #cfd8e3;padding:10px 8px;background:#f4f7fb;text-align:left">${header}</th>`).join('')}
        </tr>
      </thead>
      <tbody>
        <tr>
          ${values.map(value => `<td style="border:1px solid #cfd8e3;padding:10px 8px;vertical-align:top">${value}</td>`).join('')}
        </tr>
      </tbody>
    </table>`;
}

function buildSelectedTransactionMetaLine(label, actor, dateTime, extra = "") {
  return `
    <div style="margin-top:10px;padding:10px 12px;border:1px solid #dbe3ee;border-radius:10px;background:#fafcff;font-size:13px">
      <strong>${label}:</strong> ${esc(actor || '-')} &nbsp; | &nbsp;
      <strong>Date & Time:</strong> ${esc(formatJordanDateTime(dateTime || ''))}
      ${extra ? ` &nbsp; | &nbsp; ${extra}` : ""}
    </div>`;
}

function buildSelectedTransactionSignatures(leftLabel, leftName, rightLabel, rightName) {
  return `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:28px;margin-top:26px">
      <div style="border-top:2px solid #222;padding-top:10px;text-align:center">
        <div style="font-weight:800">${leftLabel}</div>
        <div style="margin-top:6px">${esc(leftName || '-')}</div>
      </div>
      <div style="border-top:2px solid #222;padding-top:10px;text-align:center">
        <div style="font-weight:800">${rightLabel}</div>
        <div style="margin-top:6px">${esc(rightName || '-')}</div>
      </div>
    </div>`;
}

function buildSelectedTransactionReportBody(row, scope = "normal") {
  if (!row) return `<div class="empty-state">Transaction not found.</div>`;
  const normalizedType = String(row.type || "").trim();
  const isNarcotic = scope === "narcotic";
  const drugName = esc(row.tradeName || row.drugName || "-");
  const pharmacyName = esc(row.pharmacy || row.departmentName || "-");
  const qtyBoxes = Number(row.qtyBoxes || 0);
  const qtyUnits = Number(row.qtyUnits || row.quantitySent || row.dispensedUnits || 0);
  ...
  const currentPatient = row.patientName || row.patient || "-";
  const currentFile = row.fileNumber || row.fileNo || "-";
  const currentDoctor = row.doctorName || row.doctor || "-";
  const currentPharmacist = row.pharmacistName || row.performedBy || row.registeredBy || row.receivedBy || row.receiverPharmacist || "-";

  if (["Register", "Dispense"].includes(normalizedType)) {
    return `
      <div class="section-title">${isNarcotic ? "Narcotic Dispensed Prescription" : "Dispensed Prescription"}</div>
      ${buildSelectedTransactionPrintTable(
        ["Patient Name", "File Number", isNarcotic ? "Department / Pharmacy" : "Pharmacy", "Doctor Name", "Dispensed Quantity"],
        [
          esc(currentPatient),
          esc(currentFile),
          pharmacyName,
          esc(currentDoctor),
          `${qtyBoxes} box(es) + ${qtyUnits} unit(s)`
        ]
      )}
      ${buildSelectedTransactionMetaLine("Dispensed By", row.registeredBy || row.performedBy || row.pharmacistName, row.registeredDateTime || row.dateTime, `<strong>Drug:</strong> ${drugName}`)}
    `;
  }

  if (normalizedType === "Edit Prescription") {
    const oldValues = row.oldValues || {};
    const newValues = row.newValues || {};
    return `
      <div class="section-title">${isNarcotic ? "Edited Narcotic Prescription" : "Edited Prescription"}</div>
      <div style="margin-top:14px;font-weight:800;color:#173a66">Before Edit</div>
      ${buildSelectedTransactionPrintTable(
        ["Patient Name", "File Number", "Doctor Name", "Quantity"],
        [
          esc(oldValues.patientName || row.patientName || "-"),
          esc(oldValues.fileNumber || row.fileNumber || "-"),
          esc(oldValues.doctorName || row.doctorName || "-"),
          `${Number(oldValues.qtyBoxes || 0)} box(es) + ${Number(oldValues.qtyUnits || 0)} unit(s)`
        ]
      )}
      <div style="margin-top:18px;font-weight:800;color:#173a66">After Edit</div>
      ${buildSelectedTransactionPrintTable(
        ["Patient Name", "File Number", "Doctor Name", "Quantity"],
        [
          esc(newValues.patientName || row.patientName || "-"),
          esc(newValues.fileNumber || row.fileNumber || "-"),
          esc(newValues.doctorName || row.doctorName || "-"),
          `${Number(newValues.qtyBoxes || 0)} box(es) + ${Number(newValues.qtyUnits || 0)} unit(s)`
        ]
      )}
      ${buildSelectedTransactionMetaLine("Edited By", row.editedBy || row.performedBy || row.pharmacistName, row.editedDateTime || row.dateTime, `<strong>Drug:</strong> ${drugName} &nbsp; | &nbsp; <strong>Pharmacy:</strong> ${pharmacyName}`)}
    `;
  }

  if (normalizedType === "Delete Prescription") {
    return `
      <div class="section-title">${isNarcotic ? "Deleted Narcotic Prescription" : "Deleted Prescription"}</div>
      ${buildSelectedTransactionPrintTable(
        ["Patient Name", "File Number", isNarcotic ? "Department / Pharmacy" : "Pharmacy", "Doctor Name", "Dispensed Quantity"],
        [
          esc(currentPatient),
      ...
      ${buildSelectedTransactionMetaLine("Deleted By", row.deletedBy || row.performedBy || row.pharmacistName, row.deletedDateTime || row.dateTime, `<strong>Drug:</strong> ${drugName}`)}
    `;
  }

  if (normalizedType === "Return") {
    return `
      <div class="section-title">${isNarcotic ? "Returned Narcotic Prescription" : "Returned Prescription"}</div>
      ${buildSelectedTransactionPrintTable(
        ["Patient Name", "File Number", isNarcotic ? "Department / Pharmacy" : "Pharmacy", "Doctor Name", "Returned Quantity"],
        [
          esc(currentPatient),
          esc(currentFile),
          pharmacyName,
      ...
      ${buildSelectedTransactionMetaLine("Returned By", row.returnBy || row.performedBy || row.pharmacistName || row.returnPharmacist, row.returnDateTime || row.dateTime, `<strong>Drug:</strong> ${drugName}`)}
    `;
  }

  if (normalizedType === "Receive Shipment") {
    return `
      <div class="section-title">${isNarcotic ? "Narcotic Shipment Receipt" : "Shipment Receipt"}</div>
      ${buildSelectedTransactionPrintTable(
        ["Date & Time", isNarcotic ? "Department / Pharmacy" : "Pharmacy", "Drug", "Boxes", "Units", "Invoice Number", "Invoice Date"],
        [
          esc(formatJordanDateTime(row.dateTime || "")),
          pharmacyName,
          drugName,
          String(qtyBoxes),
          String(qtyUnits),
      ...
      ${buildSelectedTransactionMetaLine("Received By", row.receivedBy || row.performedBy || row.receiverPharmacist, row.receivedDateTime || row.dateTime)}
    `;
  }

  if (normalizedType === "Transfer") {
    const transferredBy = row.performedBy || row.transferredBy || "-";
    const receivedBy = row.receiverPharmacist || row.receivedBy || "-";
    return `
      <div class="section-title">${isNarcotic ? "Narcotic Stock Transfer" : "Stock Transfer"}</div>
      ${buildSelectedTransactionPrintTable(
        ["Date & Time", "From", "To", "Drug", "Transferred Quantity", "Transferred By", "Received By"],
        [
          esc(formatJordanDateTime(row.dateTime || "")),
          esc(fromPharmacy || "-"),
          esc(toPharmacy || "-"),
      ...
      ${buildSelectedTransactionSignatures("Transferred By Signature", transferredBy, "Received By Signature", receivedBy)}
    `;
  }
  ...
  return `
    ...
    ${buildSelectedTransactionPrintTable(
      ["Type", "Drug", isNarcotic ? "Department / Pharmacy" : "Pharmacy", "Performed By", "Date & Time"],
      [
        esc(normalizedType || "-"),
        drugName,
        pharmacyName,
        esc(currentPharmacist),
        esc(formatJordanDateTime(row.dateTime || ""))
      ]
    )}
    <div style="margin-top:10px;padding:10px 12px;border:1px solid #dbe3ee;border-radius:10px;background:#fafcff;font-size:13px">
      <strong>Note:</strong> ${esc(row.note || row.notes || "-")}
    </div>
  `;
}
...
function printSelectedTransaction() {
  ...
  if (info.scope === "narcotic") {
    scope = "narcotic";
    row = (APP.cache.narcoticOrderMovements || []).find(item => String(item.id) === String(info.id));
  } else {
    row = (APP.cache.transactions || []).find(item => String(item.id) === String(info.id));
  }
  const title = scope === "narcotic" ? "Narcotic Selected Transaction Report" : "Selected Transaction Report";
  const locationLabel = row?.pharmacy || row?.departmentName || currentScopePharmacy();
  const body = buildSelectedTransactionReportBody(row, scope);
  const w = window.open('', '_blank');
  w.document.write(buildPrintShell(title, locationLabel, body));
  w.document.close();
}
