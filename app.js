  ...
  return `
    <div class="rx-actions-wrap">
      ...
      <div class="rx-actions-menu hidden" id="${menuId}">
        <button class="primary-btn mini-btn narcotic-edit-btn ${allowed ? "" : "hidden"}" data-id="${row.id}">Edit</button>
  ...
  <html>
    <head>
      ...
      <style>
        ...
        .signature-row{display:flex;justify-content:flex-end;margin-top:34px}
        .signature-box{min-width:320px;border-top:2px solid #20344a;padding-top:10px;text-align:center;font-weight:700;color:#20344a}
        .signature-box small{display:block;margin-top:6px;color:#5f7085;font-weight:600}
    ...
    <body>
      <div class="report">
        ...
        ${bodyHtml}
        <div class="signature-row"><div class="signature-box">Responsible Controlled Drugs Officer Signature<small>Name / Signature / Stamp</small></div></div>
      </div>
...
function renderNarcoticStaticOptions() {
  ...
  const selectedAssignDrug = q("narcoticDeptAssignDrug")?.value || "";
  const selectedChartDrug = q("narcoticChartDrugFilter")?.value || "";
  const selectedChartDepartment = q("narcoticChartDepartmentFilter")?.value || "";
  const term = (q("narcoticDrugSearch")?.value || "").toLowerCase().trim();
  const selectedDepartmentId = q("narcoticDepartment")?.value || "";
  const selectedOrderDepartmentId = q("narcoticOrderDepartment")?.value || "";
  const modalDepartmentId = APP.narcoticOpenDepartmentId || "";

  const quickDrugsPool = selectedDepartmentId ? deptAllowedNarcoticDrugs(selectedDepartmentId) : APP.cache.narcoticDrugs;
  const filteredQuickDrugs = quickDrugsPool.filter(d => !term || `${d.tradeName || ""} ${d.scientificName || ""} ${d.strength || ""}`.toLowerCase().includes(term));
  const makeDrugOptions = rows => rows.map(d => `<option value="${esc(d.id)}">${esc(d.tradeName)} ${esc(d.strength || "")}</option>`).join("");

  if (q("narcoticDrug")) {
    q("narcoticDrug").innerHTML = `<option value="">Select Drug</option>${makeDrugOptions(filteredQuickDrugs)}`;
    if ([...q("narcoticDrug").options].some(opt => opt.value === selectedDrug)) q("narcoticDrug").value = selectedDrug;
  }

  const orderDrugs = selectedOrderDepartmentId ? deptAllowedNarcoticDrugs(selectedOrderDepartmentId) : APP.cache.narcoticDrugs;
  if (q("narcoticOrderDrug")) {
    q("narcoticOrderDrug").innerHTML = `<option value="">Select Drug</option>${makeDrugOptions(orderDrugs)}`;
    if ([...q("narcoticOrderDrug").options].some(opt => opt.value === selectedOrderDrug)) q("narcoticOrderDrug").value = selectedOrderDrug;
  }

  const reportOptions = makeDrugOptions(APP.cache.narcoticDrugs);
  ["narcoticReportDrug","narcoticChartDrugFilter"].forEach(id => {
    if (q(id)) q(id).innerHTML = `<option value="">All Drugs</option>${reportOptions}`;
  });
  ...
  if (q("narcoticChartDrugFilter") && [...q("narcoticChartDrugFilter").options].some(opt => opt.value === selectedChartDrug)) q("narcoticChartDrugFilter").value = selectedChartDrug;

  const modalDrugs = modalDepartmentId ? deptAllowedNarcoticDrugs(modalDepartmentId) : APP.cache.narcoticDrugs;
  if (q("narcoticDeptModalDrug")) {
    q("narcoticDeptModalDrug").innerHTML = `<option value="">Select Drug</option>${makeDrugOptions(modalDrugs)}`;
    if ([...q("narcoticDeptModalDrug").options].some(opt => opt.value === selectedModalDrug)) q("narcoticDeptModalDrug").value = selectedModalDrug;
  }

  const unassignedRows = modalDepartmentId
    ? APP.cache.narcoticDepartmentStock.filter(r => String(r.departmentId) === String(modalDepartmentId) && !isDeptDrugAssigned(r))
    : [];
  const unassignedDrugs = unassignedRows.map(r => narcoticDrugById(r.drugId)).filter(Boolean);
  if (q("narcoticDeptAssignDrug")) {
    q("narcoticDeptAssignDrug").innerHTML = `<option value="">Select Drug</option>${makeDrugOptions(unassignedDrugs)}`;
    if ([...q("narcoticDeptAssignDrug").options].some(opt => opt.value === selectedAssignDrug)) q("narcoticDeptAssignDrug").value = selectedAssignDrug;
  }

  const deptOptions = APP.cache.narcoticDepartments.map(d => `<option value="${esc(d.id)}">${esc(d.departmentName || d.name || "")}</option>`).join("");
  ["narcoticDepartment","narcoticOrderDepartment","narcoticReportDepartment","narcoticChartDepartmentFilter"].forEach(id => {
    const prev = q(id)?.value || "";
    if (q(id)) {
      q(id).innerHTML = `<option value="">${id === "narcoticChartDepartmentFilter" ? "All Departments" : "Select Department"}</option>${deptOptions}`;
      if ([...q(id).options].some(opt => opt.value === prev)) q(id).value = prev;
    }
  });
  if (q("narcoticChartDepartmentFilter") && [...q("narcoticChartDepartmentFilter").options].some(opt => opt.value === selectedChartDepartment)) q("narcoticChartDepartmentFilter").value = selectedChartDepartment;
...
function isDeptDrugAssigned(stockRow) {
  if (!stockRow) return false;
  if (stockRow.isAssigned === false) return false;
  return true;
}
function assignedNarcoticStockRows(departmentId) {
  return APP.cache.narcoticDepartmentStock.filter(r => String(r.departmentId) === String(departmentId) && isDeptDrugAssigned(r));
}
function deptAllowedNarcoticDrugs(departmentId) {
  return assignedNarcoticStockRows(departmentId)
    .map(r => narcoticDrugById(r.drugId))
    .filter(Boolean)
    .sort((a,b)=>String(a.tradeName||'').localeCompare(String(b.tradeName||'')));
}
...
function narcoticActionMenu(row) {
  if (!["ADMIN","IN_PATIENT_USER"].includes(APP.currentRole)) {
    return `<button class="primary-btn mini-btn latest-edit-btn" data-id="${row.id}">Edit</button>`;
  }
  const allowed = canEditPrescription(row);
  const menuId = `rx_actions_narcotic_${row.id}`;
  return `
    <div class="rx-actions-wrap">
      <button class="soft-btn mini-btn rx-actions-btn" data-rx-actions-toggle="${menuId}">Actions ▾</button>
      <div class="rx-actions-menu hidden" id="${menuId}">
        ...
        <button class="mini-danger-btn mini-btn narcotic-delete-btn" data-id="${row.id}">Delete</button>
      </div>
    </div>`;
}
...
function renderNarcoticDepartmentCards() {
  ...
  q("narcoticDepartmentCards").innerHTML = APP.cache.narcoticDepartments
    ...
    .map(dept => {
      const assigned = assignedNarcoticStockRows(dept.id);
      const totalAvailable = assigned.reduce((sum,row)=>sum+Number(row.availableStockUnits||0),0);
      return `
      <div class="drug-card narcotic-department-card" draggable="${APP.currentRole === "ADMIN"}" data-narcotic-dept-card="${esc(dept.id)}">
        <div>
          ...
          <div class="drug-meta">Assigned Drugs: ${assigned.length}</div>
          <div class="drug-meta">Available Units: ${totalAvailable}</div>
        </div>
        <div class="card-actions"><button class="soft-btn" data-view-dept-stock="${esc(dept.id)}">View Department Stock</button></div>
      </div>`;
...
function renderNarcoticDispensingChart() {
  const drugId = q("narcoticChartDrugFilter")?.value || "";
  const departmentId = q("narcoticChartDepartmentFilter")?.value || "";
  const today = parseJordanDateTime(jordanNowIso()) || new Date();
  const items = [];
  for (let i = 6; i >= 0; i--) {
    ...
    const value = APP.cache.narcoticPrescriptions.filter(r => {
      const rowDate = String(formatJordanDateTime(r.dateTime)).slice(0,10);
      return rowDate === key && (!drugId || r.drugId === drugId) && (!departmentId || r.departmentId === departmentId);
    }).reduce((sum,r)=>sum + Number(r.dispensedUnits || 0),0);
    items.push({ label, value });
  }
  buildBarChart("narcoticDispense7DayChart", items, false);
}
function renderNarcoticDepartmentStockLists() {
  const departmentId = APP.narcoticOpenDepartmentId;
  if (!departmentId) return;
  const term = (q("narcoticDeptStockSearch")?.value || "").toLowerCase().trim();
  const assigned = assignedNarcoticStockRows(departmentId).filter(row => {
    const drug = narcoticDrugById(row.drugId);
    const label = `${drug?.tradeName || row.drugName || ""} ${drug?.strength || ""}`.toLowerCase();
    return !term || label.includes(term);
  }).sort((a,b)=> String(narcoticDrugById(a.drugId)?.tradeName || '').localeCompare(String(narcoticDrugById(b.drugId)?.tradeName || '')));
  if (q("narcoticDeptAssignedTbody")) q("narcoticDeptAssignedTbody").innerHTML = assigned.map(row => {
    const drug = narcoticDrugById(row.drugId);
    return `<tr><td>${esc(`${drug?.tradeName || row.drugName || ""} ${drug?.strength || ""}`.trim())}</td><td>${Number(row.fixedStockUnits || 0)}</td><td>${Number(row.availableStockUnits || 0)}</td><td><button class="mini-danger-btn mini-btn" data-remove-dept-drug="${esc(row.drugId)}">Remove</button></td></tr>`;
  }).join("") || `<tr><td colspan="4" class="empty-state">No drugs assigned to this department.</td></tr>`;
  const activeDrugId = q("narcoticDeptModalDrug")?.value || "";
  if (q("narcoticDeptStockListTbody")) q("narcoticDeptStockListTbody").innerHTML = assigned.map(row => {
    const drug = narcoticDrugById(row.drugId);
    return `<tr class="narcotic-stock-list-row ${activeDrugId === row.drugId ? 'active-drug' : ''}"><td>${esc(`${drug?.tradeName || row.drugName || ""} ${drug?.strength || ""}`.trim())}</td><td>${Number(row.fixedStockUnits || 0)}</td><td>${Number(row.availableStockUnits || 0)}</td></tr>`;
  }).join("") || `<tr><td colspan="3" class="empty-state">No stock rows found.</td></tr>`;
}
...
function renderNarcoticPage() {
  ...
  updateNarcoticAvailableStock();
  renderNarcoticDispensingChart();
  renderNarcoticDepartmentStockLists();
...
async function saveNarcoticDepartment() {
  ...
  const deptRef = await addDoc(collection(db, "narcotic_departments"), { departmentName: name, notes: q("narcoticDepartmentNotes").value.trim(), sortOrder: nextOrder, active: true, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
  const batch = writeBatch(db);
  APP.cache.narcoticDrugs.forEach(drug => {
    batch.set(doc(db, "narcotic_department_stock", `${deptRef.id}__${drug.id}`), {
...
function openNarcoticDepartmentModal(departmentId) {
  ...
  if (q("narcoticDeptStockSearch")) q("narcoticDeptStockSearch").value = "";
  renderNarcoticStaticOptions();
...
function renderNarcoticDepartmentModal() {
  ...
  q("narcoticDeptModalTbody").innerHTML = rows.map(r => `<tr>
    ...
    <td>${esc(r.prescriptionNumber || "")}</td>
    <td>${esc(r.dose || "")}</td>
    <td>${Number(r.dispensedUnits || 0)}</td>
    <td>${esc(r.doctorName || "")}</td>
    <td>${esc(r.pharmacist || "")}</td>
    <td>${narcoticActionMenu(r)}</td>
  </tr>`).join("") || `<tr><td colspan="9" class="empty-state">No prescriptions found for the selected criteria.</td></tr>`;
...
async function assignDrugToOpenDepartment() {
  if (APP.currentRole !== "ADMIN") return;
  const departmentId = APP.narcoticOpenDepartmentId;
  const drugId = q("narcoticDeptAssignDrug")?.value || "";
  if (!departmentId || !drugId) return;
  const stockRow = narcoticDeptStockRow(departmentId, drugId);
  if (!stockRow?.id) return;
  await updateDoc(doc(db, "narcotic_department_stock", stockRow.id), { isAssigned: true, updatedAt: serverTimestamp() });
  renderNarcoticStaticOptions();
  renderNarcoticDepartmentStockLists();
}
async function removeDrugFromOpenDepartment(drugId) {
  if (APP.currentRole !== "ADMIN") return;
  const departmentId = APP.narcoticOpenDepartmentId;
  const stockRow = narcoticDeptStockRow(departmentId, drugId);
  if (!stockRow?.id) return;
  await updateDoc(doc(db, "narcotic_department_stock", stockRow.id), { isAssigned: false, fixedStockUnits: 0, availableStockUnits: 0, updatedAt: serverTimestamp() });
  if (q("narcoticDeptModalDrug")?.value === drugId) q("narcoticDeptModalDrug").value = "";
  renderNarcoticStaticOptions();
  renderNarcoticDepartmentStockLists();
  renderNarcoticDepartmentModal();
}
...
function printNarcoticDispensingReport() {
  ...
  const totalUnits = rows.reduce((s,r)=>s+Number(r.dispensedUnits||0),0);
  const body = `
    <div class="section-title">Narcotic Dispensing Report · Dispensed Prescriptions Only</div>
...
function printNarcoticMovementsReport() {
  ...
  const totalSent = rows.reduce((s,r)=>s+Number(r.quantitySent||0),0);
  const body = `
    <div class="section-title">Department Orders Receipt Report · Department Movements Only</div>
...
document.addEventListener("click", event => {
  ...
  const narcoticEditMenuBtn = event.target.closest(".narcotic-edit-btn");
  if (narcoticEditMenuBtn) {
    closeRxActionMenus();
    return openNarcoticEditPrescription(narcoticEditMenuBtn.dataset.id);
  }
  ...
  if (removeDeptDrugBtn) {
    return removeDrugFromOpenDepartment(removeDeptDrugBtn.dataset.removeDeptDrug);
  }
...
q("narcoticDrug").onchange = () => { updateNarcoticAvailableStock(); renderNarcoticDispensingChart(); };
q("narcoticDepartment").onchange = () => { renderNarcoticStaticOptions(); updateNarcoticAvailableStock(); };
...
if (q("assignNarcoticDeptDrugBtn")) q("assignNarcoticDeptDrugBtn").onclick = assignDrugToOpenDepartment;
if (q("narcoticDeptStockSearch")) q("narcoticDeptStockSearch").oninput = renderNarcoticDepartmentStockLists;
...
if (q("narcoticChartDrugFilter")) q("narcoticChartDrugFilter").onchange = renderNarcoticDispensingChart;
if (q("narcoticChartDepartmentFilter")) q("narcoticChartDepartmentFilter").onchange = renderNarcoticDispensingChart;
