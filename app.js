...
function printTransactionsPage() {
  ...
  const renderTxSection = (title, rows, columns, renderMeta, emptyText, signatures = "") => `
    <div class="tx-print-section">
      ...
      <div class="section">
        <table>
          ...
          <tbody>
            ${rows.map(row => `
              <tr>${renderMeta(row).cells.map(cell => `<td>${cell}</td>`).join('')}</tr>
              <tr class="tx-meta-row"><td colspan="${columns.length}">${renderMeta(row).meta}</td></tr>
            `).join('') || `<tr><td colspan="${columns.length}">${emptyText}</td></tr>`}
          </tbody>
        </table>
        ${signatures}
      </div>
  ...
  const body = `
    ...
    ${renderTxSection(
      ...
      row => ({
        cells: [
          ...
          esc(row.pharmacy || '-'),
          esc(row.doctorName || '-'),
          `${Number(row.qtyBoxes || 0)} box(es) + ${Number(row.qtyUnits || 0)} unit(s)`
        ],
        meta: `<strong>Dispensed by:</strong> ${esc(row.registeredBy || row.pharmacistName || row.performedBy || '-')} &nbsp; | &nbsp; <strong>Date & Time:</strong> ${esc(formatJordanDateTime(row.registeredDateTime || row.dateTime || ''))} &nbsp; | &nbsp; <strong>Drug:</strong> ${esc(row.tradeName || '-')}`
      }),
      'No dispensing transactions found.'
    )}
    ${renderTxSection(
      'Edited Prescriptions',
      editRows,
      ['Patient Name', 'File Number', 'Pharmacy', 'Doctor', 'Quantity Before / After'],
      row => {
        const oldValues = row.oldValues || {};
        const newValues = row.newValues || {};
        return {
          cells: [
            esc(newValues.patientName || oldValues.patientName || row.patientName || '-'),
            esc(newValues.fileNumber || oldValues.fileNumber || row.fileNumber || '-'),
            esc(row.pharmacy || '-'),
            esc(newValues.doctorName || oldValues.doctorName || row.doctorName || '-'),
            `${Number(oldValues.qtyBoxes || 0)} box(es) + ${Number(oldValues.qtyUnits || 0)} unit(s) → ${Number(newValues.qtyBoxes || 0)} box(es) + ${Number(newValues.qtyUnits || 0)} unit(s)`
          ],
          meta: `<strong>Edited by:</strong> ${esc(row.editedBy || row.performedBy || '-')} &nbsp; | &nbsp; <strong>Date & Time:</strong> ${esc(formatJordanDateTime(row.editedDateTime || row.dateTime || ''))} &nbsp; | &nbsp; <strong>Drug:</strong> ${esc(row.tradeName || '-')}`
        };
      },
    ...
    ${renderTxSection(
      ...
      row => ({
        ...
        meta: `<strong>Deleted by:</strong> ${esc(row.deletedBy || row.performedBy || '-')} &nbsp; | &nbsp; <strong>Date & Time:</strong> ${esc(formatJordanDateTime(row.deletedDateTime || row.dateTime || ''))} &nbsp; | &nbsp; <strong>Drug:</strong> ${esc(row.tradeName || '-')}`
      }),
    ...
    ${renderTxSection(
      ...
      row => ({
        ...
        meta: `<strong>Returned by:</strong> ${esc(row.returnBy || row.performedBy || '-')} &nbsp; | &nbsp; <strong>Date & Time:</strong> ${esc(formatJordanDateTime(row.returnDateTime || row.dateTime || ''))} &nbsp; | &nbsp; <strong>Drug:</strong> ${esc(row.tradeName || '-')}`
      }),
    ...
    ${renderTxSection(
      'Shipment Receipts',
      shipmentRows,
      ['Date & Time', 'Pharmacy', 'Drug', 'Received Boxes', 'Received Units', 'Invoice Number', 'Invoice Date'],
      row => ({
        cells: [
          esc(formatJordanDateTime(row.dateTime || '')),
          esc(row.pharmacy || '-'),
          esc(row.tradeName || '-'),
          Number(row.qtyBoxes || 0),
          Number(row.qtyUnits || 0),
          esc(row.invoiceNumber || '-'),
          esc(row.invoiceDate || '-')
        ],
        meta: `<strong>Received by:</strong> ${esc(row.receivedBy || row.performedBy || '-')} &nbsp; | &nbsp; <strong>Recorded at:</strong> ${esc(formatJordanDateTime(row.receivedDateTime || row.dateTime || ''))}`
      }),
    ...
    ${renderTxSection(
      'Stock Transfers',
      transferRows,
      ['Date & Time', 'From Pharmacy', 'To Pharmacy', 'Drug', 'Transferred Quantity', 'Transferred By', 'Received By'],
      row => ({
        cells: [
          esc(formatJordanDateTime(row.dateTime || '')),
          esc(row.fromPharmacy || String(row.pharmacy || '').split('→')[0]?.trim() || '-'),
          esc(row.toPharmacy || String(row.pharmacy || '').split('→')[1]?.trim() || '-'),
          esc(row.tradeName || '-'),
          `${Number(row.qtyBoxes || 0)} box(es) + ${Number(row.qtyUnits || 0)} unit(s)`,
          esc(row.performedBy || '-'),
          esc(row.receiverPharmacist || '-')
        ],
        meta: `<strong>Transfer recorded at:</strong> ${esc(formatJordanDateTime(row.transferredDateTime || row.dateTime || ''))}`
      }),
      'No stock transfer transactions found.',
      `
      <div class="tx-signature-row">
        <div class="tx-signature-box"><span>Transferred By</span><strong>${esc(transferRows[0]?.performedBy || '-')}</strong><div class="tx-sign-line"></div></div>
        <div class="tx-signature-box"><span>Received By</span><strong>${esc(transferRows[0]?.receiverPharmacist || '-')}</strong><div class="tx-sign-line"></div></div>
      </div>`
    )}
  `;
...
function printNarcoticTransactionsPage() {
  ...
  const renderSection = (title, rows, columns, renderMeta, emptyText, signatures = '') => `
    <div class="tx-print-section">
      <div class="section-title">${title}</div>
      <div class="section">
        <table>
  ...
  const body = `
    ...
    ${renderSection('Narcotic Dispensing', registerRows, ['Patient Name', 'File Number', 'Department', 'Drug', 'Dispensed Units'], row => ({
      cells: [esc(row.patientName || '-'), esc(row.fileNumber || '-'), esc(row.departmentName || '-'), esc(row.drugName || '-'), Number(row.dispensedUnits || 0)],
      meta: `<strong>Dispensed by:</strong> ${esc(row.performedBy || '-')} &nbsp; | &nbsp; <strong>Date & Time:</strong> ${esc(formatJordanDateTime(row.dateTime || ''))}`
    }), 'No narcotic dispensing records found.')}
    ${renderSection('Deleted Narcotic Prescriptions', deleteRows, ['Patient Name', 'File Number', 'Department', 'Drug', 'Dispensed Units'], row => ({
      cells: [esc(row.patientName || '-'), esc(row.fileNumber || '-'), esc(row.departmentName || '-'), esc(row.drugName || '-'), Number(row.quantitySent || 0)],
      meta: `<strong>Deleted by:</strong> ${esc(row.deletedBy || row.performedBy || '-')} &nbsp; | &nbsp; <strong>Date & Time:</strong> ${esc(formatJordanDateTime(row.deletedDateTime || row.dateTime || ''))}`
    }), 'No deleted narcotic prescriptions found.')}
    ${renderSection('Returned Narcotic Prescriptions', returnRows, ['Patient Name', 'File Number', 'Department', 'Drug', 'Returned Units'], row => ({
      cells: [esc(row.patientName || '-'), esc(row.fileNumber || '-'), esc(row.departmentName || '-'), esc(row.drugName || '-'), Number(row.quantitySent || 0)],
      meta: `<strong>Returned by:</strong> ${esc(row.returnBy || row.performedBy || '-')} &nbsp; | &nbsp; <strong>Date & Time:</strong> ${esc(formatJordanDateTime(row.returnDateTime || row.dateTime || ''))}`
    }), 'No returned narcotic prescriptions found.')}
    ${renderSection('Internal Narcotic Receipts', internalReceiptRows, ['Date & Time', 'Drug', 'Received Quantity', 'Invoice Number', 'Invoice Date'], row => ({
      cells: [esc(formatJordanDateTime(row.dateTime || '')), esc(row.drugName || '-'), Number(row.quantityReceived || 0), esc(row.invoiceNumber || '-'), esc(row.invoiceDate || '-')],
      meta: `<strong>Received by:</strong> ${esc(row.performedBy || '-')}`
    }), 'No internal narcotic receipts found.')}
    ${renderSection('Department Order Movements', departmentOrderRows, ['Date & Time', 'Department', 'Drug', 'Quantity Sent', 'Empty Ampoules', 'Pharmacist', 'Nurse'], row => ({
      cells: [esc(formatJordanDateTime(row.dateTime || '')), esc(row.departmentName || '-'), esc(row.drugName || '-'), Number(row.quantitySent || 0), Number(row.emptyAmpoulesReceived || 0), esc(row.performedBy || '-'), esc(row.nurseName || '-')],
      meta: `<strong>Notes:</strong> ${esc(row.notes || '-')}`
    }), 'No department order movements found.', `
      <div class="tx-signature-row">
        <div class="tx-signature-box"><span>Pharmacist Signature</span><strong>${esc(departmentOrderRows[0]?.performedBy || '-')}</strong><div class="tx-sign-line"></div></div>
        <div class="tx-signature-box"><span>Nurse Signature</span><strong>${esc(departmentOrderRows[0]?.nurseName || '-')}</strong><div class="tx-sign-line"></div></div>
      </div>`)}
  `;
  const w = window.open("", "_blank");
