...
function buildDrugOptionsHtml(selectedValue = "") {
  const rows = sortDrugsAlphabetically(APP.cache.drugs || []);
  return `<option value="">Select Drug</option>` + rows.map(drug => {
    const selected = String(drug.id) === String(selectedValue) ? "selected" : "";
    return `<option value="${esc(drug.id)}" ${selected}>${esc(drug.tradeName || "")} ${esc(drug.strength || "")}</option>`;
  }).join("");
}

function buildLocationOptionsHtml(selectedValue = "", excludeValue = "") {
  const scope = currentScopePharmacy();
  const isAdmin = APP.currentRole === "ADMIN";
  const locations = isAdmin ? PHARMACIES : [scope];
  const rows = locations.filter(name => !excludeValue || name !== excludeValue);
...
function shipmentRowMarkup(index, values = {}) {
  return `
    <div class="batch-row shipment-line">
      <div class="batch-row-head">
        <div class="batch-row-title">Drug ${index + 1}</div>
        <button type="button" class="soft-btn remove-row-btn remove-shipment-row-btn">Remove</button>
      </div>
      <div class="form-grid two-col">
        <div class="field full"><label>Drug</label><select class="shipment-drug">${buildDrugOptionsHtml(values.drugId || "")}</select></div>
        <div class="field"><label>Boxes</label><input class="shipment-boxes" type="number" min="0" value="${Number(values.boxes || 0)}"></div>
        <div class="field"><label>Units</label><input class="shipment-units" type="number" min="0" value="${Number(values.units || 0)}"></div>
        <div class="field full"><label>Stock Location</label><select class="shipment-location">${buildLocationOptionsHtml(values.pharmacy || currentScopePharmacy())}</select></div>
      </div>
    </div>`;
}

function transferRowMarkup(index, values = {}) {
  const fromValue = values.from || currentScopePharmacy();
  const toDefault = values.to || PHARMACIES.find(name => name !== fromValue) || currentScopePharmacy();
  return `
    <div class="batch-row transfer-line">
      <div class="batch-row-head">
        <div class="batch-row-title">Drug ${index + 1}</div>
        <button type="button" class="soft-btn remove-row-btn remove-transfer-row-btn">Remove</button>
      </div>
      <div class="form-grid two-col">
        <div class="field full"><label>Drug</label><select class="transfer-drug">${buildDrugOptionsHtml(values.drugId || "")}</select></div>
        <div class="field"><label>From</label><select class="transfer-from">${buildLocationOptionsHtml(fromValue)}</select></div>
        <div class="field"><label>To</label><select class="transfer-to">${buildLocationOptionsHtml(toDefault, fromValue)}</select></div>
        <div class="field"><label>Boxes</label><input class="transfer-boxes" type="number" min="0" value="${Number(values.boxes || 0)}"></div>
        <div class="field"><label>Units</label><input class="transfer-units" type="number" min="0" value="${Number(values.units || 0)}"></div>
      </div>
    </div>`;
}

function syncBatchTransferRowToOptions(row, preferredValue = "") {
  if (!row) return;
  const fromSelect = row.querySelector(".transfer-from");
  const toSelect = row.querySelector(".transfer-to");
  if (!fromSelect || !toSelect) return;
  const from = fromSelect.value || currentScopePharmacy();
  toSelect.innerHTML = buildLocationOptionsHtml(preferredValue || toSelect.value, from);
  if (!toSelect.value || toSelect.value === from) {
    const firstValue = [...toSelect.options].map(opt => opt.value).find(Boolean) || "";
    toSelect.value = firstValue;
  }
}

function refreshShipmentRows() {
  const container = q("shipmentRows");
  if (!container) return;
  const rows = qq(".shipment-line", container);
  rows.forEach((row, index) => {
    const title = row.querySelector(".batch-row-title");
    if (title) title.textContent = `Drug ${index + 1}`;
    const removeBtn = row.querySelector(".remove-shipment-row-btn");
    if (removeBtn) removeBtn.disabled = rows.length === 1;
  });
}

function refreshTransferRows() {
  const container = q("transferRows");
  if (!container) return;
  const rows = qq(".transfer-line", container);
  rows.forEach((row, index) => {
    const title = row.querySelector(".batch-row-title");
    if (title) title.textContent = `Drug ${index + 1}`;
    const removeBtn = row.querySelector(".remove-transfer-row-btn");
    if (removeBtn) removeBtn.disabled = rows.length === 1;
    syncBatchTransferRowToOptions(row);
  });
}

function addShipmentRow(values = {}) {
  const container = q("shipmentRows");
  if (!container) return;
  container.insertAdjacentHTML("beforeend", shipmentRowMarkup(qq(".shipment-line", container).length, values));
  refreshShipmentRows();
}
...
function addTransferRow(values = {}) {
  const container = q("transferRows");
  if (!container) return;
  container.insertAdjacentHTML("beforeend", transferRowMarkup(qq(".transfer-line", container).length, values));
  refreshTransferRows();
}

function resetShipmentBatchRows() {
  const container = q("shipmentRows");
  if (!container) return;
  container.innerHTML = "";
  addShipmentRow({ pharmacy: APP.currentRole === "ADMIN" ? "General Hospital Stock" : currentScopePharmacy() });
}

function resetTransferBatchRows() {
  const container = q("transferRows");
  if (!container) return;
  container.innerHTML = "";
  addTransferRow({ from: APP.currentRole === "ADMIN" ? "General Hospital Stock" : currentScopePharmacy() });
}

function getShipmentBatchRows() {
  return qq(".shipment-line", q("shipmentRows")).map(row => ({
    row,
    drugId: row.querySelector(".shipment-drug")?.value || "",
    boxes: Number(row.querySelector(".shipment-boxes")?.value || 0),
    units: Number(row.querySelector(".shipment-units")?.value || 0),
    pharmacy: row.querySelector(".shipment-location")?.value || currentScopePharmacy()
  }));
}

function getTransferBatchRows() {
  return qq(".transfer-line", q("transferRows")).map(row => ({
    row,
    drugId: row.querySelector(".transfer-drug")?.value || "",
    boxes: Number(row.querySelector(".transfer-boxes")?.value || 0),
    units: Number(row.querySelector(".transfer-units")?.value || 0),
    from: row.querySelector(".transfer-from")?.value || currentScopePharmacy(),
    to: row.querySelector(".transfer-to")?.value || currentScopePharmacy()
  }));
}
...
function renderRecentPrescriptionsModal() {
  ...
  q("recentPrescriptionsTbody").innerHTML = rows.map(row => {
    ...
    const menuId = `recent_rx_menu_${row.id}`;
    return `
      <tr>
        ...
        <td colspan="2">${renderRxActionMenu(menuId, row, allowed)}</td>
      </tr>`;
  }).join("") || `<tr><td colspan="11" class="empty-state">No recent prescriptions found.</td></tr>`;
}



function suspendRecentPrescriptionsModalForAction() { return; }

function restoreRecentPrescriptionsModalAfterAction() { return; }

function openGenericMenuPortal(btn, menuId) {
  const sourceMenu = q(menuId);
  const portal = q("auditMenuPortal");
  if (!sourceMenu || !portal) return;

  const alreadyOpen = !portal.classList.contains("hidden") && portal.dataset.sourceId === menuId;
  if (alreadyOpen) {
  ...
  portal.dataset.sourceId = menuId;
  portal.classList.remove("hidden");

  const rect = btn.getBoundingClientRect();
  const portalWidth = Math.max(portal.offsetWidth || 210, 210);
  const left = Math.min(window.innerWidth - portalWidth - 12, Math.max(12, rect.right - portalWidth));
  let top = rect.bottom + 6;
  const portalHeight = Math.max(portal.offsetHeight || 160, 160);
...
function renderRxActionMenu(menuId, row, allowEdit = true) {
  return `
    <div class="row-actions-wrap">
      <button class="soft-btn mini-btn row-actions-btn" data-rx-menu="${menuId}">Actions</button>
      <div class="audit-menu hidden" id="${menuId}">
        ${row.status === "Returned" ? "" : `<button class="rx-menu-item rx-action-return" data-id="${row.id}">Return</button>`}
        ${row.status === "Returned" ? "" : `<button class="rx-menu-item rx-action-edit ${allowEdit ? "" : "hidden"}" data-id="${row.id}">Edit</button>`}
        ${APP.currentRole === "ADMIN" ? `<button class="rx-menu-item danger delete-rx-btn" data-id="${row.id}">Delete</button>` : ""}
      </div>
    </div>`;
}
    ...
    return `
      <div class="recent-row-layout">
        ...
        <div class="recent-row-actions">
          ${p.status === "Returned" ? statusBadge("Returned") : `<button class="soft-btn mini-btn latest-return-btn" data-id="${p.id}">Return</button>`}
          ${p.status === "Returned" ? "" : `<button class="primary-btn mini-btn latest-edit-btn ${canEdit ? "" : "hidden"}" data-id="${p.id}">Edit</button>`}
        </div>
      </div>`;
...
async function submitConfirmedAction() {
  ...
  restoreRecentPrescriptionsModalAfterAction();
...
async function registerQuickPrescription() {
  ...
  if (!payload.drugId || !payload.patientName || !payload.fileNumber || !payload.doctorName || !payload.pharmacistName) {
    ...
    q("actionOkBtn").classList.remove("hidden");
    if (!payload.patientName) q("quickPatientName")?.focus();
    else if (!payload.fileNumber) q("quickPatientFile")?.focus();
    else if (!payload.drugId) q("quickDrugSearch")?.focus();
    else if (!payload.doctorName) q("quickDoctor")?.focus();
    else q("quickPharmacist")?.focus();
    return;
  }
...
function renderDrugRows() {
  ...
  q("drugRxTbody").innerHTML = prescriptionScopeRows().filter(row => row.drugId === APP.selectedDrugId).map(row => {
    const allowed = canEditPrescription(row);
    const menuId = `drug_rx_menu_${row.id}`;
    return `
    <tr>
      <td>${esc(formatJordanDateTime(row.dateTime))}</td>
      <td>${esc(row.patientName || "")}</td>
...
async function receiveShipment() {
  ...
  const drug = APP.cache.drugs.find(d => d.id === drugId);
  if (!drug) {
    showActionModal("Receive Shipment", "Selected drug was not found.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }

  let inv = invRow(drugId, pharmacy);
  const unitsPerBox = Number(drug.unitsPerBox || 1);
  ...
  try {
    ...
    if (!inv) {
      inv = {
        id: `${drugId}__${String(pharmacy).replace(/\s+/g, "_")}`,
        drugId,
        pharmacy,
        boxes: 0,
        units: 0,
    ...
    const updated = normalizeInventory(
      Number(inv.boxes || 0) + boxes,
      Number(inv.units || 0) + units,
      unitsPerBox
    );
    ...
    APP.cache.inventory = APP.cache.inventory || [];
    const invIndex = APP.cache.inventory.findIndex(row => row.id === inv.id);
    const nextInv = { ...inv, ...updated, updatedAt: jordanNowIso() };

    if (invIndex === -1) APP.cache.inventory.push(nextInv);
    else APP.cache.inventory[invIndex] = nextInv;
...
async function transferStock() {
  ...
  if (!drugId) {
    showActionModal("Transfer", "Please select a drug.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  ...
  if (boxes < 0 || units < 0 || (boxes === 0 && units === 0)) {
    showActionModal("Transfer", "Please enter a quantity greater than zero.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  ...
  const drug = APP.cache.drugs.find(d => d.id === drugId);
  if (!drug) {
    showActionModal("Transfer", "Selected drug was not found.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }

  const unitsPerBox = Number(drug.unitsPerBox || 1);
  let fromInv = invRow(drugId, from);
  let toInv = invRow(drugId, to);
  ...
  try {
    ...
    if (!fromInv) {
      fromInv = {
        id: `${drugId}__${String(from).replace(/\s+/g, "_")}`,
        drugId,
        pharmacy: from,
        boxes: 0,
        units: 0,
    ...
    if (!toInv) {
      toInv = {
        id: `${drugId}__${String(to).replace(/\s+/g, "_")}`,
        drugId,
        pharmacy: to,
        boxes: 0,
        units: 0,
    ...
    if (delta > Number(fromInv.totalUnits || 0)) {
      if (operations.length) await apiRequest("batch", { operations });
      APP.cache.inventory = APP.cache.inventory || [];
      if (!APP.cache.inventory.find(r => r.id === fromInv.id)) APP.cache.inventory.push(fromInv);
      if (!APP.cache.inventory.find(r => r.id === toInv.id)) APP.cache.inventory.push(toInv);
    ...
    operations.push(
      {
        ...
        id: fromInv.id,
        data: {
          boxes: updatedFrom.boxes,
          units: updatedFrom.units,
          totalUnits: updatedFrom.totalUnits,
          updatedAt: jordanNowIso()
        }
      },
      {
        ...
        id: toInv.id,
        data: {
          boxes: updatedTo.boxes,
          units: updatedTo.units,
          totalUnits: updatedTo.totalUnits,
          updatedAt: jordanNowIso()
        }
      },
      {
    ...
    APP.cache.inventory = APP.cache.inventory || [];
    const fromIdx = APP.cache.inventory.findIndex(r => r.id === fromInv.id);
    const toIdx = APP.cache.inventory.findIndex(r => r.id === toInv.id);

    const nextFrom = { ...fromInv, ...updatedFrom, updatedAt: jordanNowIso() };
    const nextTo = { ...toInv, ...updatedTo, updatedAt: jordanNowIso() };

    if (fromIdx === -1) APP.cache.inventory.push(nextFrom);
    else APP.cache.inventory[fromIdx] = nextFrom;

    if (toIdx === -1) APP.cache.inventory.push(nextTo);
    else APP.cache.inventory[toIdx] = nextTo;
...
document.addEventListener("click", event => {
  ...
  const shipmentOpenBtn = event.target.closest("#openShipmentModalBtn, #openShipmentModalBtn2");
  if (shipmentOpenBtn) {
    refreshScopedSelectors();
    resetShipmentBatchRows();
    openModal("shipmentModal");
    return;
  }

  const transferOpenBtn = event.target.closest("#openTransferModalBtn, #openTransferModalBtn2");
  if (transferOpenBtn) {
    refreshScopedSelectors();
    resetTransferBatchRows();
  ...
  const rxMenuBtn = event.target.closest("[data-rx-menu]");
  if (rxMenuBtn) {
    openGenericMenuPortal(rxMenuBtn, rxMenuBtn.dataset.rxMenu);
    return;
  }
  ...
  const returnBtn = event.target.closest(".return-btn, .rx-action-return");
  if (returnBtn) {
    closeAuditMenuPortal();
    return openConfirmActionModal("return", returnBtn.dataset.id);
  }


  const editBtn = event.target.closest(".edit-rx-btn, .rx-action-edit");
  if (editBtn) {
    closeAuditMenuPortal();
  ...
  const removeShipmentRowBtn = event.target.closest(".remove-shipment-row-btn");
  if (removeShipmentRowBtn) {
    const row = removeShipmentRowBtn.closest(".shipment-line");
    if (row && qq(".shipment-line", q("shipmentRows")).length > 1) row.remove();
    refreshShipmentRows();
    return;
  }

  const removeTransferRowBtn = event.target.closest(".remove-transfer-row-btn");
  if (removeTransferRowBtn) {
    const row = removeTransferRowBtn.closest(".transfer-line");
    if (row && qq(".transfer-line", q("transferRows")).length > 1) row.remove();
    refreshTransferRows();
    return;
  }
  ...
  if (!event.target.closest(".audit-menu-wrap") && !event.target.closest(".row-actions-wrap")) {
    closeAuditMenuPortal();
  }
...
q("actionOkBtn").onclick = () => { closeModal("actionModal"); restoreRecentPrescriptionsModalAfterAction(); };
...
if (q("openShipmentModalBtn")) q("openShipmentModalBtn").onclick = () => { refreshScopedSelectors(); resetShipmentBatchRows(); openModal("shipmentModal"); };
if (q("openShipmentModalBtn2")) q("openShipmentModalBtn2").onclick = () => { refreshScopedSelectors(); resetShipmentBatchRows(); openModal("shipmentModal"); };
if (q("openTransferModalBtn")) q("openTransferModalBtn").onclick = () => { refreshScopedSelectors(); resetTransferBatchRows(); openModal("transferModal"); };
if (q("openTransferModalBtn2")) q("openTransferModalBtn2").onclick = () => { refreshScopedSelectors(); resetTransferBatchRows(); openModal("transferModal"); };
if (q("addShipmentRowBtn")) q("addShipmentRowBtn").onclick = () => addShipmentRow({ pharmacy: APP.currentRole === "ADMIN" ? "General Hospital Stock" : currentScopePharmacy() });
if (q("addTransferRowBtn")) q("addTransferRowBtn").onclick = () => addTransferRow({ from: APP.currentRole === "ADMIN" ? "General Hospital Stock" : currentScopePharmacy() });
...
if (q("quickDrug")) {
  q("quickDrug").addEventListener("change", () => {
    updateQuickAvailableStock();
  });
}
...
q("closeRecentPrescriptionsBtn").onclick = () => { const modal = q("recentPrescriptionsModal"); closeModal("recentPrescriptionsModal"); if (modal) { modal.classList.remove("modal-suspended"); delete modal.dataset.suspended; } };
q("viewPatientHistoryBtn").onclick = openPatientHistoryModal;
...
enableAutoFocusFlow(
  ["quickPatientName","quickPatientFile","quickPrescriptionType","quickDrugSearch","quickDrug","quickDoctor","quickPharmacist","quickBoxes","quickUnits"],
  () => registerQuickPrescription()
);


q("overlay").onclick = () => {
  ["passwordModal","actionModal","changePasswordModal","shipmentModal","transferModal","addDrugModal","editPrescriptionModal","confirmActionModal","themeModal","recentPrescriptionsModal","patientHistoryModal","duplicateWarningModal"].forEach(id => {
    if (q(id) && !q(id).classList.contains("hidden")) closeModal(id);
  });
};

q("loginPassword").addEventListener("keydown", event => {
  if (event.key === "Enter") {
    event.preventDefault();
    doLogin(APP.pendingRole, q("loginPassword").value);
  }
});
...
if (q("openTransferModalBtn")) q("openTransferModalBtn").onclick = () => { refreshScopedSelectors(); resetTransferBatchRows(); openModal("transferModal"); };
if (q("openTransferModalBtn2")) q("openTransferModalBtn2").onclick = () => { refreshScopedSelectors(); resetTransferBatchRows(); openModal("transferModal"); };
if (q("saveShipmentBtn")) q("saveShipmentBtn").onclick = receiveShipment;
if (q("saveTransferBtn")) q("saveTransferBtn").onclick = transferStock;
if (q("addShipmentRowBtn")) q("addShipmentRowBtn").onclick = () => addShipmentRow({ pharmacy: APP.currentRole === "ADMIN" ? "General Hospital Stock" : currentScopePharmacy() });
if (q("addTransferRowBtn")) q("addTransferRowBtn").onclick = () => addTransferRow({ from: APP.currentRole === "ADMIN" ? "General Hospital Stock" : currentScopePharmacy() });
...
document.addEventListener("change", event => {
  const transferFrom = event.target.closest(".transfer-from");
  if (transferFrom) {
    const row = transferFrom.closest(".transfer-line");
    syncBatchTransferRowToOptions(row);
  }
});
