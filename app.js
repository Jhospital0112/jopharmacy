...
const API_URL = (window.CDMS_CONFIG || window.APP_CONFIG || {}).APPS_SCRIPT_WEB_APP_URL || "";

if (!API_URL) {
  alert("Google Sheets backend is not configured yet. Open config.js and add APPS_SCRIPT_WEB_APP_URL first.");
}
...
async function apiRequest(action, payload = {}) {
  ...
  try {
    ...
    throw new Error(text || "Invalid response from Google Sheets backend.");
  }

  if (!result?.ok) {
    throw new Error(result?.error || "Google Sheets backend error.");
  }
...
function applyLocalFilters(rows, filters = []) {
  return (rows || []).filter(row => filters.every(f => {
    if (!f) return true;
    if (f.op === "==" || f.op === "eq") return String(row?.[f.field] ?? "") === String(f.value ?? "");
    return true;
  }));
}

async function getDoc(docRef) {
  const result = await apiRequest("getDoc", { table: docRef.table, id: docRef.id });
  return makeDocSnapshot(result.data || null);
}

async function fetchRows(ref) {
  const result = await apiRequest("listDocs", { table: ref.table });
  const rows = result.data || [];
  return ref.kind === "query" ? applyLocalFilters(rows, ref.filters) : rows;
}
...
function onSnapshot(ref, callback) {
  ...
  emit();
  const intervalId = setInterval(emit, SNAPSHOT_POLL_MS);
...
async function addDoc(collectionRef, payload) {
  ...
  await apiRequest("setDoc", { table: collectionRef.table, id, data: row });
  return { id };
}
...
function writeBatch() {
  const ops = [];
  return {
    set(ref, data) { ops.push({ type: "set", table: ref.table, id: ref.id, data: cleanData({ id: ref.id, ...data }) }); },
    update(ref, data) { ops.push({ type: "update", table: ref.table, id: ref.id, data: cleanData(data) }); },
    delete(ref) { ops.push({ type: "delete", table: ref.table, id: ref.id }); },
    async commit() {
      if (!ops.length) return;
      await apiRequest("batch", { operations: ops });
    }
  };
}

const DEFAULT_PASSWORD = "111111";
// Internal app password only. Database access itself is controlled by the Google Sheets web app backend.
...
function applyTheme(theme) {
  const safeTheme = ["light", "dark", "blue", "green"].includes(theme) ? theme : "light";
  document.body.classList.remove("dark", "theme-blue", "theme-green");
  if (safeTheme === "dark") document.body.classList.add("dark");
  if (safeTheme === "blue") document.body.classList.add("theme-blue");
  if (safeTheme === "green") document.body.classList.add("theme-green");
  localStorage.setItem("cdms_theme", safeTheme);
  const picker = q("themeSelect");
  if (picker) picker.value = safeTheme;
}
...
function openThemeModal() {
  const picker = q("themeSelect");
  if (picker) picker.value = localStorage.getItem("cdms_theme") || "light";
  openModal("themeModal");
}

function saveThemeSelection() {
  applyTheme(q("themeSelect")?.value || "light");
...
function findDuplicatePrescriptions(payload) {
  ...
  if (!history.length) {
    return { exactDrugDuplicates: [], smartDuplicates: [], allHistory: [] };
  }
  ...
  return { exactDrugDuplicates, smartDuplicates, allHistory: history };
}
...
function openDuplicateWarningModal(payload, duplicateResult) {
  ...
  const allRows = [...(duplicateResult.exactDrugDuplicates || []), ...(duplicateResult.smartDuplicates || [])]
    .sort((a, b) => String(b.dateTime || "").localeCompare(String(a.dateTime || "")));
...
q("themeToggle").onclick = () => { q("userMenuDropdown").classList.add("hidden"); openThemeModal(); };
...
q("saveThemeBtn").onclick = saveThemeSelection;
