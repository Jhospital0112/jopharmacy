...
async function performReturnNarcoticPrescription(id, pharmacistName) {
  ...
  batch.update(doc(db, "narcotic_prescriptions", id), { status: "Returned", returnPharmacist: pharmacistName, updatedAt: serverTimestamp(), updatedBy: currentActorName() || APP.currentRole });
...
async function performDeletePrescription(id, pharmacistName) {
  ...
  batch.set(doc(collection(db, "transactions")), {
    type: "Delete Prescription", drugId: rx.drugId, tradeName: drug?.tradeName || "", pharmacy: rx.pharmacy, qtyBoxes: rx.qtyBoxes || 0, qtyUnits: rx.qtyUnits || 0, performedBy: pharmacistName, patientName: rx.patientName || "", fileNumber: rx.fileNumber || "", deletedBy: pharmacistName, deletedDateTime: jordanNowIso(), note: `Prescription deleted for ${rx.patientName || ""}`, dateTime: jordanNowIso()
  });
...
async function performReturnPrescription(id, pharmacistName) {
  ...
  const batch = writeBatch(db);
  batch.update(doc(db, "prescriptions", id), { status: "Returned", returnBy: pharmacistName, returnDateTime: jordanNowIso(), updatedBy: currentActorName() || APP.currentRole });
  batch.update(doc(db, "inventory", inv.id), { ...updatedStock, updatedAt: serverTimestamp() });
  batch.set(doc(collection(db, "transactions")), { type: "Return", drugId: rx.drugId, tradeName: drug.tradeName, pharmacy: rx.pharmacy, qtyBoxes: Number(rx.qtyBoxes || 0), qtyUnits: Number(rx.qtyUnits || 0), performedBy: pharmacistName, patientName: rx.patientName || "", fileNumber: rx.fileNumber || "", returnBy: pharmacistName, returnDateTime: jordanNowIso(), note: `Returned prescription for ${rx.patientName}`, dateTime: jordanNowIso() });
  await batch.commit();
  finishActionModal(true, "Prescription returned successfully.");
}
...
async function auditPrescription(id, status, note) {
  ...
  await updateDoc(doc(db, "prescriptions", id), {
    ...
    auditNote: note || "",
    updatedBy: currentActorName() || APP.currentRole
  });
  await addDoc(collection(db, "transactions"), {
    type: `Audit ${status}`,
...
async function saveEditedPrescription() {
  ...
  batch.set(doc(collection(db, "transactions")), {
    ...
    qtyBoxes,
    qtyUnits,
    performedBy: APP.currentUser?.displayName || APP.currentRole,
    patientName,
    fileNumber,
    editedBy: APP.currentUser?.displayName || APP.currentRole,
...
async function transferStock() {
  ...
  try {
    ...
    for (const item of batchRows) {
      ...
      operations.push({ type: "set", table: "transactions", id: crypto.randomUUID(), data: { type: "Transfer", batchId: transactionBatchId, drugId, tradeName: drug.tradeName || "", pharmacy: `${from} → ${to}`, fromPharmacy: from, toPharmacy: to, receiverPharmacist, qtyBoxes: Number(boxes || 0), qtyUnits: Number(units || 0), performedBy: APP.currentUser?.displayName || APP.currentRole || "", note: "Stock transfer", dateTime: transferredAt } });
...
function userCanAudit(){ return APP.currentRole === "ADMIN" || !!APP.currentUser?.canAudit; }
function canAccessPrescriptionPharmacy(pharmacy){ return APP.currentRole === "ADMIN" || getUserAllowedPharmacies(APP.currentUser || {}).includes(pharmacy); }
function canDeletePrescriptionRecord(row){ return !!row && (APP.currentRole === "ADMIN" || (userCanAudit() && canAccessPrescriptionPharmacy(row.pharmacy))); }
function canAuditPrescriptionRecord(row){ return !!row && (APP.currentRole === "ADMIN" || (userCanAudit() && canAccessPrescriptionPharmacy(row.pharmacy))); }

function quickDoseEnabled(){ return !!q("quickAddDoseToggle")?.checked; }
function quickDoseDailyUsage(){
  const units = Math.max(1, Number(q("quickDoseUnitsPerDose")?.value || 1));
  const freq = q("quickDoseFrequency")?.value || "Once Daily";
  const map = {"Once Daily":1,"Every 12 hr":2,"Every 8 hr":3,"Every 6 hr":4,"Every 4 hr":6};
  return units * (map[freq] || 1);
}
function updateQuickDosePreview(){
  if (!q("quickDosePreview")) return;
  const durV = Math.max(1, Number(q("quickDoseDurationValue")?.value || 1));
  const durU = q("quickDoseDurationUnit")?.value || "Day";
  const mult = durU === 'Week' ? 7 : durU === 'Month' ? 30 : 1;
  const total = quickDoseDailyUsage() * durV * mult;
  q("quickDosePreview").value = `${quickDoseDailyUsage()} / day · ${total} total units`;
}
function toggleQuickDoseFields(){
  if (!q("quickDoseFields")) return;
  q("quickDoseFields").classList.toggle("hidden", !quickDoseEnabled());
  if (q("quickDoseStartDate") && !q("quickDoseStartDate").value) q("quickDoseStartDate").value = jordanDateKey();
  updateQuickDosePreview();
}
async function saveDoseForPrescriptionId(prescriptionId, rx){
  if (!prescriptionId || !rx) return;
  await addDoc(collection(db, 'prescription_doses'), {
    prescriptionId,
    patientFileNumber: rx.fileNumber || '',
    drugId: rx.drugId,
    scientificName: APP.cache.drugs.find(d=>d.id===rx.drugId)?.scientificName || '',
    tradeName: APP.cache.drugs.find(d=>d.id===rx.drugId)?.tradeName || '',
    strength: APP.cache.drugs.find(d=>d.id===rx.drugId)?.strength || '',
    doseUnitsPerDose: Number(q('quickDoseUnitsPerDose')?.value || 1),
    frequency: q('quickDoseFrequency')?.value || 'Once Daily',
    durationValue: Number(q('quickDoseDurationValue')?.value || 1),
    durationUnit: q('quickDoseDurationUnit')?.value || 'Day',
    startDate: q('quickDoseStartDate')?.value || jordanDateKey(),
    calculatedDailyUnits: quickDoseDailyUsage(),
    createdAt: jordanNowIso(),
    createdBy: currentActorName() || APP.currentRole,
    active: true
  });
}

const _continueQuickRegistration = continueQuickRegistration;
continueQuickRegistration = async function(payload){
  const { drugId, patientName, fileNumber, doctorName, pharmacistName, prescriptionType, qtyBoxes, qtyUnits } = payload;
  const drug = APP.cache.drugs.find(d => d.id === drugId);
  const pharmacy = currentScopePharmacy();
  const inv = invRow(drugId, pharmacy);
  ...
  try {
    ...
    const rxData = { drugId, pharmacy, patientName, fileNumber, doctorName, pharmacistName, prescriptionType: prescriptionType || "", qtyBoxes, qtyUnits, status: "New", auditBy: "", auditDateTime: null, auditNote: "", returnBy: "", returnDateTime: null, returnNote: "", dateTime: now, createdBy: currentActorName() || APP.currentRole, updatedBy: currentActorName() || APP.currentRole };
    await setDoc(prescriptionRef, rxData);
    if (quickDoseEnabled()) await saveDoseForPrescriptionId(prescriptionRef.id, rxData);
    const txRef = doc(collection(db, "transactions"));
    await setDoc(txRef, { type: "Dispense", drugId, tradeName: drug.tradeName, pharmacy, qtyBoxes, qtyUnits, performedBy: pharmacistName || currentActorName() || APP.currentRole, note: `Prescription: ${patientName}`, dateTime: now, patientName, fileNumber, doctorName });
    if (!inv?.id) throw new Error("Inventory record was not found for the selected drug and pharmacy.");
    await updateDoc(doc(db, "inventory", inv.id), { ...updatedStock, updatedAt: serverTimestamp() });
    ["quickPatientName","quickPatientFile","quickDoctor"].forEach(id => q(id) && (q(id).value = ""));
    if (q("quickPrescriptionType")) q("quickPrescriptionType").value = "";
    ["quickBoxes","quickUnits"].forEach(id => q(id) && (q(id).value = "0"));
    if (q('quickAddDoseToggle')) q('quickAddDoseToggle').checked = false;
    toggleQuickDoseFields();
    APP.pendingQuickPayload = null;
    updateQuickAvailableStock();
    finishActionModal(true, "Prescription registered successfully.");
  } catch (error) {
...
renderAudit = function(){
  if (!userCanAudit()) { if (q('auditTbody')) q('auditTbody').innerHTML = `<tr><td colspan="9" class="empty-state">Audit access not granted.</td></tr>`; return; }
  const term = (q("auditSearch")?.value || "").toLowerCase();
  const scope = currentAuditPharmacy();
  const from = q("auditFromDate")?.value || "";
  const to = q("auditToDate")?.value || "";
  ...
  const effectiveTo = to || (!from ? jordanDateKey() : "");
  const rows = scopedPrescriptionRowsByPharmacy(scope).filter(row => {
    if (!canAuditPrescriptionRecord(row)) return false;
    if (APP.auditTab === "new") return (row.status || "New") === "New";
    if (APP.auditTab === "pending") return row.status === "Pending";
    return row.status === "Verified";
  }).filter(row => {
  ...
  q("auditTbody").innerHTML = rows.map(row => {
    ...
    if (APP.auditTab === "new") {
      menuItems.push(`<button class="audit-menu-item audit-btn" data-id="${row.id}" data-status="Verified" data-note="${noteId}">Verify</button>`);
      menuItems.push(`<button class="audit-menu-item audit-btn" data-id="${row.id}" data-status="Pending" data-note="${noteId}">Pending</button>`);
    } else if (APP.auditTab === "pending") menuItems.push(`<button class="audit-menu-item audit-btn" data-id="${row.id}" data-status="Verified" data-note="${noteId}">Verify</button>`);
    menuItems.push(`<button class="audit-menu-item edit-rx-btn" data-id="${row.id}">Edit</button>`);
    if (canDeletePrescriptionRecord(row)) menuItems.push(`<button class="audit-menu-item danger delete-rx-btn" data-id="${row.id}">Delete</button>`);
    return `<tr><td>${esc(formatJordanDateTime(row.dateTime))}</td><td>${esc((drug?.tradeName || "") + " " + (drug?.strength || ""))}</td><td>${esc(row.patientName || "")}</td><td>${esc(row.fileNumber || "")}</td><td>${Number(row.qtyBoxes || 0)}</td><td>${Number(row.qtyUnits || 0)}</td><td>${esc(row.doctorName || "")}</td><td><input id="${noteId}" class="audit-note" value="${esc(row.auditNote || "")}"></td><td><div class="audit-action-cell"><div class="audit-menu-wrap"><button class="audit-menu-btn" data-audit-menu="${row.id}">Actions</button><div class="audit-menu hidden" id="audit_menu_${row.id}">${menuItems.join('')}</div></div></div></td></tr>`;
  }).join("") || `<tr><td colspan="9" class="empty-state">No prescriptions found in this audit tab.</td></tr>`;
}

buildPrescriptionActionsDropdown = function(row, { prefix = "rx" } = {}){
  if (!row) return `<span class="muted">-</span>`;
  const allowed = canEditPrescription(row) && canAccessPrescriptionPharmacy(row.pharmacy);
  const canDelete = canDeletePrescriptionRecord(row) || prefix === 'narcotic';
  const canReturn = row.status !== 'Returned' && canAccessPrescriptionPharmacy(row.pharmacy);
  const menuId = `rx_actions_${prefix}_${row.id}`;
  return `<div class="rx-actions-wrap"><button class="soft-btn mini-btn rx-actions-btn" data-rx-actions-toggle="${menuId}">Actions ▾</button><div class="rx-actions-menu hidden" id="${menuId}">${canReturn?`<button class="soft-btn mini-btn latest-return-btn" data-id="${row.id}">Return</button>`:''}${allowed?`<button class="primary-btn mini-btn latest-edit-btn" data-id="${row.id}">Edit</button>`:''}${canDelete?`<button class="mini-danger-btn mini-btn delete-rx-btn" data-id="${row.id}">Delete</button>`:''}</div></div>`;
}

const _performDeletePrescription = performDeletePrescription;
performDeletePrescription = async function(id, pharmacistName){
  const rx = APP.cache.prescriptions.find(row => row.id === id);
  if (!canDeletePrescriptionRecord(rx)) return;
  return _performDeletePrescription(id, pharmacistName);
}
const _openConfirmActionModal = openConfirmActionModal;
openConfirmActionModal = function(type,id){ const rx=APP.cache.prescriptions.find(r=>r.id===id); if ((type==='delete'&&!canDeletePrescriptionRecord(rx)) || (type==='return'&&!canAccessPrescriptionPharmacy(rx?.pharmacy))) return; return _openConfirmActionModal(type,id); }

renderTransactions = function(){
  const rows = getFilteredTransactionsRows();
  q("transactionsTbody").innerHTML = rows.map(row => `
    <tr>
      <td><button class="soft-btn mini-btn transaction-expand-btn" data-scope="normal" data-id="${esc(row.id)}">＋</button></td>
      <td>${esc(formatJordanDateTime(row.dateTime))}</td>
      <td>${esc(row.type || "")}</td>
      <td>${esc(row.tradeName || "")}</td>
      <td>${Number(row.qtyBoxes || 0)}</td>
    ...
    <tr class="transaction-expand-row hidden" id="transaction_expand_normal_${esc(row.id)}"><td colspan="9"><div class="tx-expand-shell">${buildNormalTransactionDetails(row)}<div class="toolbar-inline tx-expand-actions"><button class="primary-btn mini-btn print-single-transaction-btn" data-scope="normal" data-id="${esc(row.id)}">Print Selected Transaction</button></div></div></td></tr>`).join("") || `<tr><td colspan="9" class="empty-state">No transactions found.</td></tr>`;
}
renderNarcoticTransactionsTable = function(){
  if (!q("narcoticTransactionsTbody")) return;
  const rows = getFilteredNarcoticTransactionsRows();
  q("narcoticTransactionsTbody").innerHTML = rows.map(row => `
    <tr>
      <td><button class="soft-btn mini-btn transaction-expand-btn" data-scope="narcotic" data-id="${esc(row.id)}">＋</button></td>
      <td>${esc(formatJordanDateTime(row.dateTime))}</td>
      <td>${esc(row.type || "")}</td>
      <td>${esc(row.drugName || "")}</td>
      <td>${esc(row.departmentName || "-")}</td>
    ...
    <tr class="transaction-expand-row hidden" id="transaction_expand_narcotic_${esc(row.id)}"><td colspan="12"><div class="tx-expand-shell">${buildNarcoticTransactionDetails(row)}<div class="toolbar-inline tx-expand-actions"><button class="primary-btn mini-btn print-single-transaction-btn" data-scope="narcotic" data-id="${esc(row.id)}">Print Selected Transaction</button></div></div></td></tr>`).join("") || `<tr><td colspan="12" class="empty-state">No narcotic transactions found for the selected criteria.</td></tr>`;
}

function printSingleTransaction(scope,id){
  const row = scope==='narcotic' ? (APP.cache.narcoticOrderMovements||[]).find(r=>String(r.id)===String(id)) : (APP.cache.transactions||[]).find(r=>String(r.id)===String(id));
  const body = scope==='narcotic' ? buildNarcoticTransactionDetails(row) : buildNormalTransactionDetails(row);
  const w = window.open('', '_blank');
  w.document.write(buildPrintShell('Transaction Details', currentScopePharmacy(), body));
  w.document.close();
}

function getFilteredPrescriptionRows(){
  const term=(q('prescriptionsSearchFilter')?.value||'').toLowerCase().trim();
  const drugId=q('prescriptionsDrugFilter')?.value||'';
  const pharmacyFilter=q('prescriptionsPharmacyFilter')?.value||'';
  const from=q('prescriptionsFromFilter')?.value||'';
  const to=q('prescriptionsToFilter')?.value||'';
  const effectiveFrom = from || (!to ? jordanDateKey() : '');
  const effectiveTo = to || (!from ? jordanDateKey() : '');
  const allowedPhs = APP.currentRole==='ADMIN' ? WORK_PHARMACIES : getUserAllowedPharmacies(APP.currentUser||{});
  return (APP.cache.prescriptions||[]).filter(r=>allowedPhs.includes(r.pharmacy)).filter(r=>{
    const drug=APP.cache.drugs.find(d=>d.id===r.drugId); const day=formatJordanDateTime(r.dateTime).slice(0,10);
    if (drugId && String(r.drugId)!==String(drugId)) return false;
    if (pharmacyFilter && r.pharmacy!==pharmacyFilter) return false;
    if (effectiveFrom && day<effectiveFrom) return false;
    if (effectiveTo && day>effectiveTo) return false;
    if (term && !`${r.patientName} ${r.fileNumber} ${r.doctorName} ${r.pharmacistName} ${drug?.tradeName||''} ${drug?.scientificName||''}`.toLowerCase().includes(term)) return false;
    return true;
  }).sort((a,b)=>String(b.dateTime||'').localeCompare(String(a.dateTime||'')));
}
function renderPrescriptionsPage(){
  if (!q('prescriptionsTbody')) return;
  const options = APP.cache.drugs.map(d=>`<option value="${esc(d.id)}">${esc(d.tradeName)} ${esc(d.strength)}</option>`).join('');
  if (q('prescriptionsDrugFilter')) q('prescriptionsDrugFilter').innerHTML = `<option value="">All Drugs</option>${options}`;
  const allowedPhs = APP.currentRole==='ADMIN' ? WORK_PHARMACIES : getUserAllowedPharmacies(APP.currentUser||{});
  if (q('prescriptionsPharmacyFilter')) q('prescriptionsPharmacyFilter').innerHTML = `<option value="">${APP.currentRole==='ADMIN'?'All Pharmacies':'My Pharmacy'}</option>${allowedPhs.map(p=>`<option value="${esc(p)}">${esc(p)}</option>`).join('')}`;
  const rows=getFilteredPrescriptionRows();
  q('prescriptionsTbody').innerHTML = rows.map(row=>{ const drug=APP.cache.drugs.find(d=>d.id===row.drugId); return `<tr><td>${esc(formatJordanDateTime(row.dateTime))}</td><td>${esc((drug?.tradeName||'')+' '+(drug?.strength||''))}</td><td>${esc(row.patientName||'')}</td><td>${esc(row.fileNumber||'')}</td><td>${esc(row.pharmacy||'')}</td><td>${Number(row.qtyBoxes||0)}</td><td>${Number(row.qtyUnits||0)}</td><td>${statusBadge(row.status||'New')}</td><td>${buildPrescriptionActionsDropdown(row,{prefix:'prescriptions'})}</td></tr>`;}).join('') || `<tr><td colspan="9" class="empty-state">No prescriptions found.</td></tr>`;
}

const _renderPatientsPage = renderPatientsPage;
renderPatientsPage = function(){
  if (q('patientsFileFilter') && !String(q('patientsFileFilter').value || '').trim()) {
    ['patientsTotalRx','patientsActiveMedications','patientsRemainingUnits','patientsClinicalAlerts'].forEach(id=>q(id)&&(q(id).textContent='0'));
    if (q('patientsTbody')) q('patientsTbody').innerHTML = `<tr><td colspan="10" class="empty-state">Enter patient file number to display prescriptions.</td></tr>`;
    return;
  }
  return _renderPatientsPage();
}

const _applyRoleUI = applyRoleUI;
applyRoleUI = function(){
  _applyRoleUI();
  const auditAllowed = userCanAudit();
  document.querySelectorAll('[data-page="audit"]').forEach(el=>el.classList.toggle('hidden', !auditAllowed));
  const auditPage = q('page-audit'); if (auditPage) auditPage.classList.toggle('admin-only-block', false);
};

const _saveEditedPrescription = saveEditedPrescription;
saveEditedPrescription = async function(){ return _saveEditedPrescription(); }
const _saveNarcoticEditPrescription = saveNarcoticEditPrescription;
saveNarcoticEditPrescription = async function(){ return _saveNarcoticEditPrescription(); }

// enhance event handling
const _docClickHandler = document.onclick;

document.addEventListener('click', function(event){
  const expBtn = event.target.closest('.transaction-expand-btn');
  if (expBtn) {
    const row = q(`transaction_expand_${expBtn.dataset.scope}_${expBtn.dataset.id}`);
    if (row) row.classList.toggle('hidden');
    return;
  }
  const printBtn = event.target.closest('.print-single-transaction-btn');
  if (printBtn) { printSingleTransaction(printBtn.dataset.scope, printBtn.dataset.id); return; }
});

if (q('quickAddDoseToggle')) q('quickAddDoseToggle').onchange = toggleQuickDoseFields;
['quickDoseUnitsPerDose','quickDoseFrequency','quickDoseDurationValue','quickDoseDurationUnit','quickDoseStartDate'].forEach(id=>{ if(q(id)){ q(id).oninput=updateQuickDosePreview; q(id).onchange=updateQuickDosePreview; }});
if (q('prescriptionsTodayBtn')) q('prescriptionsTodayBtn').onclick = ()=>{ const today=jordanDateKey(); q('prescriptionsFromFilter').value=today; q('prescriptionsToFilter').value=today; renderPrescriptionsPage(); };
if (q('prescriptionsClearFiltersBtn')) q('prescriptionsClearFiltersBtn').onclick = ()=>{ ['prescriptionsSearchFilter','prescriptionsFromFilter','prescriptionsToFilter'].forEach(id=>q(id)&&(q(id).value='')); ['prescriptionsDrugFilter','prescriptionsPharmacyFilter'].forEach(id=>q(id)&&(q(id).value='')); renderPrescriptionsPage(); };
['prescriptionsSearchFilter','prescriptionsDrugFilter','prescriptionsPharmacyFilter','prescriptionsFromFilter','prescriptionsToFilter'].forEach(id=>{ if(q(id)){ q(id).oninput=renderPrescriptionsPage; q(id).onchange=renderPrescriptionsPage; }});

const _renderAll = renderAll;
renderAll = function(){ _renderAll(); renderPrescriptionsPage(); };
const _showPage = showPage;
showPage = function(page){ _showPage(page); if (page==='prescriptions') renderPrescriptionsPage(); };
