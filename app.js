...
function closeRxActionMenus() {
  ...
  const portal = q("rxActionsPortal");
  if (!portal) return;
...
function openRxActionsPortal(btn, menuId) {
...
window.addEventListener("scroll", () => { closeAuditMenuPortal(); closeRxActionMenus(); }, true);
window.addEventListener("resize", () => { closeAuditMenuPortal(); closeRxActionMenus(); });
...
function setNarcoticDetailTab(tab) {
  ...
  if (tab === "manage") renderNarcoticDepartmentManageTable();
...
function renderNarcoticDepartmentFullStockSummary() {
  const summary = q("narcoticDeptStockSummaryAll");
  if (!summary) return;
  summary.innerHTML = "";
...
function renderNarcoticDepartmentManageTable() {
  ...
  q("narcoticDeptAssignedTbody").innerHTML = rows.map(row => {
    ...
    return `<tr>
      ...
      <td>${Number(row.availableStockUnits || 0)}</td>
      <td><button class="mini-danger-btn mini-btn" data-remove-dept-drug="${row.drugId}">Remove</button></td>
    </tr>`;
...
function openNarcoticOverflowModal(row, stockRow) {
  ...
  q("narcoticStockOverflowText").innerHTML = `
    ...
    <div><strong>Drug:</strong> ${esc(row.drugName || row.drugLabel || "")}</div>
...
function updateNarcoticReportPreview() {
  ...
  if (previewType === "movements") {
    ...
    q("narcoticReportPreviewTbody").innerHTML = rows.map(r => `<tr>
      <td>${esc(formatJordanDateTime(r.dateTime))}</td>
      <td>${esc(r.drugName || r.drugLabel || "")}</td>
...
function printNarcoticMovementsReport() {
  ...
  const body = `
    ...
    <div class="section" style="margin-top:42px;display:flex;justify-content:space-between;gap:40px">
...
document.addEventListener("click", event => {
  ...
  if (rxMenuToggle) {
    openRxActionsPortal(rxMenuToggle, rxMenuToggle.dataset.rxActionsToggle);
    return;
  }

  const narcoticInnerTabBtn = event.target.closest(".narcotic-inner-tab-btn");
  if (narcoticInnerTabBtn) {
    setNarcoticDetailTab(narcoticInnerTabBtn.dataset.narcoticdetailtab);
    return;
  }
