...
function renderNarcoticStaticOptions() {
  ...
  const drugOptions = APP.cache.narcoticDrugs.map(d => `<option value="${esc(d.id)}">${esc(d.tradeName)} ${esc(d.strength || "")}</option>`).join("");
  const deptOptions = APP.cache.narcoticDepartments.map(d => `<option value="${esc(d.id)}">${esc(d.departmentName || d.name || "")}</option>`).join("");
  ["narcoticDrug","narcoticOrderDrug","narcoticReportDrug","narcoticDeptModalDrug"].forEach(id => {
    if (q(id)) q(id).innerHTML = `<option value="">Select Drug</option>${drugOptions}`;
  });
...
function resetNarcoticEntryForm() {
  ["narcoticPatientName","narcoticFileNumber","narcoticNationalId","narcoticPrescriptionNumber","narcoticDoctorName","narcoticDose","narcoticRemainingDose","narcoticDiscardDose","narcoticNotes"].forEach(id => { if (q(id)) q(id).value = ""; });
...
function renderNarcoticDepartmentCards() {
  ...
  q("narcoticDepartmentCards").innerHTML = APP.cache.narcoticDepartments
    ...
    .map(dept => {
      const stockRows = APP.cache.narcoticDepartmentStock.filter(r => r.departmentId === dept.id);
      const totalFixed = stockRows.reduce((s,r)=>s+Number(r.fixedStockUnits||0),0);
      const totalAvail = stockRows.reduce((s,r)=>s+Number(r.availableStockUnits||0),0);
      const rxCount = APP.cache.narcoticPrescriptions.filter(r => r.departmentId === dept.id).length;
      return `<div class="drug-card narcotic-department-card" draggable="${APP.currentRole === "ADMIN"}" data-narcotic-dept-card="${esc(dept.id)}">
        <div class="drug-title">${esc(dept.departmentName || "")}</div>
        <div class="department-card-meta">
          <div class="subline">Fixed stock: ${Number(totalFixed)} units</div>
          <div class="subline">Available stock: ${Number(totalAvail)} units</div>
          <div class="subline">Registered prescriptions: ${Number(rxCount)}</div>
        </div>
      </div>`;
...
async function registerNarcoticPrescription() {
  const payload = {
    ...
    remainingDose: q("narcoticRemainingDose").value.trim(),
...
function setNarcoticTab(tab) {
  APP.narcoticTab = tab;
  document.querySelectorAll(".narcotic-tab-btn").forEach(btn => btn.classList.toggle("active", btn.dataset.narcotictab === tab));
  document.querySelectorAll(".narcotic-tab-content").forEach(el => el.classList.toggle("hidden", el.id !== `narcoticTab-${tab}`));
}
