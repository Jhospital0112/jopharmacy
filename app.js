...
async function receiveShipment() {
  ...
  try {
    ...
    for (const item of batchRows) {
      ...
      operations.push({ type: "set", table: "transactions", id: crypto.randomUUID(), data: { type: "Receive Shipment", drugId, tradeName: drug.tradeName || "", pharmacy, qtyBoxes: Number(boxes || 0), qtyUnits: Number(units || 0), invoiceDate, invoiceNumber, receivedBy: APP.currentUser?.displayName || APP.currentRole || "", performedBy: APP.currentRole || "", note: "Shipment received", dateTime: jordanNowIso() } });
...
function renderNarcoticTransactionsTable() {
  ...
  q("narcoticTransactionsTbody").innerHTML = rows.map(row => `
    <tr>
      ...
      <td>${Number(row.quantitySent || row.quantityReceived || 0)}</td>
      <td>${esc(row.invoiceDate || "-")}</td>
      <td>${esc(row.invoiceNumber || "-")}</td>
      <td>${esc(row.performedBy || "-")}</td>
  ...
  `).join("") || `<tr><td colspan="10" class="empty-state">No narcotic transactions found for the selected criteria.</td></tr>`;
}
