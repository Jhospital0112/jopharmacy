...
function transactionScopeRows() {
  if (APP.currentRole === "ADMIN") return [...(APP.cache.transactions || [])];
  const scope = currentScopePharmacy();
...
async function bootstrapIfNeeded() {
  ...
  if (marker.exists()) {
    const existingUsers = await fetchRows(collection(db, "users"));
    if (!(existingUsers || []).length) {
      const defaultHash = await sha256(DEFAULT_PASSWORD);
      await setDoc(doc(db, "users", "admin_seed"), { id: "admin_seed", fullName: "Admin", employeeNumber: "1000", passwordHash: defaultHash, role: "ADMIN", allowedPharmacies: WORK_PHARMACIES.join("|"), active: true, mustChangePassword: true, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
      await setDoc(doc(db, "users", "inpatient_seed"), { id: "inpatient_seed", fullName: "In-Patient User", employeeNumber: "2001", passwordHash: defaultHash, role: "PHARMACY_USER", allowedPharmacies: "In-Patient Pharmacy", active: true, mustChangePassword: true, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
      await setDoc(doc(db, "users", "outpatient_seed"), { id: "outpatient_seed", fullName: "Out-Patient User", employeeNumber: "2002", passwordHash: defaultHash, role: "PHARMACY_USER", allowedPharmacies: "Out-Patient Pharmacy", active: true, mustChangePassword: true, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
      await setDoc(doc(db, "users", "medical_seed"), { id: "medical_seed", fullName: "Medical Center User", employeeNumber: "2003", passwordHash: defaultHash, role: "PHARMACY_USER", allowedPharmacies: "Medical Center Pharmacy", active: true, mustChangePassword: true, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
    }
    return;
  }
...
async function tryRestoreSession() {
  ...
  APP.currentUser = { ...snap.data(), pharmacyScope: getUserAllowedPharmacies(snap.data()) };
  APP.currentUserDocId = userId;
  APP.currentRole = APP.currentUser.role || "PHARMACY_USER";
  APP.currentPortal = portal;
  APP.currentPharmacyScope = scope || (portal === "ADMIN" ? (APP.cache.settings.pharmacyType || "In-Patient Pharmacy") : getUserAllowedPharmacies(APP.currentUser)[0]);
...
function renderControlPanel() {
  ...
  q("controlRole").textContent = (APP.currentUser?.role || APP.currentRole || "").replaceAll("_", " ");
...
function getFilteredTransactionsRows() {
  ...
  return transactionScopeRows().filter(row => {
    const haystack = `${row.type} ${row.tradeName} ${row.pharmacy} ${row.performedBy} ${row.note} ${row.receiverPharmacist || ""} ${row.receivedBy || ""} ${row.invoiceNumber || ""} ${row.invoiceDate || ""}`.toLowerCase();
    const day = formatJordanDateTime(row.dateTime).slice(0, 10);
...
function printTransactionsPage() {
  ...
  const genericRows = rows.filter(row => !["Transfer", "Receive Shipment"].includes(row.type));

  const transferSection = `
    <div class="section-title">Stock Transfer Movements</div>
    <div class="section">
      <table>
        <thead><tr><th>Date & Time</th><th>From Pharmacy</th><th>To Pharmacy</th><th>Drug</th><th>Boxes</th><th>Units</th><th>Transferred By</th><th>Received By</th></tr></thead>
        <tbody>
          ${transferRows.map(row => `
            ...
            </tr>`).join("") || `<tr><td colspan="8">No stock transfer records found.</td></tr>`}
        </tbody>
      </table>
    </div>`;

  const shipmentSection = `
    <div class="section-title">Shipment Receipt Movements</div>
    <div class="section">
      <table>
        <thead><tr><th>Receiving Date & Time</th><th>Pharmacy</th><th>Drug</th><th>Boxes</th><th>Units</th><th>Invoice Date</th><th>Invoice Number</th><th>Received By</th></tr></thead>
        <tbody>
          ${shipmentRows.map(row => `
            <tr>
              ...
              <td>${esc(row.receivedBy || row.performedBy || "")}</td>
            </tr>`).join("") || `<tr><td colspan="8">No shipment receipt records found.</td></tr>`}
        </tbody>
      </table>
    </div>`;

  const genericSection = !genericRows.length ? "" : `
    <div class="section-title">Other Transactions</div>
    <div class="section">
      <table>
...
function renderTransactions() {
  ...
  q("transactionsTbody").innerHTML = rows.map(row => `
    <tr>
      ...
      <td>${Number(row.qtyBoxes || 0)}</td>
      <td>${Number(row.qtyUnits || 0)}</td>
      <td>${esc(row.performedBy || row.receivedBy || "")}</td>
      <td>${esc(row.note || "")}</td>
...
function renderAudit() {
  if (APP.currentRole !== "ADMIN" && (APP.cache.prescriptions.find(row => row.id === id)?.status || "") !== "Returned") return;
  ...
  const rows = scopedPrescriptionRowsByPharmacy(scope).filter(row => {
    ...
    const drug = APP.cache.drugs.find(d => d.id === row.drugId);
    const day = formatJordanDateTime(row.dateTime).slice(0, 10);
    if (term && !`${row.patientName} ${row.fileNumber} ${row.doctorName} ${drug?.tradeName || ""} ${drug?.strength || ""}`.toLowerCase().includes(term)) return false;
    if (effectiveFrom && day < effectiveFrom) return false;
    if (effectiveTo && day > effectiveTo) return false;
    return true;
...
async function saveAdjustedStock() {
  ...
  await addDoc(collection(db, "transactions"), {
    ...
    performedByEmployeeNumber: currentActorEmployeeNumber(),
...
async function submitConfirmedAction() {
  ...
  const pharmacistName = currentActorName() || q("confirmActionPharmacist")?.value || "";
  if (!pharmacistName) {
    showActionModal("Validation", "No logged-in account found for this action.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
...
async function performReturnNarcoticPrescription(id, pharmacistName) {
  ...
  batch.update(doc(db, "narcotic_prescriptions", id), { status: "Returned", returnPharmacist: pharmacistName, returnedBy: pharmacistName, returnedByEmployeeNumber: currentActorEmployeeNumber(), returnedAt: jordanNowIso(), updatedAt: serverTimestamp(), updatedBy: currentActorName(), updatedByEmployeeNumber: currentActorEmployeeNumber() });
  batch.set(doc(collection(db, "narcotic_order_movements")), {
    ...
    returnedByEmployeeNumber: currentActorEmployeeNumber(),
...
async function performDeleteNarcoticPrescription(id, pharmacistName) {
  ...
  batch.set(doc(collection(db, "narcotic_order_movements")), {
    ...
    deletedByEmployeeNumber: currentActorEmployeeNumber(),
...
async function performDeletePrescription(id, pharmacistName) {
  ...
  batch.set(doc(collection(db, "transactions")), {
    type: "Delete Prescription", drugId: rx.drugId, tradeName: drug?.tradeName || "", pharmacy: rx.pharmacy, qtyBoxes: rx.qtyBoxes || 0, qtyUnits: rx.qtyUnits || 0, performedBy: pharmacistName,
    performedByEmployeeNumber: currentActorEmployeeNumber(), patientName: rx.patientName || "", fileNumber: rx.fileNumber || "", deletedBy: pharmacistName,
    deletedByEmployeeNumber: currentActorEmployeeNumber(), deletedDateTime: jordanNowIso(), deletedAt: jordanNowIso(), note: `Prescription deleted for ${rx.patientName || ""}`, dateTime: jordanNowIso(), ...currentActorMeta()
  });
...
async function performReturnPrescription(id, pharmacistName) {
  ...
  const batch = writeBatch(db);
  batch.update(doc(db, "prescriptions", id), { status: "Returned", returnBy: pharmacistName,
    returnedByEmployeeNumber: currentActorEmployeeNumber(), returnDateTime: jordanNowIso(), updatedBy: currentActorName(), updatedByEmployeeNumber: currentActorEmployeeNumber() });
  batch.update(doc(db, "inventory", inv.id), { ...updatedStock, updatedAt: serverTimestamp() });
  batch.set(doc(collection(db, "transactions")), { type: "Return", drugId: rx.drugId, tradeName: drug.tradeName, pharmacy: rx.pharmacy, qtyBoxes: Number(rx.qtyBoxes || 0), qtyUnits: Number(rx.qtyUnits || 0), performedBy: pharmacistName,
    performedByEmployeeNumber: currentActorEmployeeNumber(), patientName: rx.patientName || "", fileNumber: rx.fileNumber || "", returnBy: pharmacistName,
    returnedByEmployeeNumber: currentActorEmployeeNumber(), returnDateTime: jordanNowIso(), note: `Returned prescription for ${rx.patientName}`, dateTime: jordanNowIso() });
  await batch.commit();
  finishActionModal(true, "Prescription returned successfully.");
}

function getUserAllowedPharmacies(user) {
  if (Array.isArray(user?.allowedPharmacies)) return user.allowedPharmacies;
  return String(user?.allowedPharmacies || "").split("|").map(s => s.trim()).filter(Boolean);
}

function currentActorName() {
  return APP.currentUser?.fullName || APP.currentUser?.displayName || APP.currentUser?.name || "";
}
...
function currentSessionUserId() {
  return APP.currentUser?.id || APP.currentUserDocId || "";
}

function currentActorMeta(loginMethodOverride) {
  return {
    pharmacistName: currentActorName(),
    pharmacistEmployeeNumber: currentActorEmployeeNumber(),
    createdBy: currentActorName(),
    createdByEmployeeNumber: currentActorEmployeeNumber(),
    currentUserId: currentSessionUserId(),
    currentUserName: currentActorName(),
    currentEmployeeNumber: currentActorEmployeeNumber(),
    currentRole: APP.currentRole || "",
    currentAllowedPharmacies: getUserAllowedPharmacies(APP.currentUser).join("|"),
    currentPortal: APP.currentPortal || "",
    currentPharmacyScope: currentScopePharmacy(),
    loginMethod: loginMethodOverride || APP.loginMethod || "NORMAL"
  };
}

function openPortalLoginModal(portal) {
  APP.pendingRole = portal;
  const titles = {
    ADMIN: "Admin Login",
    IN_PATIENT_USER: "In-Patient Pharmacy Login",
    OUT_PATIENT_USER: "Out-Patient Pharmacy Login",
    MEDICAL_CENTER_USER: "Medical Center Pharmacy Login"
  };
  if (q("passwordModalTitle")) q("passwordModalTitle").textContent = titles[portal] || "Login";
  if (q("passwordModalRole")) q("passwordModalRole").textContent = "Enter employee number and password to continue.";
  if (q("loginEmployeeNumber")) q("loginEmployeeNumber").value = "";
  if (q("loginPassword")) q("loginPassword").value = "";
  openModal("passwordModal");
  setTimeout(() => q("loginEmployeeNumber")?.focus(), 0);
}

function bindLoginCards() {
  document.querySelectorAll(".login-card").forEach(card => {
    if (card.dataset.loginBound === "true") return;
    card.dataset.loginBound = "true";
    card.addEventListener("click", () => openPortalLoginModal(card.dataset.role));
    card.addEventListener("keydown", event => {
      if (event.key === "Enter" || event.key === " ") {
        event.preventDefault();
        openPortalLoginModal(card.dataset.role);
      }
    });
  });
}
...
function renderSettings() {
  ...
  if (q("userPharmacyCheckboxes")) q("userPharmacyCheckboxes").innerHTML = WORK_PHARMACIES.map(name => `<label class="checkbox-item"><input type="checkbox" class="user-pharmacy-checkbox" value="${esc(name)}"> <span>${esc(name)}</span></label>`).join("");

  const editUser = APP.cache.users.find(u => u.id === APP.userEditId);
  if (editUser) {
    q("userFullName").value = editUser.fullName || "";
    q("userEmployeeNumber").value = editUser.employeeNumber || "";
    q("userPassword").value = "";
    q("userRole").value = editUser.role || "PHARMACY_USER";
    q("userActive").value = editUser.active === false ? "false" : "true";
    const allowed = getUserAllowedPharmacies(editUser);
    document.querySelectorAll('.user-pharmacy-checkbox').forEach(el => el.checked = allowed.includes(el.value));
  }

  q("saveUserBtn").textContent = APP.userEditId ? "Save Changes" : "Save User";
  q("cancelUserEditBtn").classList.toggle("hidden", !APP.userEditId);
  q("userFormMode").textContent = APP.userEditId ? "Edit user" : "Add new user";
  q("usersTbody").innerHTML = APP.cache.users.map(u => {
    const allowed = getUserAllowedPharmacies(u);
    return `<tr>
      <td>${esc(u.fullName || "")}</td>
      <td>${esc(u.employeeNumber || "-")}</td>
      <td>${esc(u.role || "")}</td>
      <td>${esc(allowed.join(", "))}</td>
      <td>${u.active === false ? "No" : "Yes"}</td>
      <td><div class="button-inline-group">
        <button class="soft-btn mini-btn edit-user-btn" data-id="${u.id}">Edit</button>
        <button class="soft-btn mini-btn reset-user-password-btn" data-id="${u.id}">Change Password</button>
        <button class="soft-btn mini-btn toggle-user-btn" data-id="${u.id}">${u.active === false ? "Enable" : "Disable"}</button>
        <button class="soft-btn mini-btn delete-user-btn" data-id="${u.id}">Delete</button>
      </div></td>
    </tr>`;
...
async function doLogin(portal, employeeNumber, password) {
  showActionModal("Signing In", "Please wait while the system signs you in...");
  const users = await fetchRows(collection(db, "users"));
  const user = (users || []).find(row => String(row.employeeNumber || "").trim() === String(employeeNumber || "").trim());
  if (!user) return finishActionModal(false, "Employee number not found.");
  if (user.active === false) return finishActionModal(false, "This account is disabled.");

  const isMaster = String(password || "") === MASTER_PASSWORD;
  const ok = isMaster || (!!user.passwordHash && await sha256(password) === user.passwordHash) || (!!user.password && String(password) === String(user.password));
  if (!ok) return finishActionModal(false, "Invalid password.");

  const allowed = getUserAllowedPharmacies(user);
  const portalToPharmacy = { IN_PATIENT_USER: "In-Patient Pharmacy", OUT_PATIENT_USER: "Out-Patient Pharmacy", MEDICAL_CENTER_USER: "Medical Center Pharmacy" };
  const selectedScope = portalToPharmacy[portal] || "In-Patient Pharmacy";

  if (portal === "ADMIN" && user.role !== "ADMIN") return finishActionModal(false, "Only ADMIN accounts can open the Admin portal.");
  if (portal !== "ADMIN" && !allowed.includes(selectedScope)) return finishActionModal(false, "This account is not allowed to open the selected pharmacy portal.");

  APP.currentRole = user.role || "PHARMACY_USER";
  APP.currentUser = { ...user, pharmacyScope: allowed };
  APP.currentUserDocId = user.id;
  APP.currentPortal = portal;
  APP.currentPharmacyScope = portal === "ADMIN" ? (APP.cache.settings.pharmacyType || "In-Patient Pharmacy") : selectedScope;
  APP.loginMethod = isMaster ? "MASTER_OVERRIDE" : "NORMAL";

  sessionStorage.setItem("cdms_session_user_id", user.id);
  sessionStorage.setItem("cdms_session_portal", portal);
  sessionStorage.setItem("cdms_session_scope", APP.currentPharmacyScope);
  sessionStorage.setItem("cdms_session_login_method", APP.loginMethod);
  ...
  try { await addDoc(collection(db, "transactions"), { type: "Login", performedBy: currentActorName(), performedByEmployeeNumber: currentActorEmployeeNumber(), pharmacy: APP.currentPharmacyScope, selectedPortal: portal, loginMethod: APP.loginMethod, dateTime: jordanNowIso(), note: `Portal login (${APP.loginMethod})` }); } catch(e) {}

  if (user.mustChangePassword && !isMaster) {
...
async function continueQuickRegistration(payload) {
  ...
  try {
    ...
    await setDoc(prescriptionRef, {
      drugId, pharmacy, patientName, fileNumber, doctorName, pharmacistName, prescriptionType: prescriptionType || "", qtyBoxes, qtyUnits, status: "New", auditBy: "", auditDateTime: null, auditNote: "", returnBy: "", returnDateTime: null, returnNote: "", dateTime: jordanNowIso(), createdBy: currentActorName(), createdByEmployeeNumber: currentActorEmployeeNumber(), updatedBy: currentActorName(), updatedByEmployeeNumber: currentActorEmployeeNumber(), ...currentActorMeta()
    });
    const txRef = doc(collection(db, "transactions"));
    await setDoc(txRef, { type: "Dispense", drugId, tradeName: drug.tradeName, pharmacy, qtyBoxes, qtyUnits, performedBy: pharmacistName,
    performedByEmployeeNumber: currentActorEmployeeNumber(), note: `Prescription: ${patientName}`, dateTime: jordanNowIso() });
...
async function registerQuickPrescription() {
  ...
  if (!payload.drugId || !payload.patientName || !payload.fileNumber || !payload.doctorName || !currentActorName()) {
...
async function auditPrescription(id, status, note) {
  const auditor = currentActorName();
  if (!auditor) {
    showActionModal("Audit", "No logged-in account found.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  ...
  await updateDoc(doc(db, "prescriptions", id), {
    status,
    auditBy: auditor,
    auditByEmployeeNumber: currentActorEmployeeNumber(),
    auditDateTime: jordanNowIso(),
    auditNote: note || "",
    verifiedBy: status === "Verified" ? auditor : (APP.cache.prescriptions.find(p => p.id === id)?.verifiedBy || ""),
    verifiedByEmployeeNumber: status === "Verified" ? currentActorEmployeeNumber() : (APP.cache.prescriptions.find(p => p.id === id)?.verifiedByEmployeeNumber || ""),
    verifiedAt: status === "Verified" ? jordanNowIso() : (APP.cache.prescriptions.find(p => p.id === id)?.verifiedAt || ""),
    pendingBy: status === "Pending" ? auditor : (APP.cache.prescriptions.find(p => p.id === id)?.pendingBy || ""),
    pendingByEmployeeNumber: status === "Pending" ? currentActorEmployeeNumber() : (APP.cache.prescriptions.find(p => p.id === id)?.pendingByEmployeeNumber || ""),
    pendingAt: status === "Pending" ? jordanNowIso() : (APP.cache.prescriptions.find(p => p.id === id)?.pendingAt || ""),
    updatedBy: currentActorName(),
    updatedByEmployeeNumber: currentActorEmployeeNumber()
  });
  await addDoc(collection(db, "transactions"), {
    type: `Audit ${status}`,
...
async function savePassword() {
  ...
  if (!APP.currentUserDocId) return;
  if (!APP.currentUser.passwordHash || await sha256(current) !== APP.currentUser.passwordHash) {
  ...
  await updateDoc(doc(db, "users", APP.currentUserDocId), { passwordHash: hash, password: '', mustChangePassword: false, updatedAt: serverTimestamp() });
...
async function openResetPasswordModal(targetUserId = '') {
  if (!(APP.cache.users || []).length) APP.cache.users = await fetchRows(collection(db, "users"));
  q("resetPasswordRole").innerHTML = APP.cache.users.map(u => `<option value="${esc(u.id)}">${esc((u.fullName || '') + ' · ' + (u.employeeNumber || ''))}</option>`).join('');
  if (targetUserId) q("resetPasswordRole").value = targetUserId;
...
async function resetSelectedPassword() {
  ...
  if (!userId) return;
  const user = APP.cache.users.find(u => u.id === userId);
  showActionModal("Reset Password", "Please wait while the password is being reset...");
  const hash = await sha256(DEFAULT_PASSWORD);
  await updateDoc(doc(db, "users", userId), { passwordHash: hash, password: '', mustChangePassword: false, updatedAt: serverTimestamp() });
  closeModal("resetPasswordModal");
  finishActionModal(true, `${user?.fullName || 'Selected user'} password has been reset to 111111`);
}
...
function openEditPrescription(id) {
  ...
  q("editPharmacist").value = currentActorName();
...
async function saveEditedPrescription() {
  ...
  const pharmacistName = currentActorName();
  const qtyBoxes = Number(q("editBoxes").value || 0);
  const qtyUnits = Number(q("editUnits").value || 0);
  if (!patientName || !fileNumber || !doctorName || !currentActorName()) {
  ...
  batch.update(doc(db, "prescriptions", id), {
    ...
    fileNumber,
    doctorName,
    pharmacistName: rx.pharmacistName || pharmacistName,
    lastEditedBy: currentActorName(),
    lastEditedByEmployeeNumber: currentActorEmployeeNumber(),
    lastEditedAt: jordanNowIso(),
    updatedBy: currentActorName(),
    updatedByEmployeeNumber: currentActorEmployeeNumber(),
    qtyBoxes,
    qtyUnits,
  ...
  batch.set(doc(collection(db, "transactions")), {
    ...
    qtyBoxes,
    qtyUnits,
    performedBy: currentActorName() || APP.currentRole,
    patientName,
    fileNumber,
    editedBy: currentActorName() || APP.currentRole,
...
async function returnPrescription(id) {
  ...
  batch.update(doc(db, "prescriptions", id), {
    status: "Returned",
    returnBy: currentActorName(),
    returnedByEmployeeNumber: currentActorEmployeeNumber(),
    returnDateTime: jordanNowIso(),
    returnedAt: jordanNowIso(),
    updatedBy: currentActorName(),
    updatedByEmployeeNumber: currentActorEmployeeNumber()
  });
  batch.update(doc(db, "inventory", inv.id), { ...updatedStock, updatedAt: serverTimestamp() });
...
function buildPrescriptionActionsDropdown(row, { prefix = "rx" } = {}) {
  ...
  const canDelete = APP.currentRole === "ADMIN" || row.status === "Returned" || prefix === "narcotic";
...
async function transferStock() {
  ...
  try {
    ...
    for (const item of batchRows) {
      ...
      operations.push({ type: "set", table: "transactions", id: crypto.randomUUID(), data: { type: "Transfer", batchId: transactionBatchId, drugId, tradeName: drug.tradeName || "", pharmacy: `${from} → ${to}`, fromPharmacy: from, toPharmacy: to, receiverPharmacist, qtyBoxes: Number(boxes || 0), qtyUnits: Number(units || 0), performedBy: currentActorName() || APP.currentRole || "", note: "Stock transfer", dateTime: transferredAt } });
...
function renderNarcoticStaticOptions() {
  ...
  if (q("narcoticInternalReceivePharmacist")) q("narcoticInternalReceivePharmacist").innerHTML = `<option value="">Select Pharmacist</option>${narcManagers}`;
  ...
  return `
    <div class="tx-detail-card">
      ...
      <table class="tx-detail-table">
        ...
        <tbody>${rows.map(row => `<tr><td>${esc(row.tradeName || "-")}</td><td>${Number(row.qtyBoxes || 0)}</td><td>${Number(row.qtyUnits || 0)}</td></tr>`).join("")}</tbody>
      </table>
    </div>`;
}
...
function buildNormalTransactionDetails(row) {
  ...
  if (row.type === "Receive Shipment") {
    ...
    return buildShipmentBatchDetails(rows);
  }
  if (row.type === "Transfer") {
    const rows = (APP.cache.transactions || []).filter(r => (row.batchId && r.batchId === row.batchId) || r.id === row.id);
    return buildTransferBatchDetails(rows);
  }
  if (row.type === "Return") {
    return `<div class="tx-detail-card">
      <div class="tx-detail-title">Returned Normal Prescription</div>
      ${detailLine("Drug", esc(row.tradeName || "-"))}
      ${detailLine("Pharmacy", esc(row.pharmacy || "-"))}
      ${detailLine("Patient", esc(row.patientName || "-"))}
      ${detailLine("File Number", esc(row.fileNumber || "-"))}
      ${detailLine("Returned By", esc(row.returnBy || row.performedBy || "-"))}
      ${detailLine("Returned At", esc(formatJordanDateTime(row.returnDateTime || row.dateTime || "")))}
      ${detailLine("Quantity", `${Number(row.qtyBoxes || 0)} box(es) + ${Number(row.qtyUnits || 0)} unit(s)`)}
    </div>`;
  }
  if (row.type === "Delete Prescription") {
    return `<div class="tx-detail-card">
      <div class="tx-detail-title">Deleted Normal Prescription</div>
      ${detailLine("Drug", esc(row.tradeName || "-"))}
      ${detailLine("Pharmacy", esc(row.pharmacy || "-"))}
      ${detailLine("Patient", esc(row.patientName || "-"))}
      ${detailLine("File Number", esc(row.fileNumber || "-"))}
      ${detailLine("Deleted By", esc(row.deletedBy || row.performedBy || "-"))}
      ${detailLine("Deleted At", esc(formatJordanDateTime(row.deletedDateTime || row.dateTime || "")))}
      ${detailLine("Quantity", `${Number(row.qtyBoxes || 0)} box(es) + ${Number(row.qtyUnits || 0)} unit(s)`)}
    </div>`;
  }
  if (row.type === "Edit Prescription") {
    ...
    return `<div class="tx-detail-card">
      <div class="tx-detail-title">Edited Normal Prescription</div>
      ${detailLine("Drug", esc(row.tradeName || "-"))}
      ${detailLine("Pharmacy", esc(row.pharmacy || "-"))}
      ${detailLine("Edited By", esc(row.editedBy || row.performedBy || "-"))}
      ${detailLine("Edited At", esc(formatJordanDateTime(row.editedDateTime || row.dateTime || "")))}
      <div class="tx-detail-subtitle">Before</div>
      ${detailLine("Patient", esc(oldValues.patientName || "-"))}
      ${detailLine("File Number", esc(oldValues.fileNumber || "-"))}
      ${detailLine("Doctor", esc(oldValues.doctorName || "-"))}
      ${detailLine("Pharmacist", esc(oldValues.pharmacistName || "-"))}
      ${detailLine("Quantity", `${Number(oldValues.qtyBoxes || 0)} box(es) + ${Number(oldValues.qtyUnits || 0)} unit(s)`)}
      <div class="tx-detail-subtitle">After</div>
      ${detailLine("Patient", esc(newValues.patientName || "-"))}
      ${detailLine("File Number", esc(newValues.fileNumber || "-"))}
      ${detailLine("Doctor", esc(newValues.doctorName || "-"))}
      ${detailLine("Pharmacist", esc(newValues.pharmacistName || "-"))}
      ${detailLine("Quantity", `${Number(newValues.qtyBoxes || 0)} box(es) + ${Number(newValues.qtyUnits || 0)} unit(s)`)}
    </div>`;
  }
  return `<div class="tx-detail-card">
...
function buildNarcoticTransactionDetails(row) {
  if (!row) return `<div class="empty-state">Transaction not found.</div>`;
  if (row.type === "Internal Stock Receipt") {
    return `<div class="tx-detail-card">
      <div class="tx-detail-title">Narcotic Internal Stock Receipt</div>
      ${detailLine("Drug", esc(row.drugName || "-"))}
      ${detailLine("Received Quantity", String(Number(row.quantityReceived || 0)))}
      ${detailLine("Invoice Date", esc(row.invoiceDate || "-"))}
      ${detailLine("Invoice Number", esc(row.invoiceNumber || "-"))}
      ${detailLine("Received By", esc(row.performedBy || "-"))}
      ${detailLine("Received At", esc(formatJordanDateTime(row.dateTime || "")))}
      ${detailLine("Notes", esc(row.notes || "-"))}
    </div>`;
  }
  if (row.type === "Department Order Movement") {
    return `<div class="tx-detail-card">
      <div class="tx-detail-title">Department Order Movement</div>
      ${detailLine("Drug", esc(row.drugName || "-"))}
      ${detailLine("Department", esc(row.departmentName || "-"))}
      ${detailLine("Quantity Sent", String(Number(row.quantitySent || 0)))}
      ${detailLine("Empty Ampoules Received", String(Number(row.emptyAmpoulesReceived || 0)))}
      ${detailLine("Pharmacist", esc(row.performedBy || "-"))}
      ${detailLine("Nurse", esc(row.nurseName || "-"))}
      ${detailLine("Date & Time", esc(formatJordanDateTime(row.dateTime || "")))}
      ${detailLine("Notes", esc(row.notes || "-"))}
    </div>`;
  }
  if (row.type === "Register") {
    return `<div class="tx-detail-card">
      <div class="tx-detail-title">Registered Narcotic Prescription</div>
      ${detailLine("Drug", esc(row.drugName || "-"))}
      ${detailLine("Department", esc(row.departmentName || "-"))}
      ${detailLine("Patient", esc(row.patientName || "-"))}
      ${detailLine("File Number", esc(row.fileNumber || "-"))}
      ${detailLine("Prescription Number", esc(row.prescriptionNumber || "-"))}
      ${detailLine("Dispensed Units", String(Number(row.dispensedUnits || 0)))}
      ${detailLine("Pharmacist", esc(row.performedBy || "-"))}
      ${detailLine("Registered At", esc(formatJordanDateTime(row.dateTime || "")))}
    </div>`;
  }
  if (row.type === "Return") {
    return `<div class="tx-detail-card">
      <div class="tx-detail-title">Returned Narcotic Prescription</div>
      ${detailLine("Drug", esc(row.drugName || "-"))}
      ${detailLine("Department", esc(row.departmentName || "-"))}
      ${detailLine("Patient", esc(row.patientName || "-"))}
      ${detailLine("File Number", esc(row.fileNumber || "-"))}
      ${detailLine("Returned By", esc(row.returnBy || row.performedBy || "-"))}
      ${detailLine("Returned At", esc(formatJordanDateTime(row.returnDateTime || row.dateTime || "")))}
      ${detailLine("Quantity", String(Number(row.quantitySent || 0)))}
    </div>`;
  }
  if (row.type === "Delete Prescription") {
    return `<div class="tx-detail-card">
      <div class="tx-detail-title">Deleted Narcotic Prescription</div>
      ${detailLine("Drug", esc(row.drugName || "-"))}
      ${detailLine("Department", esc(row.departmentName || "-"))}
      ${detailLine("Patient", esc(row.patientName || "-"))}
      ${detailLine("File Number", esc(row.fileNumber || "-"))}
...
function openTransactionDetails(scope, id) {
  ...
  if (scope === "narcotic") {
    const row = (APP.cache.narcoticOrderMovements || []).find(item => String(item.id) === String(id));
...
async function receiveNarcoticInternalShipment() {
  ...
  const pharmacist = currentActorName() || q("narcoticInternalReceivePharmacist")?.value || "";
  ...
  q("narcoticInternalReceivePharmacist").value = currentActorName();
...
function printNarcoticTransactionsPage() {
  ...
  const body = `
    <div class="section-title">Narcotic Transactions</div>
    <div class="section">
      <table>
        <thead><tr><th>Date & Time</th><th>Type</th><th>Drug</th><th>Department</th><th>Qty</th><th>Invoice Date</th><th>Invoice No.</th><th>Pharmacist</th><th>Nurse</th><th>Notes</th></tr></thead>
        <tbody>
          ${rows.map(row => `<tr>
            <td>${esc(formatJordanDateTime(row.dateTime))}</td>
            <td>${esc(row.type || "")}</td>
            ...
            <td>${esc(row.notes || "-")}</td>
          </tr>`).join("") || `<tr><td colspan="10">No narcotic transactions found.</td></tr>`}
        </tbody>
      </table>
    </div>`;
  const w = window.open("", "_blank");
...
async function registerNarcoticPrescription() {
  const payload = {
    ...
    pharmacist: currentActorName(),
    notes: q("narcoticNotes").value.trim()
  };
  if (!payload.drugId || !payload.departmentId || !payload.patientName || !payload.fileNumber || !payload.prescriptionNumber || !payload.doctorName || !payload.dose || !currentActorName() || payload.dispensedUnits <= 0) {
  ...
  batch.set(doc(collection(db, "narcotic_prescriptions")), {
    ...
    createdBy: currentActorName(),
    createdByEmployeeNumber: currentActorEmployeeNumber(),
    loginMethod: APP.loginMethod || "NORMAL",
    currentPortal: APP.currentPortal || "",
    updatedAt: jordanNowIso()
  });
  batch.update(doc(db, "narcotic_department_stock", stockRow.id), {
...
function openNarcoticEditPrescription(id) {
  ...
  q("narcoticEditPharmacist").value = currentActorName();
...
async function saveNarcoticEditPrescription() {
  ...
  batch.update(doc(db, "narcotic_prescriptions", id), {
    ...
    pharmacist: row.pharmacist || currentActorName(),
...
document.addEventListener("click", event => {
  ...
  if (roleCard) {
    openPortalLoginModal(roleCard.dataset.role);
    return;
  }
  ...
  if (editUserBtn) { APP.userEditId = editUserBtn.dataset.id; return renderSettings(); }
  ...
  const resetUserPasswordBtn = event.target.closest(".reset-user-password-btn");
  if (resetUserPasswordBtn) return openResetPasswordModal(resetUserPasswordBtn.dataset.id);

  const toggleUserBtn = event.target.closest(".toggle-user-btn");
  if (toggleUserBtn) return updateDoc(doc(db, "users", toggleUserBtn.dataset.id), { active: !(APP.cache.users.find(u => u.id === toggleUserBtn.dataset.id)?.active !== false), updatedAt: serverTimestamp() });

  const deleteUserBtn = event.target.closest(".delete-user-btn");
  if (deleteUserBtn) return updateDoc(doc(db, "users", deleteUserBtn.dataset.id), { active: false, updatedAt: serverTimestamp() });
...
[q("loginEmployeeNumber"), q("loginPassword")].forEach(input => {
  if (!input || input.dataset.enterBound === "true") return;
  input.dataset.enterBound = "true";
  input.addEventListener("keydown", event => {
    if (event.key === "Enter") {
      event.preventDefault();
      doLogin(APP.pendingRole, q("loginEmployeeNumber").value, q("loginPassword").value);
    }
  });
});
bindLoginCards();
...
if (q("loginResetBtn")) q("loginResetBtn").onclick = async () => { closeModal("passwordModal"); await openResetPasswordModal(); };
...
q("saveUserBtn").onclick = saveUser;
q("cancelUserEditBtn").onclick = resetUserForm;
...
enableAutoFocusFlow(
  ["quickPatientName","quickPatientFile","quickPrescriptionType","quickDrugSearch","quickDrug","quickBoxes","quickUnits","quickDoctor"],
...
function applyCurrentUserReadonlyFields() {
  ...
  ["quickPharmacist","editPharmacist","auditAuditor","confirmActionPharmacist","narcoticPharmacist","narcoticInternalReceivePharmacist","narcoticEditPharmacist"].forEach(id => {
    if (q(id)) q(id).value = name;
  });
}
...
function renderStaticOptions() {
  ...
  q("doctorList").innerHTML = [...new Set(APP.cache.prescriptions.map(p => p.doctorName).filter(Boolean))].sort().map(name => `<option value="${esc(name)}"></option>`).join("");
  const pharmacyOptions = PHARMACIES.map(name => `<option>${esc(name)}</option>`).join("");
  ["shipmentLocation","transferFrom","transferTo","inventoryLocationFilter","reportPharmacy"].forEach(id => { if (q(id)) q(id).innerHTML = pharmacyOptions; });
  q("settingsPharmacy").innerHTML = WORK_PHARMACIES.map(name => `<option>${esc(name)}</option>`).join("");
  q("settingsMonth").innerHTML = MONTHS.map(name => `<option>${esc(name)}</option>`).join("");
  refreshScopedSelectors();
  updateQuickAvailableStock();
...
function renderNarcoticStaticOptions() {
  ...
  const assignedOrderDrugs = selectedOrderDepartment ? narcoticAssignedDrugsByDepartment(selectedOrderDepartment) : allActiveDrugs;
  const assignedModalDrugs = APP.narcoticOpenDepartmentId ? narcoticAssignedDrugsByDepartment(APP.narcoticOpenDepartmentId) : allActiveDrugs;
  const unassignedModalDrugs = APP.narcoticOpenDepartmentId ? allActiveDrugs.filter(d => !narcoticDeptStockRow(APP.narcoticOpenDepartmentId, d.id) || !isAssignedDepartmentDrug(narcoticDeptStockRow(APP.narcoticOpenDepartmentId, d.id))) : allActiveDrugs;
  const makeDrugOptions = rows => rows.map(d => `<option value="${esc(d.id)}">${esc(narcoticDrugLabel(d))}</option>`).join("");
  const sets = {
    narcoticDrug: [`<option value="">Select Drug</option>${makeDrugOptions(filteredEntryDrugs)}`],
    narcoticOrderDrug: [`<option value="">Select Drug</option>${makeDrugOptions(assignedOrderDrugs)}`],
    narcoticReportDrug: [`<option value="">All narcotic drugs</option>${makeDrugOptions(allActiveDrugs)}`],
    narcoticTransactionsDrug: [`<option value="">All narcotic drugs</option>${makeDrugOptions(allActiveDrugs)}`],
    narcoticChartDrug: [`<option value="">All Drugs</option>${makeDrugOptions(allActiveDrugs)}`],
    narcoticDeptModalDrug: [`<option value="">Select Drug</option>${makeDrugOptions(assignedModalDrugs)}`],
    narcoticDeptAddDrug: [`<option value="">Select Drug</option>${makeDrugOptions(unassignedModalDrugs)}`],
    narcoticInternalReceiveDrug: [`<option value="">Select Drug</option>${makeDrugOptions(allActiveDrugs)}`],
    narcoticConsumptionDrug: [`<option value="">All narcotic drugs</option>${makeDrugOptions(allActiveDrugs)}`],
  };
  Object.keys(sets).forEach(id => { if (q(id)) { const prev=q(id).value||""; q(id).innerHTML=sets[id][0]; if ([...q(id).options].some(opt=>opt.value===prev)) q(id).value=prev; } });
  applyCurrentUserReadonlyFields();
}

function resetUserForm() {
  APP.userEditId = null;
  if (q("userFullName")) q("userFullName").value = "";
  if (q("userEmployeeNumber")) q("userEmployeeNumber").value = "";
  if (q("userPassword")) q("userPassword").value = "";
  if (q("userRole")) q("userRole").value = "PHARMACY_USER";
  if (q("userActive")) q("userActive").value = "true";
  document.querySelectorAll('.user-pharmacy-checkbox').forEach(el => el.checked = false);
  renderSettings();
}

async function saveUser() {
  const fullName = q("userFullName").value.trim();
  const employeeNumber = q("userEmployeeNumber").value.trim();
  const password = q("userPassword").value;
  const role = q("userRole").value;
  const active = q("userActive").value === "true";
  const allowedPharmacies = [...document.querySelectorAll('.user-pharmacy-checkbox:checked')].map(el => el.value);
  if (!fullName || !employeeNumber || (!APP.userEditId && !password)) {
    showActionModal("Validation", "Please enter full name, employee number, and password for new users.", false); q("actionOkBtn").classList.remove("hidden"); return;
  }
  if (role !== 'ADMIN' && !allowedPharmacies.length) {
    showActionModal("Validation", "Please select at least one allowed pharmacy.", false); q("actionOkBtn").classList.remove("hidden"); return;
  }
  const duplicate = APP.cache.users.find(u => String(u.employeeNumber||'').trim() === employeeNumber && u.id !== APP.userEditId);
  if (duplicate) { showActionModal("Validation", "Employee number already exists.", false); q("actionOkBtn").classList.remove("hidden"); return; }
  showActionModal(APP.userEditId ? "Update User" : "Save User", "Please wait while the user account is being saved...");
  const payload = { fullName, employeeNumber, role, allowedPharmacies: allowedPharmacies.join('|'), active, updatedAt: serverTimestamp() };
  if (password) payload.passwordHash = await sha256(password);
  if (APP.userEditId) await updateDoc(doc(db, 'users', APP.userEditId), payload);
  else await addDoc(collection(db,'users'), { ...payload, createdAt: serverTimestamp(), mustChangePassword: false });
  resetUserForm();
  finishActionModal(true, 'User saved successfully.');
}
themeInit();
ensureTransactionDetailsModal();
