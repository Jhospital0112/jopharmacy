...
async function registerNarcoticPrescription() {
  ...
  if (!stockRow || Number(stockRow.availableStockUnits || 0) <= 0) {
  ...
  batch.update(doc(db, "narcotic_department_stock", stockRow.id), {
    availableStockUnits: Math.max(0, Number(stockRow.availableStockUnits || 0) - 1),
    updatedAt: serverTimestamp()
  });
...
async function submitNarcoticOrders() {
  ...
  for (const row of APP.narcoticOrdersBatchRows) {
    ...
    const nextAvailable = Math.min(fixed, Number(stockRow?.availableStockUnits || 0) + Number(row.quantitySent || 0));
    if (stockRow?.id) {
...
function renderNarcoticDepartmentModal() {
  ...
  let rows = APP.cache.narcoticPrescriptions.filter(r => r.departmentId === departmentId);
  if (drugId) rows = rows.filter(r => r.drugId === drugId);
  ...
  </tr>`).join("") || `<tr><td colspan="8" class="empty-state">No prescriptions found.</td></tr>`;
  const stockRows = APP.cache.narcoticDepartmentStock.filter(r => r.departmentId === departmentId && (!drugId || r.drugId === drugId));
  const fixed = stockRows.reduce((s,r)=>s+Number(r.fixedStockUnits||0),0);
  const avail = stockRows.reduce((s,r)=>s+Number(r.availableStockUnits||0),0);
  ...
  const selectedStock = stockRows[0] || null;
  if (q("narcoticDeptFixedStock")) q("narcoticDeptFixedStock").value = selectedStock ? Number(selectedStock.fixedStockUnits || 0) : 0;
  if (q("narcoticDeptAvailableStock")) q("narcoticDeptAvailableStock").value = selectedStock ? Number(selectedStock.availableStockUnits || 0) : 0;
}
...
q("searchNarcoticDeptModalBtn").onclick = renderNarcoticDepartmentModal;
...
q("printNarcoticDeptModalBtn").onclick = printNarcoticDispensingReport;
