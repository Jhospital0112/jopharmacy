...
function toSmartTitleCase(value) {
  const trimmed = String(value || "").replace(/\s+/g, ' ').trimStart();
...
function printNarcoticConsumptionReport() {
  ...
  rows.forEach(r => {
    ...
    const current = grouped.get(key) || { drugId: key, drugLabel: narcoticDrugLabel(narcoticDrugById(key)) || r.drugName || '-', totalUnits: 0, prescriptions: 0, departments: new Set() };
    current.totalUnits += Number(r.dispensedUnits || 0);
    current.prescriptions += 1;
    if (r.departmentName) current.departments.add(r.departmentName);
    grouped.set(key, current);
  });
  const summaryRows = [...grouped.values()].sort((a, b) => a.drugLabel.localeCompare(b.drugLabel));
  const internalRows = APP.cache.narcoticDrugs.map(drug => {
    const stock = narcoticInternalStockRow(drug.id);
    return { label: narcoticDrugLabel(drug), available: Number(stock?.availableStockUnits || 0), reorder: Number(stock?.reorderLevelUnits ?? drug.reorderLevelUnits ?? 0) };
  }).sort((a, b) => a.label.localeCompare(b.label));
  const totalUnits = summaryRows.reduce((sum, row) => sum + row.totalUnits, 0);
  const body = `
    ...
    <div class="section"><table><thead><tr><th>Drug</th><th>Total Dispensed Units</th><th>Registered Prescriptions</th><th>Departments</th></tr></thead>
    <tbody>${summaryRows.map(r => `<tr><td>${esc(r.drugLabel)}</td><td>${r.totalUnits}</td><td>${r.prescriptions}</td><td>${esc([...r.departments].join(', ') || '-')}</td></tr>`).join('') || `<tr><td colspan="4">No narcotic consumption rows found for the selected range.</td></tr>`}</tbody></table></div>
    <div class="section"><table><thead><tr><th>Total Dispensed Units</th><th>Total Prescriptions</th><th>Drugs Included</th></tr></thead><tbody><tr><td>${totalUnits}</td><td>${rows.length}</td><td>${summaryRows.length}</td></tr></tbody></table></div>
    <div class="section-title" style="margin-top:22px">Current In-Patient Internal Stock</div>
    <div class="section"><table><thead><tr><th>Drug</th><th>Available Stock</th><th>Reorder Level</th></tr></thead>
    <tbody>${internalRows.map(r => `<tr><td>${esc(r.label)}</td><td>${r.available}</td><td>${r.reorder}</td></tr>`).join('') || `<tr><td colspan="3">No internal stock rows found.</td></tr>`}</tbody></table></div>`;
