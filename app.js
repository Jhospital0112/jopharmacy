...
function openConfirmActionModal(type, id) {
  const isNarcotic = type === "narcotic_delete" || type === "narcotic_return";
  const rx = isNarcotic
    ? APP.cache.narcoticPrescriptions.find(row => row.id === id)
    : APP.cache.prescriptions.find(row => row.id === id);
  if (!rx) return;

  APP.confirmAction = { type, id };
  const needsPharmacist = !isNarcotic;
  if (q("confirmActionPharmacistField")) q("confirmActionPharmacistField").classList.toggle("hidden", !needsPharmacist);
  ...
  if (needsPharmacist) {
    ...
    q("confirmActionPharmacist").innerHTML = `<option value="">-</option>`;
  }

  const titleMap = {
    delete: "Delete Prescription",
    return: "Return Prescription",
    narcotic_delete: "Delete Prescription",
    narcotic_return: "Return Prescription"
  };
  const textMap = {
    delete: "Please confirm deleting this prescription.",
    return: "Please confirm returning this prescription.",
    narcotic_delete: "Please confirm deleting this prescription.",
    narcotic_return: "Please confirm returning this prescription."
  };

  q("confirmActionTitle").textContent = titleMap[type] || "Confirm Action";
  q("confirmActionText").textContent = textMap[type] || "Please confirm this action.";
...
async function submitConfirmedAction() {
  ...
  const needsPharmacist = !(type === "narcotic_delete" || type === "narcotic_return");
  const pharmacistName = q("confirmActionPharmacist").value;

  if (needsPharmacist && !pharmacistName) {
  ...
  if (type === "narcotic_return") return returnNarcoticPrescription(id);
  if (type === "narcotic_delete") return deleteNarcoticPrescription(id);
}
...
async function returnNarcoticPrescription(id) {
  const rx = APP.cache.narcoticPrescriptions.find(row => row.id === id);
  if (!rx || rx.status === "Returned") return;
  const stockRow = narcoticDeptStockRow(rx.departmentId, rx.drugId);
  ...
  if (stockRow?.id) {
    batch.update(doc(db, "narcotic_department_stock", stockRow.id), {
      availableStockUnits: Number(stockRow.availableStockUnits || 0) + Number(rx.dispensedUnits || 0),
      updatedAt: serverTimestamp()
    });
  }
  batch.set(doc(collection(db, "transactions")), {
    type: "Narcotic Return",
    drugId: rx.drugId,
    tradeName: rx.drugName || (narcoticDrugById(rx.drugId)?.tradeName || ""),
    pharmacy: rx.departmentName || (narcoticDeptById(rx.departmentId)?.departmentName || ""),
    qtyBoxes: 0,
    qtyUnits: Number(rx.dispensedUnits || 0),
    performedBy: APP.currentRole,
    note: `Returned narcotic prescription for ${rx.patientName}`,
    dateTime: jordanNowIso()
  });
  await batch.commit();
  finishActionModal(true, "Prescription returned successfully.");
...
function closeRxActionMenus() {
  ...
  if (portal && portal.dataset.mode === "rx") {
...
function toggleRxActionsMenu(menuId, btn = null) {
  const menu = q(menuId);
  const portal = q("auditMenuPortal");
  if (!menu || !portal || !btn) {
    if (!menu) return;
  ...
  const alreadyOpen = !portal.classList.contains("hidden") && portal.dataset.mode === "rx" && portal.dataset.sourceId === menuId;
  closeRxActionMenus();
  if (alreadyOpen) return;
  portal.innerHTML = menu.innerHTML;
  portal.dataset.sourceId = menuId;
  portal.dataset.mode = "rx";
...
function isAssignedDepartmentDrug(stockRow) {
  return !!stockRow && stockRow.active !== false && stockRow.assigned !== false;
}
...
function setNarcoticDetailTab(tab) {
  APP.narcoticDetailTab = tab === "manage" ? "manage" : "stock";
  const modal = q("narcoticDepartmentModal");
  const buttons = modal ? modal.querySelectorAll(".narcotic-inner-tab-btn") : document.querySelectorAll(".narcotic-inner-tab-btn");
  const panels = modal ? modal.querySelectorAll(".narcotic-detail-tab-content") : document.querySelectorAll(".narcotic-detail-tab-content");

  buttons.forEach(btn => {
    btn.classList.toggle("active", btn.dataset.narcoticdetailtab === APP.narcoticDetailTab);
  });
  panels.forEach(el => {
    const shouldShow = el.id === `narcoticDetailTab-${APP.narcoticDetailTab}`;
  ...
  if (APP.narcoticDetailTab === "manage") {
...
function renderNarcoticDepartmentManageTable() {
  ...
  q("narcoticDeptAssignedTbody").innerHTML = rows.map(row => {
    ...
    return `<tr>
      <td>${esc(narcoticDrugLabel(drug) || row.drugName || row.drugLabel || "")}</td>
...
async function addDrugToNarcoticDepartment() {
  ...
  if (!departmentId || !drugId) {
    showActionModal("Validation", "Please select a drug first.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  ...
  if (!dept || !drug) {
    showActionModal("Validation", "Department or drug data is missing.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }

  const docId = `${departmentId}__${drugId}`;
  ...
  if (existing?.id) {
    ...
    await setDoc(doc(db, "narcotic_department_stock", docId), {
      id: docId,
      departmentId,
      departmentName: dept.departmentName || "",
      drugId,
  ...
  if (q("narcoticDeptAddDrug")) q("narcoticDeptAddDrug").value = "";
...
function addNarcoticOrderRow() {
  ...
  APP.narcoticOrdersBatchRows.push({
    ...
    emptyAmpoulesReceived,
    quantitySent,
    performedBy,
    nurseName,
    notes: q("narcoticOrderNotes").value.trim()
  });
...
function printNarcoticMovementsReport() {
  ...
  const body = `
    ...
    <tbody>${rows.map(r=>`<tr><td>${esc(formatJordanDateTime(r.dateTime))}</td><td>${esc(r.drugName||r.drugLabel||"")}</td><td>${Number(r.emptyAmpoulesReceived||0)}</td><td>${Number(r.quantitySent||0)}</td><td>${esc(r.performedBy||"")}</td><td>${esc(r.nurseName||"")}</td></tr>`).join("") || `<tr><td colspan="6">No movement records found.</td></tr>`}</tbody></table></div>
    ...
    <div class="section" style="margin-top:42px;display:flex;justify-content:space-between;gap:40px"><div style="min-width:260px;text-align:center"><div style="font-weight:800">Pharmacist Signature</div><div style="margin-top:38px;border-top:1px solid #1b2b44"></div></div><div style="min-width:260px;text-align:center"><div style="font-weight:800">Nurse Signature</div><div style="margin-top:38px;border-top:1px solid #1b2b44"></div></div></div>`;
...
document.addEventListener("click", event => {
  ...
  if (rxMenuToggle) {
    toggleRxActionsMenu(rxMenuToggle.dataset.rxActionsToggle, rxMenuToggle);
    return;
  }
  ...
  if (latestReturnBtn) {
    closeRxActionMenus();
    if (latestReturnBtn.closest("#page-narcotic, #narcoticDepartmentModal, #narcoticRecentModal")) return openConfirmActionModal("narcotic_return", latestReturnBtn.dataset.id);
  ...
  if (latestEditBtn) {
    closeRxActionMenus();
    if (latestEditBtn.closest("#page-narcotic, #narcoticDepartmentModal, #narcoticRecentModal")) return openNarcoticEditPrescription(latestEditBtn.dataset.id);
  ...
  if (deleteRxBtn) {
    closeAuditMenuPortal();
    closeRxActionMenus();
    if (deleteRxBtn.closest("#page-narcotic, #narcoticDepartmentModal, #narcoticRecentModal")) return openConfirmActionModal("narcotic_delete", deleteRxBtn.dataset.id);
  ...
  if (narcoticDeleteBtn) {
    closeRxActionMenus();
    return openConfirmActionModal("narcotic_delete", narcoticDeleteBtn.dataset.id);
  }
...
if (q("narcoticDepartment")) q("narcoticDepartment").addEventListener("change", () => { renderNarcoticStaticOptions(); updateNarcoticAvailableStock(); });
...
document.querySelectorAll(".narcotic-inner-tab-btn").forEach(btn => {
  btn.addEventListener("click", (event) => {
    ...
    const tab = btn.dataset.narcoticdetailtab || "stock";
    setNarcoticDetailTab(tab);
  });
});
const narcoticDeptModal = q("narcoticDepartmentModal");
if (narcoticDeptModal) {
  narcoticDeptModal.addEventListener("click", (event) => {
    const btn = event.target.closest(".narcotic-inner-tab-btn");
    if (!btn) return;
    event.preventDefault();
    event.stopPropagation();
    const tab = btn.dataset.narcoticdetailtab || "stock";
