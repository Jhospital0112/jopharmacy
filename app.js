...
async function submitConfirmedAction() {
  ...
  if (action.scope === "narcotic") {
    ...
    if (action.type === "return") return performReturnNarcoticPrescription(action.id);
    if (action.type === "delete") return performDeleteNarcoticPrescription(action.id);
    return;
  }
...
async function performReturnNarcoticPrescription(id) {
  const row = narcoticPrescriptionById(id);
  if (!row) return;
  const stockRow = narcoticDeptStockRow(row.departmentId, row.drugId);
  const batch = writeBatch(db);
  batch.update(doc(db, "narcotic_prescriptions", id), { status: "Returned", updatedAt: serverTimestamp(), updatedBy: APP.currentRole });
  if (stockRow?.id) {
...
async function performDeleteNarcoticPrescription(id) {
  const row = narcoticPrescriptionById(id);
  if (!row) return;
  const stockRow = narcoticDeptStockRow(row.departmentId, row.drugId);
  const batch = writeBatch(db);
...
function openNarcoticConfirmActionModal(type, id) {
  ...
  if (q("confirmActionPharmacist")) q("confirmActionPharmacist").value = "";
  if (q("confirmActionPharmacist").parentElement) q("confirmActionPharmacist").parentElement.classList.add("hidden");
  if (q("confirmActionPharmacist").parentElement) q("confirmActionPharmacist").parentElement.classList.remove("hidden");
  openModal("confirmActionModal");
}
...
function renderNarcoticDepartmentFullStockSummary() {
  ...
  q("narcoticDeptStockSummaryAll").innerHTML = `
    <div class="narcotic-stock-chip"><span>Assigned Drugs</span><strong>${rows.length}</strong></div>
    <div class="narcotic-stock-chip"><span>Total Fixed Stock</span><strong>${totalFixed}</strong></div>
    <div class="narcotic-stock-chip"><span>Total Available Stock</span><strong>${totalAvail}</strong></div>
    <div class="narcotic-stock-chip"><span>Department</span><strong>${esc(narcoticDeptById(departmentId)?.departmentName || "-")}</strong></div>`;
}
...
function printNarcoticDispensingReport() {
  ...
  const body = `
    ...
    <div class="section" style="margin-top:42px;display:flex;justify-content:flex-end"><div style="min-width:280px;text-align:center"><div style="font-weight:800">Controlled Drugs Responsible Signature</div><div style="margin-top:38px;border-top:1px solid #1b2b44"></div></div></div>`;
...
document.addEventListener("click", event => {
  ...
  if (latestReturnBtn) {
    closeRxActionMenus();
    if (latestReturnBtn.closest("#page-narcotic, #narcoticDepartmentModal, #narcoticRecentModal")) return openNarcoticConfirmActionModal("return", latestReturnBtn.dataset.id);
