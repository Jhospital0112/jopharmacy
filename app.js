...
async function savePharmacist() {
  ...
  if (!name || !normalizedJobNumber || !pharmacies.length) {
  ...
  const duplicateInCache = APP.cache.pharmacists.find(
    p => p.active !== false && String(p.jobNumber || "").trim() === normalizedJobNumber && p.id !== APP.pharmacistEditId
  );
  if (duplicateInCache) {
  ...
  if (q("addPharmacistBtn")) q("addPharmacistBtn").disabled = true;
  if (q("savePharmacistBtn")) q("savePharmacistBtn").disabled = true;
  ...
  try {
    ...
    showActionModal(
      "Save Pharmacist Error",
      error?.message || "Unexpected error while saving pharmacist.",
      false
    );
    q("actionOkBtn").classList.remove("hidden");
  } finally {
    savingPharmacistNow = false;
    if (q("addPharmacistBtn")) q("addPharmacistBtn").disabled = false;
    if (q("savePharmacistBtn")) q("savePharmacistBtn").disabled = false;
  }
}
