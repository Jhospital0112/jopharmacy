...
async function setDoc(docRef, payload, options = {}) {
  ...
  if (options && options.merge) {
    ...
    const { error } = await supabase.from(normalizeTable(docRef.table)).upsert(merged, { onConflict: 'id' });
    if (error) throw error;
    return;
  }
  const { error } = await supabase.from(normalizeTable(docRef.table)).upsert(row, { onConflict: 'id' });
  if (error) throw error;
}

async function updateDoc(docRef, payload) {
  const { error } = await supabase.from(normalizeTable(docRef.table)).update(cleanData(payload)).eq("id", docRef.id);
  if (error) throw error;
}
...
async function addDoc(collectionRef, payload) {
  ...
  const { error } = await supabase
    .from(normalizeTable(collectionRef.table))
    .insert(cleanData({ id, ...payload }));
  if (error) throw error;
  return { id };
}

async function deleteDoc(docRef) {
  const { error } = await supabase.from(normalizeTable(docRef.table)).delete().eq("id", docRef.id);
  if (error) throw error;
}
