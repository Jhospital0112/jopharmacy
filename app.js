...
function applyCurrentUserReadonlyFields() {
  ...
  ["quickPharmacist","narcoticPharmacist","narcoticInternalReceivePharmacist"].forEach(id => {
    const el = q(id);
    if (!el) return;
...
async function doLogin(portalRole, employeeNumber, password) {
  ...
  if (portalRole === "ADMIN") {
    if (String(user.role || "").toUpperCase() !== "ADMIN") return finishActionModal(false, "This account is not allowed to open Admin portal.");
  } else if (!allowedPharmacies.includes(targetScope)) {
    return finishActionModal(false, "This account is not allowed to open the selected pharmacy portal.");
  }
...
function updateNarcoticReportPreview() {
  ...
  const rows = APP.cache.narcoticDepartmentStock.filter(r => {
    if (r.departmentId !== departmentId) return false;
    if (drugId && r.drugId !== drugId) return false;
    return Number(r.fixedStockUnits || 0) > 0 || Number(r.availableStockUnits || 0) > 0;
  });
  ...
  </tr>`).join("") || `<tr><td colspan="3" class="empty-state">No narcotic drugs with stock are assigned to this department.</td></tr>`;
  renderNarcoticReportPreviewCards(rows, previewType);
}
...
function printNarcoticStockReport() {
  ...
  const body = `
    ...
    <div class="sub"><strong>Department:</strong> ${esc(dept?.departmentName || "")} &nbsp; | &nbsp; <strong>Drug:</strong> ${esc(drug ? `${drug.tradeName || ""} ${drug.strength || ""}`.trim() : "Assigned department drugs only")}</div>
    <div class="section"><table><thead><tr><th>Drug</th><th>Fixed Stock</th><th>Available Stock</th></tr></thead>
    <tbody>${rows.map(r=>`<tr><td>${esc(r.drugName||"")}</td><td>${Number(r.fixedStockUnits||0)}</td><td>${Number(r.availableStockUnits||0)}</td></tr>`).join("") || `<tr><td colspan="3">No narcotic drugs with stock are assigned to this department.</td></tr>`}</tbody></table></div>
    <div class="section" style="margin-top:42px;display:flex;justify-content:flex-end"><div style="min-width:280px;text-align:center"><div style="font-weight:800">Pharmacist Signature</div><div style="margin-top:38px;border-top:1px solid #1b2b44"></div></div></div>`;
