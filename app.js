const APPS_SCRIPT_WEB_APP_URL = (window.CDMS_CONFIG && window.CDMS_CONFIG.APPS_SCRIPT_WEB_APP_URL) || "PASTE_YOUR_APPS_SCRIPT_WEB_APP_URL_HERE";
const POLL_INTERVAL_MS = 15000;
const UI_STATE_KEY = "cdms_ui_state_v1";
const SYNC_CACHE_KEY = "cdms_last_sync_fingerprint";

if (APPS_SCRIPT_WEB_APP_URL.includes("PASTE_YOUR_APPS_SCRIPT_WEB_APP_URL_HERE")) {
  alert("Google Apps Script backend is not configured yet. Open config.js and paste your deployed web app URL first.");
}

const db = { type: "google-sheets-api", url: APPS_SCRIPT_WEB_APP_URL };
...
async function getDoc(docRef) {
  const json = await apiRequest("getDoc", { table: docRef.table, id: docRef.id });
  return makeDocSnapshot(json.data || null);
}

async function applyFilters(rows, filters = []) {
  let result = [...(rows || [])];
  for (const f of filters) {
    if (!f) continue;
    if (f.op === "==" || f.op === "eq") result = result.filter(row => row?.[f.field] === f.value);
  }
  return result;
}

async function fetchRows(ref) {
  const json = await apiRequest("listDocs", { table: ref.table });
  const rows = json.data || [];
  return ref.kind === "query" ? applyFilters(rows, ref.filters) : rows;
}
...
function onSnapshot(ref, callback) {
  ...
  emit();
  const timer = setInterval(emit, POLL_INTERVAL_MS);
...
async function updateDoc(docRef, payload) {
  ...
  await apiRequest("updateDoc", { table: docRef.table, id: docRef.id, data: row });
}

async function addDoc(collectionRef, payload) {
  const row = cleanData({ id: crypto.randomUUID(), ...payload });
  await apiRequest("setDoc", { table: collectionRef.table, id: row.id, data: row });
  return { id: row.id };
}
...
function writeBatch() {
  ...
  return {
    ...
    async commit() {
      const payload = ops.map(([type, ref, data]) => ({ type, table: ref.table, id: ref.id, data }));
      await apiRequest("batch", { operations: payload });
    }
  };
}

const DEFAULT_PASSWORD = "111111";
// Internal app password only. Spreadsheet access itself is controlled by the Google Apps Script deployment.
...
function loadUiState() {
  try {
    APP.uiState = JSON.parse(localStorage.getItem(UI_STATE_KEY) || "{}") || {};
  } catch {
    APP.uiState = {};
  }
}
...
function saveUiState() {
  localStorage.setItem(UI_STATE_KEY, JSON.stringify(APP.uiState || {}));
}

function rememberFieldValue(id, value) {
  APP.uiState[id] = value;
  saveUiState();
}

function readRememberedFieldValue(id, fallback = "") {
  const remembered = APP.uiState?.[id];
  return remembered === undefined || remembered === null || remembered === "" ? fallback : remembered;
}

function bindPersistentField(id, eventName = "change") {
  const el = q(id);
  if (!el) return;
  el.addEventListener(eventName, () => rememberFieldValue(id, el.value));
}

function setSelectOptions(selectId, options, placeholder = null, preferredValue = "") {
  const el = q(selectId);
  if (!el) return;
  const currentValue = preferredValue || el.value || readRememberedFieldValue(selectId, "");
  const rendered = [];
  if (placeholder !== null) rendered.push(`<option value="">${esc(placeholder)}</option>`);
  for (const option of options || []) {
    if (typeof option === "string") {
      rendered.push(`<option value="${esc(option)}">${esc(option)}</option>`);
    } else {
      rendered.push(`<option value="${esc(option.value)}">${esc(option.label ?? option.value)}</option>`);
    }
  }
  ...
  if (fallbackValue) rememberFieldValue(selectId, fallbackValue);
}

function cacheFingerprint(payload) {
  return JSON.stringify({
    drugs: (payload?.tables?.drugs || []).map(r => [r.id, r.updatedAt, r.active]),
    inventory: (payload?.tables?.inventory || []).map(r => [r.id, r.updatedAt, r.totalUnits]),
    prescriptions: (payload?.tables?.prescriptions || []).slice(0, 250).map(r => [r.id, r.updatedAt, r.status, r.dateTime]),
    transactions: (payload?.tables?.transactions || []).slice(0, 250).map(r => [r.id, r.dateTime, r.type]),
    pharmacists: (payload?.tables?.pharmacists || []).map(r => [r.id, r.updatedAt, r.active]),
    settings: payload?.docs?.["settings:main"] || {}
  });
}
...
function openModal(id) {
  ...
  if (id === "actionModal") document.body.classList.add("action-modal-open");
}
...
function showActionModal(title, body, waiting = true) {
  ...
  if (!waiting) showToast(body.replace(/<[^>]+>/g, ""), "error");
}
...
function refreshScopedSelectors() {
  ...
  setSelectOptions("inventoryLocationFilter", inventoryOptions, null, readRememberedFieldValue("inventoryLocationFilter", scope));
  ...
  setSelectOptions("reportPharmacy", reportOptions, null, readRememberedFieldValue("reportPharmacy", isAdmin ? "ALL_WORK_PHARMACIES" : scope));
  q("reportPharmacy").disabled = false;

  const auditOptions = isAdmin ? WORK_PHARMACIES : [scope];
  setSelectOptions("auditPharmacy", auditOptions, null, readRememberedFieldValue("auditPharmacy", scope));
...
function bindListeners() {
  ...
  const syncAll = async (force = false) => {
    if (busy || !APP.currentUser || document.hidden) return;
    busy = true;
    try {
      const json = await apiRequest("syncState", {
        tables: ["drugs", "inventory", "prescriptions", "transactions", "pharmacists"],
        docs: [{ table: "settings", id: "main" }]
      });
      const nextFingerprint = cacheFingerprint(json);
      if (!force && APP.syncFingerprint && APP.syncFingerprint === nextFingerprint) return;
      APP.syncFingerprint = nextFingerprint;
      localStorage.setItem(SYNC_CACHE_KEY, nextFingerprint);
      const tables = json.tables || {};
      const docs = json.docs || {};
      APP.cache.drugs = (tables.drugs || []).filter(row => row.active !== false).sort((a, b) => String(a.tradeName || "").localeCompare(String(b.tradeName || "")));
      APP.cache.inventory = tables.inventory || [];
      APP.cache.prescriptions = (tables.prescriptions || []).sort((a, b) => String(b.dateTime || "").localeCompare(String(a.dateTime || "")));
      APP.cache.transactions = (tables.transactions || []).sort((a, b) => String(b.dateTime || "").localeCompare(String(a.dateTime || "")));
      APP.cache.pharmacists = (tables.pharmacists || []).filter(p => p.active !== false);
      APP.cache.settings = docs["settings:main"] || {};
      renderStaticOptions();
      restoreRememberedInputs();
      renderAll();
    } catch (error) {
      console.error("Sync error:", error);
      showToast("Sync failed. The system will retry automatically.", "error");
    } finally {
      busy = false;
    }
  };
  ...
  syncAll(true);
  const timer = setInterval(syncAll, POLL_INTERVAL_MS);
  APP.listeners.push(() => clearInterval(timer));
}
...
function restoreRememberedInputs() {
  Object.entries(APP.uiState || {}).forEach(([id, value]) => {
    const el = q(id);
    if (!el || el.tagName === "SELECT" || document.activeElement === el) return;
    el.value = value;
  });
}
...
function renderStaticOptions() {
  const drugOptions = APP.cache.drugs.map(d => ({ value: d.id, label: `${d.tradeName || ""} ${d.strength || ""}`.trim() }));
  ["quickDrug","shipmentDrug","transferDrug","reportDrug"].forEach(id => {
    if (q(id)) setSelectOptions(id, drugOptions, "Select Drug", readRememberedFieldValue(id, q(id).value));
  });

  const scope = currentScopePharmacy();
  const pharmacists = getScopePharmacists(scope).map(p => ({ value: p.name, label: p.name }));
  setSelectOptions("quickPharmacist", pharmacists, "Select Pharmacist", readRememberedFieldValue("quickPharmacist", q("quickPharmacist")?.value));
  if (q("editPharmacist")) setSelectOptions("editPharmacist", pharmacists, "Select Pharmacist", q("editPharmacist")?.value);
  setSelectOptions("auditAuditor", APP.cache.pharmacists.filter(p => p.canAudit && p.active !== false).map(p => ({ value: p.name, label: p.name })), "Select Auditor", readRememberedFieldValue("auditAuditor", q("auditAuditor")?.value));
  ...
  setSelectOptions("settingsPharmacy", WORK_PHARMACIES, null, readRememberedFieldValue("settingsPharmacy", APP.cache.settings.pharmacyType || "In-Patient Pharmacy"));
  ...
  setSelectOptions("settingsMonth", MONTHS, null, readRememberedFieldValue("settingsMonth", APP.cache.settings.month || MONTHS[new Date().getMonth()]));
...
q("settingsPharmacy").onchange = () => {
  rememberFieldValue("settingsPharmacy", q("settingsPharmacy").value);
  if (APP.currentRole !== "ADMIN") return;
  APP.cache.settings = { ...(APP.cache.settings || {}), pharmacyType: q("settingsPharmacy").value };
  rememberFieldValue("inventoryLocationFilter", q("settingsPharmacy").value);
  rememberFieldValue("auditPharmacy", q("settingsPharmacy").value);
  renderAll();
};
...
q("inventoryLocationFilter").onchange = () => { rememberFieldValue("inventoryLocationFilter", q("inventoryLocationFilter").value); renderInventory(); updateQuickAvailableStock(); };
q("reportPharmacy").onchange = () => { rememberFieldValue("reportPharmacy", q("reportPharmacy").value); renderReports?.(); };
q("auditPharmacy").onchange = () => { rememberFieldValue("auditPharmacy", q("auditPharmacy").value); renderAudit(); };
...
q("quickDrug").onchange = () => { rememberFieldValue("quickDrug", q("quickDrug").value); updateQuickAvailableStock(); };
...
["inventorySearch","transactionsSearch","drugCardsSearch","auditSearch","quickPharmacist","shipmentDrug","transferDrug","reportDrug","auditAuditor","settingsMonth","settingsYear","reportFrom","reportTo","quickPatient","quickFile","quickDoctor","quickBoxes","quickUnits"].forEach((id) => bindPersistentField(id, "input"));
["quickDrug","inventoryLocationFilter","reportPharmacy","auditPharmacy","quickPharmacist","shipmentDrug","transferDrug","reportDrug","auditAuditor","settingsMonth","settingsYear"].forEach((id) => bindPersistentField(id, "change"));
...
loadUiState();
APP.syncFingerprint = localStorage.getItem(SYNC_CACHE_KEY) || "";
themeInit();
