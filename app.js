...
function bindListeners() {
  ...
  APP.listeners.push(onSnapshot(collection(db, "prescriptions"), snap => {
    APP.cache.prescriptions = snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .map(row => ({ ...row, prescriptionType: normalizePrescriptionType(row.prescriptionType) }))
  ...
  q("recentPrescriptionsTbody").innerHTML = rows.map(row => {
    ...
    return `
      <tr>
        ...
        <td>${esc(normalizePrescriptionType(row.prescriptionType || ""))}</td>
...
function normalizePrescriptionType(value) {
  ...
  const map = {
    ...
    "Outpatient": "Outpatient prescription",
    "Clinic": "Outpatient prescription",
    "Regular": "Outpatient prescription",
    "وصفة مرضى خارجيين": "Outpatient prescription",
    "Inpatient prescription": "Inpatient prescription",
    "Inpatient": "Inpatient prescription",
    "وصفة مرضى داخليين": "Inpatient prescription",
    "Emergency prescriptions": "Emergency prescriptions",
    "Emergency": "Emergency prescriptions",
    "وصفة طوارئ": "Emergency prescriptions"
  };
  return map[raw] || raw;
}
...
async function continueQuickRegistration(payload) {
  ...
  try {
    ...
    await setDoc(prescriptionRef, {
      drugId, pharmacy, patientName, fileNumber, doctorName, pharmacistName, prescriptionType: normalizePrescriptionType(prescriptionType || ""), qtyBoxes, qtyUnits, status: "New", auditBy: "", auditDateTime: null, auditNote: "", returnBy: "", returnDateTime: null, returnNote: "", dateTime: jordanNowIso(), createdBy: APP.currentRole, updatedBy: APP.currentRole
    });
...
async function registerQuickPrescription() {
  const payload = {
    ...
    prescriptionType: normalizePrescriptionType(q("quickPrescriptionType")?.value || ""),
      ...
      <table>
        ...
        <tbody>
          ${rows.map(row => `
            ...
            <tr>
              <td colspan="7"><strong>Status:</strong> ${esc(row.status || "-")} &nbsp; | &nbsp; <strong>Prescription Type:</strong> ${esc(normalizePrescriptionType(row.prescriptionType || "-"))} &nbsp; | &nbsp; <strong>Audit Details:</strong> ${row.status === "Returned" ? "-" : esc((row.auditBy || "") + (row.auditDateTime ? ` • ${formatJordanDateTime(row.auditDateTime)}` : ""))}</td>
            </tr>
              ...
              <table>
                ...
                <tbody>
                  ${group.rows.map(row => `
                    ...
                    <tr>
                      <td colspan="7"><strong>Prescription Type:</strong> ${esc(normalizePrescriptionType(row.prescriptionType || "-"))} &nbsp; | &nbsp; <strong>Audit Details:</strong> ${row.status === 'Returned' ? '-' : esc((row.auditBy || '') + (row.auditDateTime ? ` • ${formatJordanDateTime(row.auditDateTime)}` : ''))}</td>
                    </tr>
                  `).join("")}
                </tbody>
              </table>
...
function enableAutoFocusFlow(fieldIds, onSubmit) {
  const ids = (fieldIds || []).filter(id => q(id));
  ids.forEach((id, index) => {
    const el = q(id);
    if (!el) return;
    el.addEventListener("keydown", event => {
      if (event.key !== "Enter") return;
      if (el.tagName === "TEXTAREA" && !event.ctrlKey) return;
      event.preventDefault();
      const next = ids[index + 1] ? q(ids[index + 1]) : null;
      if (next) {
        next.focus();
